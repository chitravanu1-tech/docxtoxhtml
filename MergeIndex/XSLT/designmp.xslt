<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="index" select="document('indexing.xml')"/>
    <xsl:variable name="stylecol" select="document('index.xml')"/>
    <!--<xsl:strip-space elements="*"/>-->
    <xsl:output method="xml" use-character-maps="Ascii2Unicode"/>
    ...
    <xsl:template match="Document">
       <xsl:copy>
           ...
           <xsl:element name="Index">
                ...
                <xsl:for-each select="$index//body//*//span[@class='Main' or matches(@class,'Sub([0-9]+)')]">
                    <!--Level 1-->
                    <xsl:if test="self::span[@class='Main']">
                        ...
                        <!--<xsl:if test="not(self::*[@data-dummy = $data-id])">-->
                            <xsl:if test="self::*[@data-seeallso='true']">
                                <xsl:variable name="data-id" select="self::*[@data-seeallso='true']/normalize-space(@data-id)"/>
                                <xsl:variable name="data-duplicate" select="self::*[@data-seeallso='true']/normalize-space(@data-duplicate)"/>
                                <xsl:for-each select="$stylecol//Chapter[seeAlsoFound[. ='true']][descendant::seeAlsoMark = $data-id or descendant::seeAlsoMark = substring-before($data-duplicate,'Sub') and not(key = $index//body//*//span[@class='Main' or matches(@class,'Sub([0-9]+)')]/@data-id)]/descendant::title">
                                
                                <xsl:variable name="duplicateterm" select="normalize-space(current())"/>
                                    <xsl:element name="Topic">
                                        <xsl:attribute name="Self" select="concat('di2', 'Topicn', $duplicateterm)"/>
                                        <xsl:attribute name="SortOrder" select="''"/>
                                        <xsl:attribute name="Name" select="replace($duplicateterm, '-', ':')"/>
                                    </xsl:element>
  
                                </xsl:for-each>
                            </xsl:if>
                        <xsl:element name="Topic">
                            <xsl:attribute name="Self" select="concat('di2', 'Topicn', $index1)"/>
                            <xsl:attribute name="SortOrder" select="''"/>
                            <xsl:attribute name="Name" select="replace($index1, '-', ':')"/>
                            
                            <xsl:if test="self::*[@data-seeallso='true']">
                                
                            <xsl:variable name="data-id" select="self::*[@data-seeallso='true']/normalize-space(@data-id)"/>
                                <xsl:variable name="data-duplicate" select="self::*[@data-seeallso='true']/normalize-space(@data-duplicate)"/>                                  
                                <xsl:for-each select="$stylecol//Chapter[seeAlsoFound[. ='true']][descendant::seeAlsoMark = $data-id or descendant::seeAlsoMark = $data-dummy or descendant::seeAlsoMark = substring-before($data-duplicate,'Sub')]/descendant::title">
                                    
                                        <xsl:variable name="crossText">
                                    ...
                                    <xsl:element name="CrossReference">
                                            ...
                                            <xsl:attribute name="ReferencedTopic">
                                                <xsl:value-of select="concat('di2', 'Topicn', $crossText)"/>
                                            </xsl:attribute>
                                ...
                                <!--<xsl:for-each select="$stylecol//Chapter[seeAlsoFound[. ='true']][bookmark = $data-id or bookmark = $data-dummy]/seeAlsoTemp">
                                    <xsl:variable name="sat" select="current()"/>
                                    
                                    <xsl:variable name="crossText1">
                                        <xsl:value-of select="$index//body//*//span[@class='Main'][@data-dummy = $sat]/text()"/>
                                    </xsl:variable>
                                    <xsl:element name="CrossReference">
                                        <xsl:attribute name="Self">
                                            <xsl:value-of select="'u'"/>
                                        ...
                                        <xsl:attribute name="ReferencedTopic">
                                            <xsl:value-of select="concat('di2', 'Topicn', $crossText1)"/>
                                        </xsl:attribute>
                                        <xsl:attribute name="CrossReferenceType">
                                            <xsl:value-of select="'SeeAlso'"/>
                                        </xsl:attribute>
                        ...
                        <!-- Level 2 -->
                            <xsl:for-each select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id,@class)=@data-mark or $data-duplicate=@data-mark] | //span[matches(@class,'Sub([0-9]+)')][not(@data-id)][$data-duplicate=@data-mark]">
                                
                                  <xsl:variable name="index2">
                                    <xsl:choose>
                                        <xsl:when test="//span[matches(@class,'Sub([0-9]+)')][concat($data-id,@class)=@data-mark or $data-duplicate=@data-mark]">
                                            <xsl:value-of select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id,@class)=@data-mark or $data-duplicate=@data-mark]/current()"/>
                                        </xsl:when>
                                        <xsl:otherwise>
                                            <xsl:value-of select="//span[matches(@class,'Sub([0-9]+)')][$data-duplicate=@data-mark]/current()"/>
                                        </xsl:otherwise>
                                    </xsl:choose>
                                </xsl:variable>
                                
                                <xsl:variable name="data-id2" select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id,@class)=@data-mark]/current()/@data-id"/>
                                <xsl:variable name="data-duplicate2" select="//span[matches(@class,'Sub([0-9]+)')][not(concat($data-id,@class)=@data-mark)][$data-duplicate=@data-mark]/current()/@data-duplicate"/>
                                ...
                                <xsl:element name="Topic">
                                    <xsl:attribute name="Self"
                                        select="concat('di2', 'Topicn', $index1, 'Topicn', $index2)"/>
                                    <xsl:attribute name="SortOrder" select="''"/>
                                    <xsl:attribute name="Name" select="replace($index2, '-', ':')"/>
                                    ...
                                    <xsl:if test="self::*[@data-seeallso='true']">
                                        ...
                                        <xsl:variable name="data-id" select="self::*[@data-seeallso='true']/normalize-space(@data-id)"/>
                                        <xsl:variable name="data-duplicate" select="self::*[@data-seeallso='true']/normalize-space(@data-duplicate)"/>
                                        <xsl:for-each select="$stylecol//Chapter[seeAlsoFound[. ='true']][descendant::seeAlsoMark = $data-id or descendant::seeAlsoMark = substring-before($data-duplicate,'Sub')]/descendant::title">
                                            <xsl:variable name="crossText">
                                                <xsl:value-of select="current()"/>
                                            </xsl:variable>
                                            <xsl:element name="CrossReference">
                                        ...
                                        <!--<xsl:for-each select="$stylecol//Chapter[seeAlsoFound[. ='true']][bookmark = $data-id]/seeAlsoTemp">
                                            <xsl:variable name="sat" select="current()"/>
                                            
                                            <xsl:variable name="crossText1">
                                                <xsl:value-of select="$index//body//*//span[@class='Main' or matches(@class,'Sub([0-9]+)')][@data-dummy = $sat]/text()"/>
                                            </xsl:variable>
                                            <xsl:element name="CrossReference">
                                                <xsl:attribute name="Self">
                                                    <xsl:value-of select="'u'"/>
                                    ...
                                    <!-- Level 3 -->
                                    <xsl:for-each select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id2,@class)=@data-mark or $data-duplicate2=@data-mark] | //span[matches(@class,'Sub([0-9]+)')][$data-duplicate2=@data-mark]">
                                    <xsl:variable name="index3">
                                            <xsl:choose>
                                                <xsl:when test="//span[matches(@class,'Sub([0-9]+)')][concat($data-id2,@class)=@data-mark or $data-duplicate2=@data-mark]">
                                                    <xsl:value-of select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id2,@class)=@data-mark or $data-duplicate2=@data-mark]/current()"/>
                                                </xsl:when>
                                                <xsl:otherwise>
                                                    <xsl:value-of select="//span[matches(@class,'Sub([0-9]+)')][$data-duplicate2=@data-mark]/current()"/>
                                                </xsl:otherwise>
                                            </xsl:choose>
                                        </xsl:variable>
                                        <xsl:variable name="data-id3" select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id2,@class)=@data-mark]/current()/@data-id"/>
                                        <xsl:variable name="data-duplicate3" select="//span[matches(@class,'Sub([0-9]+)')][not(concat($data-id2,@class)=@data-mark)][$data-duplicate2=@data-mark]/current()/@data-duplicate"/>
                                        <!--<xsl:if test="not(self::*[@data-dummy = $data-id3])">-->
                                            <xsl:if test="self::*[@data-seeallso='true']">
                                                <xsl:variable name="data-id" select="self::*[@data-seeallso='true']/normalize-space(@data-id)"/>
                                                <xsl:variable name="data-duplicate" select="self::*[@data-seeallso='true']/normalize-space(@data-duplicate)"/> 
                                                <xsl:for-each select="$stylecol//Chapter[seeAlsoFound[. ='true']][descendant::seeAlsoMark = $data-id or descendant::seeAlsoMark = $data-duplicate and not(key = $index//body//*//span[@class='Main' or matches(@class,'Sub([0-9]+)')]/@data-id)]/descendant::title">
                                        ...
                                        <xsl:element name="Topic">
                                            <xsl:attribute name="Self"
                                                select="concat('di2', 'Topicn', $index1, 'Topicn', $index2, 'Topicn', $index3)"/>
                                            <xsl:attribute name="SortOrder" select="''"/>
                                            <xsl:attribute name="Name"
                                                select="replace($index3, '-', ':')"/>
                                            <xsl:if test="self::*[@data-seeallso='true']">
                                                
                                                <xsl:variable name="data-id" select="self::*[@data-seeallso='true']/normalize-space(@data-id)"/>
                                                <xsl:variable name="data-duplicate" select="self::*[@data-seeallso='true']/normalize-space(@data-duplicate)"/>
                                                <xsl:for-each select="$stylecol//Chapter[seeAlsoFound[. ='true']][descendant::seeAlsoMark = $data-id or descendant::seeAlsoMark = $data-duplicate]/descendant::title">
                                                    <xsl:variable name="crossText">
                                                        <xsl:value-of select="current()"/>
                                                    </xsl:variable>
                                                    <xsl:element name="CrossReference">
                                             ...
                                             <!-- Level 4 -->
                                            <xsl:for-each select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id3,@class)=@data-mark or $data-duplicate3=@data-mark]|//span[matches(@class,'Sub([0-9]+)')][$data-duplicate3=@data-mark]">
                                               <!--<xsl:variable name="index4" select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id3,@class)=@data-mark]/current()"/>-->
                                                <xsl:variable name="index4">
                                                    <xsl:choose>
                                                        <xsl:when test="//span[matches(@class,'Sub([0-9]+)')][concat($data-id3,@class)=@data-mark or $data-duplicate3=@data-mark]">
                                                            <xsl:value-of select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id3,@class)=@data-mark or $data-duplicate=@data-mark]/current()"/>
                                                        </xsl:when>
                                                        <xsl:otherwise>
                                                            <xsl:value-of select="//span[matches(@class,'Sub([0-9]+)')][$data-duplicate3=@data-mark]/current()"/>
                                                        </xsl:otherwise>
                                                    </xsl:choose>
                                                </xsl:variable>
                                                 <xsl:variable name="data-id4" select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id3,@class)=@data-mark]/current()/@data-id"/>
                                                <xsl:variable name="data-duplicate4" select="//span[matches(@class,'Sub([0-9]+)')][not(concat($data-id3,@class)=@data-mark)][$data-duplicate3=@data-mark]/current()/@data-duplicate"/>
                                                ...
                                                <xsl:element name="Topic">
                                                     <xsl:attribute name="Self"
                                                         select="concat('di2', 'Topicn', $index1, 'Topicn', $index2, 'Topicn', $index3, 'Topicn', $index4)"/>
                                                     <xsl:attribute name="SortOrder" select="''"/>
                                                     <xsl:attribute name="Name"
                                                         select="replace($index4, '-', ':')"/>
                                                    <xsl:if test="self::*[@data-seeallso='true']">
                                                     ...
                                                     <xsl:for-each select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id4,@class)=@data-mark]">
                                                         ...
                                                         <xsl:element name="Topic">
                                                             <xsl:attribute name="Self"
                                                                 select="concat('di2', 'Topicn', $index1, 'Topicn', $index2, 'Topicn', $index3, 'Topicn', $index4, 'Topicn', $index5)"/>
                                                             <xsl:attribute name="SortOrder" select="''"/>
                                                             <xsl:attribute name="Name"
                                                                 select="replace($index5, '-', ':')"/>
                                                             ...
                                                             <xsl:for-each select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id5,@class)=@data-mark]">
                                                                 ...
                                                                 <xsl:element name="Topic">
                                                                     <xsl:attribute name="Self"
                                                                         select="concat('di2', 'Topicn', $index1, 'Topicn', $index2, 'Topicn', $index3, 'Topicn', $index4, 'Topicn', $index5, 'Topicn', $index6)"/>
                                                                     <xsl:attribute name="SortOrder" select="''"/>
                                                                     <xsl:attribute name="Name"
                                                                         select="replace($index6, '-', ':')"/>
                                                                     ...
                                                                     <xsl:for-each select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id6,@class)=@data-mark]">
                                                                         ...
                                                                         <xsl:element name="Topic">
                                                                             <xsl:attribute name="Self"
                                                                                 select="concat('di2', 'Topicn', $index1, 'Topicn', $index2, 'Topicn', $index3, 'Topicn', $index4, 'Topicn', $index5, 'Topicn', $index6, 'Topicn', $index7)"/>
                                                                             <xsl:attribute name="SortOrder" select="''"/>
                                                                             <xsl:attribute name="Name"
                                                                                 select="replace($index7, '-', ':')"/>
                                                                             ...
                                                                             <xsl:for-each select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id7,@class)=@data-mark]">
                                                                                 ...
                                                                                 <xsl:element name="Topic">
                                                                                     <xsl:attribute name="Self"
                                                                                         select="concat('di2', 'Topicn', $index1, 'Topicn', $index2, 'Topicn', $index3, 'Topicn', $index4, 'Topicn', $index5, 'Topicn', $index6, 'Topicn', $index7, 'Topicn', $index8)"/>
                                                                                     <xsl:attribute name="SortOrder" select="''"/>
                                                                                     <xsl:attribute name="Name"
                                                                                         select="replace($index8, '-', ':')"/>
                                                                                     <xsl:if test="self::*[@data-seeallso='true']">
                                                                                     ...
                                                                                     <xsl:for-each select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id8,@class)=@data-mark]">
                                                                                         ...
                                                                                         <xsl:element name="Topic">
                                                                                             <xsl:attribute name="Self"
                                                                                                 select="concat('di2', 'Topicn', $index1, 'Topicn', $index2, 'Topicn', $index3, 'Topicn', $index4, 'Topicn', $index5, 'Topicn', $index6, 'Topicn', $index7, 'Topicn', $index8,'Topicn', $index9)"/>
                                                                                             <xsl:attribute name="SortOrder" select="''"/>
                                                                                             <xsl:attribute name="Name"
                                                                                                 select="replace($index9, '-', ':')"/>
                                                                                             <xsl:if test="self::*[@data-seeallso='true']">
                                                                                             ...
                                                                                             <xsl:for-each select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id9,@class)=@data-mark]">
                                                                                                 ...
                                                                                                 <xsl:element name="Topic">
                                                                                                     <xsl:attribute name="Self"
                                                                                                         select="concat('di2', 'Topicn', $index1, 'Topicn', $index2, 'Topicn', $index3, 'Topicn', $index4, 'Topicn', $index5, 'Topicn', $index6, 'Topicn', $index7, 'Topicn', $index8,'Topicn', $index9,'Topicn', $index10)"/>
                                                                                                     <xsl:attribute name="SortOrder" select="''"/>
                                                                                                     <xsl:attribute name="Name"
                                                                                                         select="replace($index10, '-', ':')"/>
                                                                                                     <xsl:if test="self::*[@data-seeallso='true']">
                                                                                                     ...
                                                                                                     <xsl:for-each select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id10,@class)=@data-mark]">
                                                                                                         ...
                                                                                                         <xsl:element name="Topic">
                                                                                                             <xsl:attribute name="Self"
                                                                                                                 select="concat('di2', 'Topicn', $index1, 'Topicn', $index2, 'Topicn', $index3, 'Topicn', $index4, 'Topicn', $index5, 'Topicn', $index6, 'Topicn', $index7, 'Topicn', $index8,'Topicn', $index9,'Topicn', $index10,'Topicn', $index11)"/>
                                                                                                             <xsl:attribute name="SortOrder" select="''"/>
                                                                                                             <xsl:attribute name="Name"
                                                                                                                 select="replace($index11, '-', ':')"/>
                                                                                                             <xsl:if test="self::*[@data-seeallso='true']">
                                                                                                             ...
                                                                                                             <xsl:for-each select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id11,@class)=@data-mark]">
                                                                                                                 ...
                                                                                                                 <xsl:element name="Topic">
                                                                                                                     <xsl:attribute name="Self"
                                                                                                                         select="concat('di2', 'Topicn', $index1, 'Topicn', $index2, 'Topicn', $index3, 'Topicn', $index4, 'Topicn', $index5, 'Topicn', $index6, 'Topicn', $index7, 'Topicn', $index8,'Topicn', $index9,'Topicn', $index10,'Topicn', $index11,'Topicn', $index12)"/>
                                                                                                                     <xsl:attribute name="SortOrder" select="''"/>
                                                                                                                     <xsl:attribute name="Name"
                                                                                                                         select="replace($index12, '-', ':')"/>
                                                                                                                     <xsl:if test="self::*[@data-seeallso='true']">
                                                                                                                     ...
                                                                                                                     <xsl:for-each select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id12,@class)=@data-mark]">
                                                                                                                         ...
                                                                                                                         <xsl:element name="Topic">
                                                                                                                             <xsl:attribute name="Self"
                                                                                                                                 select="concat('di2', 'Topicn', $index1, 'Topicn', $index2, 'Topicn', $index3, 'Topicn', $index4, 'Topicn', $index5, 'Topicn', $index6, 'Topicn', $index7, 'Topicn', $index8,'Topicn', $index9,'Topicn', $index10,'Topicn', $index11,'Topicn', $index12,'Topicn', $index13)"/>
                                                                                                                             <xsl:attribute name="SortOrder" select="''"/>
                                                                                                                             <xsl:attribute name="Name"
                                                                                                                                 select="replace($index13, '-', ':')"/>
                                                                                                                             ...
                                                                                                                             <xsl:for-each select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id13,@class)=@data-mark]">
                                                                                                                                 ...
                                                                                                                                 <xsl:element name="Topic">
                                                                                                                                     <xsl:attribute name="Self"
                                                                                                                                         select="concat('di2', 'Topicn', $index1, 'Topicn', $index2, 'Topicn', $index3, 'Topicn', $index4, 'Topicn', $index5, 'Topicn', $index6, 'Topicn', $index7, 'Topicn', $index8,'Topicn', $index9,'Topicn', $index10,'Topicn', $index11,'Topicn', $index12,'Topicn', $index13,'Topicn', $index14)"/>
                                                                                                                                     <xsl:attribute name="SortOrder" select="''"/>
                                                                                                                                     <xsl:attribute name="Name"
                                                                                                                                         select="replace($index14, '-', ':')"/>
                                                                                                                                     <xsl:if test="self::*[@data-seeallso='true']">
                                                                                                                                     ...
                                                                                                                                     <xsl:for-each select="//span[matches(@class,'Sub([0-9]+)')][concat($data-id14,@class)=@data-mark]">
                                                                                                                                         ...
                                                                                                                                         <xsl:element name="Topic">
                                                                                                                                             <xsl:attribute name="Self"
                                                                                                                                                 select="concat('di2', 'Topicn', $index1, 'Topicn', $index2, 'Topicn', $index3, 'Topicn', $index4, 'Topicn', $index5, 'Topicn', $index6, 'Topicn', $index7, 'Topicn', $index8,'Topicn', $index9,'Topicn', $index10,'Topicn', $index11,'Topicn', $index12,'Topicn', $index13,'Topicn', $index14,'Topicn', $index15)"/>
                                                                                                                                             <xsl:attribute name="SortOrder" select="''"/>
                                                                                                                                             <xsl:attribute name="Name"
                                                                                                                                                 select="replace($index15, '-', ':')"/>
            ...
            <!--<xsl:element name="Index">
                <xsl:attribute name="Self" select="'di2'"/>
                <xsl:for-each select="$index//p[span[@class='Main'] | span[matches(@class,'Sub([0-9]+)')]]">
                    
                    <xsl:variable name="index1" select="descendant::span[@class = 'Main']"/>
                    <xsl:variable name="index2" select="p[@class = 'index2']"/>
                    <xsl:variable name="index3" select="p[@class = 'index3']"/>
                    <xsl:variable name="index4" select="p[@class = 'index4']"/>
                    <xsl:variable name="index5" select="p[@class = 'index5']"/>
                    <xsl:variable name="index6" select="p[@class = 'index6']"/>
                    <xsl:variable name="index7" select="p[@class = 'index7']"/>
                    <xsl:variable name="index8" select="p[@class = 'index8']"/>
                    <xsl:variable name="strlen2" select="string-length($index2)"/>
                    <xsl:variable name="strlen3" select="string-length($index3)"/>
                    <xsl:variable name="strlen4" select="string-length($index4)"/>
                    <xsl:variable name="strlen5" select="string-length($index5)"/>
                    <xsl:variable name="strlen6" select="string-length($index6)"/>
                    <xsl:variable name="strlen7" select="string-length($index7)"/>
                    <xsl:variable name="strlen8" select="string-length($index8)"/>
                    <xsl:element name="Topic">
                        <xsl:attribute name="Self" select="concat('di2', 'Topicn', $index1)"/>
                        <xsl:attribute name="SortOrder" select="''"/>
                        <xsl:attribute name="Name" select="replace($index1, '-', ':')"/>
                        <!-\- Level 2 -\->
                        <xsl:if test="$strlen2 > 2">
                            <xsl:element name="Topic">
                                <xsl:attribute name="Self"
                                    select="concat('di2', 'Topicn', $index1, 'Topicn', $index2)"/>
                                <xsl:attribute name="SortOrder" select="''"/>
     ...
     <!--Add character-map {For Index Purpose} 16.02.2017 -->
    ...
    <!--<xsl:template match="span[@data-seeallso='true']" name="seealso">
        <xsl:param name="see"/>
        <xsl:variable name="data-id" select="self::*[@data-seeallso='true']/@data-id"/>
        <xsl:variable name="data-duplicate" select="self::*[@data-seeallso='true']/@data-duplicate"/>
            <xsl:for-each select="$stylecol//Chapter[key =$data-id][seeAlsoTemp]/seeAlsoTemp|$stylecol//Chapter[key =$data-duplicate][seeAlsoTemp]/seeAlsoTemp">
                <!-\-<xsl:variable name="bindval" select="."/>-\->
                <xsl:value-of select="."/>
            </xsl:for-each>
    </xsl:template>-->
    <!--<xsl:template match="//Index"/>-->
    <xsl:template match="Topic/CrossReference[@ReferencedTopic = preceding-sibling::CrossReference/@ReferencedTopic]"/>    
</xsl:stylesheet>