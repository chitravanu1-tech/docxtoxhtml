						...
						<xsl:element name="w:r">
							<xsl:element name="w:rPr">
								<xsl:element name="w:rStyle">
									<!--<xsl:attribute name="w:val" select="translate($style,'_|‡| ','')"/>-->
									<xsl:attribute name="w:val" select="replace(replace($style,'_',''),'‡','')"/>
								</xsl:element>
							</xsl:element>
...
<xsl:otherwise>
	...
	<!-- XHTML hyperlink -->
	<xsl:template
		match="a[@href[not(contains(.,'BibRef'))] and not(contains(@class, 'footnotecitation') or contains(@href,'#ftnref') or contains(@class, 'endnotecitation')) and not(ancestor::span[(lower-case(@class) = 'endnotereference') or (lower-case(@class) = 'EndnoteNumber')])][not(@data-alias | @data-boxname | @data-boxtag | @data-id | @data-tag)]">
		<xsl:param name="pPr_Default"/>
		<xsl:param name="rPr_Default"/>
		<w:r>
	...
	<xsl:template match="text()">
		...
		<xsl:variable name="cstyle">
			...
			<xsl:choose>
				<xsl:when test="./ancestor::span[@data-name][1]">
					<xsl:value-of select="./ancestor::span[@data-name][1]/@data-name"
					/>
				</xsl:when>
				<!--<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="./ancestor::span[@class][1]">
							<xsl:value-of
								select="./ancestor::span[@class][1]/@class"/>
						</xsl:when>
						<xsl:otherwise>
							<!-\-   if there is no data-name and class in the element there is no style in the element-\->
						</xsl:otherwise>
					...
					<xsl:element name="w:r">
						<xsl:element name="w:rPr">
							...
							<xsl:if test="(string-length($cstyle) > 0)">
								<xsl:element name="w:rStyle">
									<!--<xsl:attribute name="w:val" select="replace(replace($cstyle,'_',''),'‡','')"/>-->
									<xsl:attribute name="w:val" select="replace(replace($cstyle,'_',''),'‡','')"/>
...
<!--								example w:eastAsia-->
		...
		<xsl:if test="string-length($highlight) > 0">
			...
			<xsl:if test="matches($highlight,'^#([a-z]+)$|^([a-z]+)$')">
...
<!--<xsl:template match="span[@data-commentgroup]">
	...
	<xsl:template match="//a[contains(@data-alias,'Suppl')]">
		<xsl:param name="pPr_Default"/>
		<xsl:param name="rPr_Default"/>
		<xsl:element name="w:hyperlink">
	...
	<!--<xsl:template match="//span[count(child::span) &gt; 1]">
		<xsl:for-each select="self::span[count(child::span) &gt; 1]">
			<xsl:apply-templates select="node() except child::span[last()]/following-sibling::text()[.=' ']|child::span[contains(@class,'volumeNumber')]/following-sibling::text()[.=' ']"/>
		</xsl:for-each>
	</xsl:template>-->
	</xsl:stylesheet>