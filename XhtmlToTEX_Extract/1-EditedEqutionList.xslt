...
<!--<xsl:template match="xhtml:body">
    ...
    <xsl:template match="/">
        <xsl:element name="root">
            <xsl:text>&#x000A;</xsl:text>
        <xsl:for-each select="//xhtml:span[@data-math-changed='true']">
            <xsl:element name="Equation">
                <xsl:attribute name="name"><xsl:value-of select="self::xhtml:span/@data-filename"/></xsl:attribute>
                <xsl:attribute name="latex"><xsl:value-of select="self::xhtml:span/@latex"/></xsl:attribute>
                <xsl:attribute name="type"><xsl:value-of select="self::xhtml:span/@data-type"/></xsl:attribute>
                <xsl:copy-of select="xhtml:math"/>
            </xsl:element>
            <xsl:text>&#x000A;</xsl:text>
        </xsl:for-each>
        </xsl:element>
