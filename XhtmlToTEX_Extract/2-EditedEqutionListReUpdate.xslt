...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs xhtml html" 
    ...
    <xsl:output method="xml" omit-xml-declaration="no" use-character-maps="a" indent="no" cdata-section-elements="alt"/>
    ...
    <xsl:template match="//xhtml:span[contains(@data-math-changed,'true')]">
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:attribute name="data-math-changed" select="'false'"/>
            <!--<xsl:variable name="bsl" select="concat(substring-before(@data-filename,'.tex'),'.bsl')"/>-->
            <xsl:variable name="bsl">
                <xsl:choose>
                    <xsl:when test="contains(@data-filename,'.tex')">
                        <xsl:value-of select="concat(substring-before(@data-filename,'.tex'),'.bsl')"/>
                    </xsl:when>
                    <xsl:when test="contains(@data-filename,'.pdf')">
                        <xsl:value-of select="concat(substring-before(@data-filename,'.pdf'),'.bsl')"/>
                    </xsl:when>
                    <xsl:when test="contains(@data-filename,'.eps')">
                        <xsl:value-of select="concat(substring-before(@data-filename,'.eps'),'.bsl')"/>
                    </xsl:when>
                </xsl:choose></xsl:variable>
            <xsl:variable name="bslName" select="document($bsl)//root//text()"/>
            <xsl:variable name="bslcode">
                <xsl:apply-templates select="$bslName"/>
            </xsl:variable>
            <xsl:attribute name="depth">
                <xsl:value-of select="substring-before(substring-after($bslcode,'Depth = '),'&#xA;')"/>
            </xsl:attribute>
            <xsl:attribute name="height">
                <xsl:value-of select="substring-before(substring-after($bslcode,'Height = '),'&#xA;')"/>
            </xsl:attribute>
            <xsl:attribute name="totalheight">
                <xsl:value-of select="substring-before(substring-after($bslcode,'TotalHeight = '),'&#xA;')"/>
            </xsl:attribute>
            <xsl:attribute name="width">
                <xsl:value-of select="substring-before(substring-after($bslcode,'Width = '),'&#xA;')"/>
            </xsl:attribute>
            <xsl:apply-templates select="node()"/>
        </xsl:copy>
    </xsl:template>
