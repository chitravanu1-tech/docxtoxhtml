...
$Tmp=~s{<grammarly\-desktop\-integration ([^\>]+)\>}{}gsi;
...
$Tmp=~s{<([^\>]+)\>(.*?)\[para([^\>]+)\]<\/\1>}{<para$3><$1>$2<\/$1>}gsi;



$Tmp=~s{\[(\w+)\]<\/(\w)([^\>]+)\>}{<\/$2$3>}gsi;

$Tmp=~s{\[(\w+)\_(\w+)\]<\/(\w)([^\>]+)\>}{<\/$3$4>\[$1\_$2\]}gsi;
...
while($Authors_Tmp=~m{<([^\>]+)\><FMAuthors>(.*?)<\/FMAuthors>}gsi)
{
	...
	my $Author_Tmp=$Authors;

	

while($Author_Tmp=~m{<\/fmauSurname>(.*?)<fmauGivenName>}gsi)
{
	my $Name=$1;
	...
	if($Name!~m{\,(\s?)$}gsi)
	{
		...
		$Err="1";
push(@Error,"\~Para No:$Paragraph_No Col No:$cnum Comma seperator missing in between author names");
	...
	while($Ast_Tmp=~m{\,(\s?)\*}gsi)
	{
		...
		$Err="1";
push(@Error,"\~Para No:$Paragraph_No Col No:$cnum Comma present before * symbol");
...
my $Back_After_Tmp2=$Tmp;

$Back_After_Tmp2=~s{<bold>(.*?)<\/bold>}{$1}gsi;

$Back_After_Tmp2=~s{<bold>}{}gsi;

$Back_After_Tmp2=~s{<\/bold>}{}gsi;



while($Back_After_Tmp2=~m{<([^\>]+)\><([^\>]+)>(Algorithm) (\w+)}gsi)
{
	my $FM_Keyword=$2;
	my $Paragraph_No=$1;

	
	if($FM_Keyword!~m{Algorithm}gs)
	{
		
	$Err="1";
push(@Error,"\~Para No:$Paragraph_No Col No:1 Warning: Check the Algorithm content without algorithm style");
	}
...
my $Back_After_Tmp111=$Tmp;

$Back_After_Tmp111=~s{<bold>(.*?)<\/bold>}{$1}gsi;

while($Back_After_Tmp111=~m{<([^\>]+)\><EMHead[A-Z]>(Author contributions)<\/EMHead[A-Z]>(.*?)Acknowledgment}gsi)
{
	my $FM_Keyword=$2;
	my $Paragraph_No=$1;
	...
	$Err="1";
push(@Error,"\~Para No:$Paragraph_No Col No:1 Check the author contribution and Acknowledgement order");
...
if($Subject_Type1!~m{(corrections|retractions|editorial)}gsi)
{
	#$FMNoteCorrespondence5=~s{<(\w([^\>]+))\>(.*?)<\/\1>}{}gsi;

		

	if($FMNoteCorrespondence8!~m{\@}gsi)
	{
		push(@Error,"\~Para No:1 Col No:1 Email ID is missing for correspondence");
	}
...
while($Keyword_Before_Tmp1=~m{<\/([^\>]+)\>\n<([^\>]+)\><FM([^\>]+)>}gsi)
{
	...
	if($FM_Keyword!~m{^(FM|Keywords|Abbreviations|TSComment|SummaryBL1)}gsi)
	{
		if($Front_Style!~m{(FMNote|FMAbbreviations|SummaryBL1)}gsi)
		{
		$Err="1";
...
push(@Error,"\~Para No:$Paragraph_No Col No:1 More then one figure number style present in word document");
	...
	$Figure_Caption10=~s{<bold>(.*?)<\/bold>}{$1}gsi;
	
	if($Figure_Caption10=~m{<figurenumber>(fig\.(\s?)(\d+)\.)}gsi)
	{
		$Err="1";
push(@Error,"\~Para No:$Paragraph_No Col No:1 Figure number format in Figure caption $1");
...
while($Figure_Tmp=~m{\[([^\]]+)\]<FigureCaption>(.*?)<\/FigureCaption>}gsi)
{
	...
	my $Figure_Caption14=$Figure_Caption;
...
push(@Error,"\~Para No:$Paragraph_No Col No:1 More then one figure number style present in word document");
	...
	if($Figure_Caption10=~m{<\/figuretext>}gsi)
	{
		...
		if($Figure_Caption101!~m{\.(\s?)<\/figuretext>}si)
	{
...
$Para_Tmp=~s{<para([^\>]+)\><para([^\>]+)\>}{<para$1>}gsi;

$Para_Tmp=~s{<para([^\>]+)\><([^\>]+)\><para([^\>]+)\>}{<para$1><$2>}gsi;
...
$Para_Tmp=~s{<figure ([^\>]+)\>}{}gsi;

$Para_Tmp=~s{<\/figure>}{}gsi;

$Para_Tmp=~s{<span(\d+) ([^\>]+)\>}{}gsi;
...
$Supp_Tmp=~s{<EqnXref>(.*?)<\/EqnXref>}{}gsi;

$Supp_Tmp=~s{<FigXref>(.*?)<\/FigXref>}{}gsi;

$Supp_Tmp=~s{<TabXref>(.*?)<\/TabXref>}{}gsi;

$Supp_Tmp=~s{<FigureNumber>(.*?)<\/FigureNumber>}{}gsi;

$Supp_Tmp=~s{<TableNumber>(.*?)<\/TableNumber>}{}gsi;
...
while($Supp_Tmp=~m{<([^\>]+)\><([^\>]+)\>(.*?)<\/\2>}gsi)
{
	...
	while($Content1=~m{(S(\d+) Fig|S(\d+) Table|S(\d+) Appendix|S(\d+) Text)}gsi)
	{
		my $Character_Style=$1;
		
		my $cnum = index($Content, $Character_Style) + 1;
		
		push(@Error,"\~Para No:$Paragraph_No Col No:$cnum Warning: $1 Supplementary citation found without SupplementaryMaterialXref style");
	}

	$Content2=~s{<EqnXref>(.*?)<\/EqnXref>}{}gsi;
	
	while($Content1=~m{(equation\(\w+\)\s?\(\d+\)|eqs\.(\s?)\(\d+\)|eqns\.(\s?)(\d+)|eqns\.(\s?)\((\d+)\))}gsi)
	{
		my $Character_Style=$1;
		
		my $cnum = index($Content, $Character_Style) + 1;
		
		push(@Error,"\~Para No:$Paragraph_No Col No:$cnum Warning: $1 Equation citation found without EqnXref style");
	}

	$Content3=~s{<BoxXref>(.*?)<\/BoxXref>}{}gsi;
	
	while($Content3=~m{(Box(\s?)(\d+))}gsi)
	{
		my $Character_Style=$1;
		
		my $cnum = index($Content, $Character_Style) + 1;
		
		push(@Error,"\~Para No:$Paragraph_No Col No:$cnum Warning: $1 Box citation found without BoxXref style");
	}

	$Content3=~s{<FigXref>(.*?)<\/FigXref>}{}gsi;

	$Content3=~s{<figurenumber>(.*?)<\/figurenumber>}{}gsi;
	
	while($Content3=~m{(Fig(s?)\.(\d+)|Figure(s?) (\d+)|Fig(s?) (\d+))}gsi)
	{
		my $Character_Style=$1;
		
		my $cnum = index($Content, $Character_Style) + 1;
		
		push(@Error,"\~Para No:$Paragraph_No Col No:$cnum Warning: $1 Figure citation found without FigXref style");
	}

	$Content4=~s{<TabXref>(.*?)<\/TabXref>}{}gsi;

	$Content4=~s{<Tablenumber>(.*?)<\/Tablenumber>}{}gsi;


	
	
	while($Content4=~m{(Table(s?) (\d+))}gsi)
	{
		my $Character_Style=$1;
		...
		push(@Error,"\~Para No:$Paragraph_No Col No:$cnum Warning: $1 Table citation found without TabXref style");
	}

	$Content5=~s{<SecXref>(.*?)<\/SecXref>}{}gsi;
	
	while($Content5=~m{(Secion(s?) (\d+)|Sec(\.) (\d+)|Sect\. (\d+))}gsi)
	{
		my $Character_Style=$1;
		
		my $cnum = index($Content, $Character_Style) + 1;
		
		push(@Error,"\~Para No:$Paragraph_No Col No:$cnum Warning: $1 Section citation found without SecXref style");
	}
...
while($Tab_Tmp1=~m{<tbody>(.*?)<\/tbody>}gsi)
{
	...
	$Table=~s{<bold>(.*?)<\/bold>}{$1}gsi;

	$Table=~s{<italic>(.*?)<\/italic>}{$1}gsi;
...
$Err="1";
push(@Error,"\~Para No:1 Col No:1 Table $Table_Count Row No:1 Col No:1 TableColumn head style or table Body style is missing. Check the style $Entry");
		}
...
#system("$File_Path\/UTF8.exe \"$Log_File1\"");
