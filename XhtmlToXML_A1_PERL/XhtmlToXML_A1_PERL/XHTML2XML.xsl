...
<ref-list><br/>
		...
		<xsl:when test="w:pPr/w:pStyle/@w:val='RefPara'"><refpara><p><xsl:apply-templates/></p></refpara><br/></xsl:when>
...
<xsl:if test="w:p/w:pPr/w:pStyle[@w:val='Footnote'] or //w:p/w:pPr/w:pStyle[@w:val='FootnoteExtract'] or //w:p/w:pPr/w:pStyle[@w:val='Endnote'] or //w:p/w:pPr/w:pStyle[@w:val='EMNotesTitle']">
<notes><br/>
   <xsl:for-each select="w:p"><xsl:choose><xsl:when test="w:pPr/w:pStyle/@w:val='EMNotesTitle'"><title><xsl:apply-templates/></title><br/></xsl:when></xsl:choose></xsl:for-each>
 <fn-group><br/>   
<xsl:for-each select="w:p">
...
<xsl:template match="w:p/table">
 <table frame="all" orient="port"/>
 
    <!-- Process colgroup first -->
    <xsl:apply-templates select="colgroup"/>
    <!-- Process tbody and rows (tr) -->
...
<xsl:template match="colgroup">
  <colgroup>
      <xsl:copy-of select="@*"/>
    <!-- Process each col element within colgroup -->
...
<xsl:template match="tr">
  <oasis:row>
    <!-- Process each cell (td or th) in the row -->
...
<xsl:template match="td | th">
  <xsl:choose>
    ...
    <xsl:when test="self::td">
      <td>
        <oasis:entry>
          <!-- Copy attributes from the td to the entry -->
          <xsl:apply-templates select="@*"/>
          <!-- Process the content of the td -->
          <xsl:apply-templates/>
        </oasis:entry>
      </td>
    </xsl:when>
    ...
    <xsl:otherwise>
      <th>
        <entry>
          <!-- Copy attributes from the th to the entry -->
          <xsl:apply-templates select="@*"/>
          <!-- Process the content of the th -->
          <xsl:apply-templates/>
        </entry>
      </th>
    </xsl:otherwise>
...
<!-- Template to copy attributes to the entry tag -->
<xsl:template match="@*">
