...
use File::BOM qw( :all );
use Config::INI::IniFiles;
use File::Find;
no warnings;
my $Year1 = strftime "%H%M%S", localtime;
my $Year = strftime "%Y", localtime;

my ($XML_File,$TXT_File,$ART_Name,$Tmp,$FileName,$ENT,$Thin,$Element_File,$Element);
...
#my $DTD_Element="$File_Path\\Elements.ini";
...
while($Authors_Tmp=~m{<(\d+)><FMAuthors>(.*?)<\/FMAuthors>}gsi)
{
	my $Paragraph_No=$1;
	my $Authors=$2;
	my $Authors1=$2;
...
push(@Error,"\~Para No:$Paragraph_No Col No:$cnum\'Warning:Dummy $Style is present in the author name'");
	...
	if($Affiliation_Path!~m{(JData\\LWW|Intellect|Brill|Medknow|BUP)}gsi)
	{
...
while($Back_After_Tmp=~m{<(\d+)><EMHead[A-Z]>(.*?)<\/EMHead[A-Z]>\n<(\d+)><FM}gsi)
{
	my $FM_Keyword=$1;
	my $Paragraph_No=$3;
...
while($Back_After_Tmp=~m{<(\d+)><EMAcknowledgment}gsi)
{
	my $FM_Keyword=$1;
	my $Paragraph_No=$3;
...
while($Arti_Tmp=~m{<(\d+)><FMArticleTitle>(.*?)<\/FMArticleTitle>}gsi)
{
	my $Paragraph_No=$1;
...
while($Authors_Tmp=~m{<(\d+)><FMNoteStudyGroupMembers>(.*?)<\/FMNoteStudyGroupMembers>}gsi)
{
	my $Paragraph_No=$1;
	my $Authors=$2;
	my $Authors1=$2;
	...
	$Authors=~s{<(fmaffInstitution|fmaffAddressLine|fmaffCountry|fmaffFax|fmaffPhone|fmaffEmail|AffXref|Affxrefonline|italic|bold|sup|sub|smallcaps|comment|query)>}{}gsi;
...
my $Affiliation_Path1=$ARGV[0];
	...
	if($Affiliation_Path1!~m{(JData\\LWW|Intellect|Brill|Medknow|BUP)}gsi)
	{
...
if($FM_Author_Bio=~m{<(\d+)><FMNoteAuthorBio>(.*?)<\/FMNoteAuthorBio>}gsi)
{
	my $Paragraph_No=$1;

	my $Author_Tmp=$Tmp;

	if($Author_Tmp!~m{<(\d+)><FMAuthors>(.*?)<\/FMAuthors>}gsi)
	{
		$Err="1";
...
if($FM_Notes_Text=~m{<(\d+)><FMNotesText>(.*?)<\/FMNotesText>}gsi)
{
	my $Paragraph_No=$1;
...
my $Graph_Abstract_Path=$ARGV[0];
	...
	if($Gra=~m{<(\d+)><(FMGrapAbstractParaFlushLeft|FMGrapAbstractParaInd)>(.*?)<\/(FMGrapAbstractParaFlushLeft|FMGrapAbstractParaInd)>}gsi)
		{
		my $Paragraph_No=$1;
...
while($Affiliation_Tmp=~m{<(\d+)><FMAffiliations>(.*?)<\/FMAffiliations>}gsi)
{
	my $Paragraph_No=$1;
	my $FMAffiliations=$2;
	...
	$FMAffiliations=~s{<(fmaffInstitution|fmaffAddressLine|fmaffCountry|fmaffFax|fmaffPhone|fmaffEmail|AffXref|Affxrefonline|italic|bold|sup|sub|smallcaps|comment|query)>}{}gsi;
...
push(@Error,"\~Para No:$Paragraph_No Col No:$cnum\'Warning: Free text $1 is not allowed in FMAffiliations'");
	}
	#}
...
while($Footnote_Tmp=~m{<(\d+)><Footnote>(.*?)<\/Footnote>}gsi)
{
	my $Paragraph_No=$1;
...
while($Correspondence_Tmp=~m{<(\d+)><FMNoteCorrespondence>(.*?)<\/FMNoteCorrespondence>}gsi)
{
	my $Paragraph_No=$1;
	...
	$FMNoteCorrespondence=~s{<(fmaffInstitution|fmcorrSuffix|fmcorrSurname|fmcorrGivenName|fmcorrPrefix|fmcorrDegree|fmaffAddressLine|fmaffCountry|fmaffFax|fmaffPhone|fmaffEmail|AffXref|Affxrefonline|italic|bold|sup|sub|smallcaps|comment|query|url)>}{}gsi;
...
if($Subject_Type11=~m{(Retraction)}gsi)
{

	my $Figure_Tmp=$Tmp;
	my $Figure_Tmp1=$Tmp;

	if($Figure_Tmp!~m{<FMLayAbstract}gsi)
	{
		push(@Error,"\~Para No:1 Col No:1\'For Retraction type artcles, lay summary abstract is not allowed'");
	}
...
while($FM_Article_Title1=~m{<(\d+)><(FMAbstractParaFlushLeft|FMAbstractParaInd)>(.*?)<\/FMAbstractParaFlushLeft|FMAbstractParaInd>}gsi)
{
	my $Title=$3;
...
if($FM_Article_Title1!~m{<(\d+)><HeadA>(.*?)<\/HeadA>}gsi)
{
...
if($FM_Article_Title1!~m{<(\d+)><HeadB>(.*?)<\/HeadB>}gsi)
{
...
if($FM_Article_Title1!~m{<(\d+)><HeadC>(.*?)<\/HeadC>}gsi)
{
...
if($FM_Article_Title1!~m{<(\d+)><HeadD>(.*?)<\/HeadD>}gsi)
{
...
if($FM_Article_Title1!~m{<(\d+)><HeadE>(.*?)<\/HeadE>}gsi)
{
...
if($FM_Article_Title1!~m{<(\d+)><HeadF>(.*?)<\/HeadF>}gsi)
{
...
while($FM_Article_Title1=~m{<(\d+)><HeadA>(.*?)<\/HeadA>}gsi)
{
...
while($FM_Article_Title1=~m{<(\d+)><HeadB>(.*?)<\/HeadB>}gsi)
{
...
while($FM_Article_Title1=~m{<(\d+)><HeadC>(.*?)<\/HeadC>}gsi)
{
...
while($FM_Article_Title1=~m{<(\d+)><HeadD>(.*?)<\/HeadD>}gsi)
{
...
while($FM_Article_Title1=~m{<(\d+)><HeadE>(.*?)<\/HeadE>}gsi)
{
...
while($FM_Article_Title1=~m{<(\d+)><HeadF>(.*?)<\/HeadF>}gsi)
{
...
if($Subject_Tmp134=~m{\nHeadArunIn=Yes}gsi)
{
	...
	if($FM_Article_Title1!~m{<(\d+)><([^>]+)\><HeadArunIn>(.*?)<\/HeadArunIn>}gsi)
	{
...
if($Subject_Tmp135=~m{\nHeadBrunIn=Yes}gsi)
{
	...
	if($FM_Article_Title1!~m{<(\d+)><([^>]+)\><HeadBrunIn>(.*?)<\/HeadBrunIn>}gsi)
	{
...
if($Subject_Tmp136=~m{\nHeadCrunIn=Yes}gsi)
{
	...
	if($FM_Article_Title1!~m{<(\d+)><([^>]+)\><HeadCrunIn>(.*?)<\/HeadCrunIn>}gsi)
	{
...
if($Subject_Tmp137=~m{\nHeadDrunIn=Yes}gsi)
{
	...
	if($FM_Article_Title1!~m{<(\d+)><([^>]+)\><HeadDrunIn>(.*?)<\/HeadDrunIn>}gsi)
	{
...
if($Subject_Tmp138=~m{\nHeadErunIn=Yes}gsi)
{
	...
	if($FM_Article_Title1!~m{<(\d+)><HeadErunIn>(.*?)<\/HeadErunIn>}gsi)
	{
...
if($Subject_Tmp139=~m{\nHeadFrunIn=Yes}gsi)
{
	...
	if($FM_Article_Title1!~m{<(\d+)><HeadFrunIn>(.*?)<\/HeadFrunIn>}gsi)
	{
...
if($Subject_Tmp140=~m{\nHeadArunIn=No}gsi)
{
	...
	while($FM_Article_Title1=~m{<(\d+)><([^>]+)\><HeadArunIn>(.*?)<\/HeadArunIn>}gsi)
	{
...
if($Subject_Tmp141=~m{\nHeadBrunIn=No}gsi)
{
	...
	while($FM_Article_Title1=~m{<(\d+)><([^>]+)\><HeadBrunIn>(.*?)<\/HeadBrunIn>}gsi)
	{
...
if($Subject_Tmp142=~m{\nHeadCrunIn=No}gsi)
{
	...
	while($FM_Article_Title1=~m{<(\d+)><([^>]+)\><HeadCrunIn>(.*?)<\/HeadCrunIn>}gsi)
	{
...
if($Subject_Tmp143=~m{\nHeadDrunIn=No}gsi)
{
	...
	while($FM_Article_Title1=~m{<(\d+)><([^>]+)\><HeadDrunIn>(.*?)<\/HeadDrunIn>}gsi)
	{
...
if($Subject_Tmp144=~m{\nHeadErunIn=No}gsi)
{
	...
	while($FM_Article_Title1=~m{<(\d+)><([^>]+)\><HeadErunIn>(.*?)<\/HeadErunIn>}gsi)
	{
...
if($Subject_Tmp145=~m{\nHeadFrunIn=No}gsi)
{
	...
	while($FM_Article_Title1=~m{<(\d+)><([^>]+)\><HeadFrunIn>(.*?)<\/HeadFrunIn>}gsi)
	{
...
while($FM_Article_Title1=~m{<(\d+)><Tablecaption>(.*?)<\/Tablecaption>}gsi)
{
...
while($FM_Article_Title1=~m{<(\d+)><Figurecaption>(.*?)<\/Figurecaption>}gsi)
{
...
while($FM_Article_Title1=~m{<(\d+)><BoxTitle>(.*?)<\/BoxTitle>}gsi)
{
...
while($FM_Article_Title1=~m{<(\d+)><Reference>(.*?)<\/Reference>}gsi)
{
...
while($FM_Article_Title1=~m{<(\d+)><Footnote>(.*?)<\/Footnote>}gsi)
{
	my $Paragraph_No=$1;
...
while($FM_Article_Title1=~m{<(\d+)><refauGivenName>(.*?)<\/refauGivenName>}gsi)
{
	my $Paragraph_No=$1;
	my $Italic=$2;
	my $Italic1=$2;
...
if($Art_Type=~m{\n$Journal\t$Art_Subject\t([^\t]+)\t([^\t]+)\t([^\n]*)}gsi)
{
	...
	if($FM_Article_Type=~m{<FMArticleType>$Subject}gsi)
	{
		...
		if($Guest_Editor=~m{^(\w(.*?))$}gsi)
		{
			...
			if($FM_Guest_Editor=~m{<(\d+)><FMNoteGuestEditor>(.*?)<\/FMNoteGuestEditor>}gsi)
			{
				my $Editor_Names=$1;
...
while($Art_Tmp=~m{<(\d+)><FMArticleType>(.*?)<\/FMArticleType>}gsi)
	{
		my $Title=$2;
...
while($Art_Tmp=~m{<(\d+)><(HeadA|HeadB|HeadC|HeadD|HeadE|HeadF)>(.*?)<\/HeadA|HeadB|HeadC|HeadD|HeadE|HeadF>}gsi)
	{
		my $Title=$2;
...
if($JBREIM_Journal=~m{(JBREIM)}gsi)
{
	...
	while($Abstract_Tmp=~m{<(\d+)><(FMGrapAbstract)}gsi)
	{
		my $Paragraph_No=$1;
...
push(@Error,"\~Para No:$Paragraph_No Col No:1\'FMGraphAbstarct is not allowed for JBREIM\'");
	...
	while($Article_Type_Tmp=~m{<(\d+)><FMArticleType>(.*?)<\/FMArticleType>}gsi)
	{
		my $Title=$2;
		my $Paragraph_No=$1;
...
push(@Error,"\~Para No:$Paragraph_No Col No:1\'Check the case sensitive in FMArticle type\'");
	...
	while($Abstract_Tmp=~m{<(\d+)><FMAuthors>(.*?)<\/FMAuthors>}gsi)
	{
	...
	if($Conflict_Tmp!~m{<(\d+)><([^\>]+)\>Conflict of interest}gsi)
	{
		my $Style=$2;
...
if($NEOUNC_Journal=~m{(NEOUNC)}gsi)
{
	...
	if($Abstract_Tmp=~m{<(\d+)><(BoxTitle)>key point}gsi)
	{
		my $Paragraph_No=$1;
...
if($NEOUNC_Journal=~m{(Intellect)}gsi)
{
	...
	if($Abstract_Tmp!~m{<(\d+)><EMReferencesHead>Suggested Citation}gsi)
	{
		my $Paragraph_No=$1;
...
if($EJILAW_Journal=~m{(EJILAW)}gsi)
{
	...
	if($Abstract_Tmp=~m{<(\d+)><FMNoteCorrespondence>}gsi)
	{
		my $Paragraph_No=$1;
...
if($ANNWEH_Journal=~m{(ANNWEH)}gsi)
{
	...
	if($Abstract_Tmp!~m{What\'s important about this paper}gsi)
	{
		my $Paragraph_No=$1;

		$Err="1";
		
push(@Error,"\~Para No:$Paragraph_No Col No:1\'what\'s important about this paper is must for Article type $Article_Type\'");
	}
	}
	}
...
if($JOFORE_Journal=~m{(JOFORE|FORSCI)}gsi)
{
	...
	while($Abstract_Tmp=~m{<(\d+)><(FMAbstractHead)>(Management and Policy Implication)}gsi)
	{
		my $Paragraph_No=$1;
...
while($Study_Tmp=~m{<(\d+)><([^\>]+)\>Study Implication(s?)<\/\2>}gsi)
{
	my $Paragraph_No=$1;
	my $Content=$3;
	my $Content1=$3;
...
while($Abstract_Tmp=~m{<(\d+)><(FMAbstractParaFlushLeft|FMAbstractParaInd)>(.*?)<\/FMAbstractParaFlushLeft|FMAbstractParaInd>}gsi)
{
	my $Paragraph_No=$1;
	my $Abstract=$3;
	my $Abstract1=$3;
	...
	$Abstract=~s{<(fmabstractsectionHeadrunIn|italic|bold|sup|sub|smallcaps|comment|query|url)>}{}gsi;
...
if($EXBOTJ_Jou=~m{EXBOTJ}gsi)
{
	...
	while($Supplement=~m{<(\d+)><(SupplementaryMaterialCaption)><bold>}gsi)
	{
		push(@Error,"\~Para No:$1 Col No:1\'Bold format applied for citation inside SupplementaryMaterialCaption'");
	}

	while($Editor=~m{<(\d+)><(FMRunningHeadRecto|FMRunningHeadVerso)>}gsi)
	{
		push(@Error,"\~Para No:$1 Col No:1\'Bold format applied for citation inside SupplementaryMaterialCaption'");
	}
...
if($FM_Article_Type=~m{<FMArticleType>(.*?)<\/FMArticleType>}gsi)
{
	...
	{
		if($Tmp=~m{<(\d+)><(FMAbstractParaFlushLeft|FMAbstractParaInd)>}gsi)
		{
			my $Paragraph_No=$1;
			my $Style=$2;
...
push(@Error,"\~Para No:$Paragraph_No Col No:1 Abstarct is not allowed for $Article_Type Article_type");
		...
		if($Abs_Tmp!~m{<(\d+)><(ParaFlushLeft|ParaInd)><bold>}gsi)
		{
			my $Paragraph_No=$1;
			my $Style=$2;
...
if($Abstract_Jou=~m{(JMAMMA|OCCMED|CROCOL|NOAJNL|AMJHSP|ANNBOT|BIOLIN|BOTLIN|CLINID|FORSCI|IBDJNL|INFDIS|JOFORE|JPIDSJ|LABMED|NEUONC|NOPRAC|OFIDIS|ZOOLIN|ENDOCR|JCEMET|JESOCI)}gsi)
{
	...
	while($Abs_Tmp=~m{<(\d+)><(FMAbstractHead)>(.*?)<\/FMAbstractHead>}gsi)
	{
		my $Paragraph_No=$1;
...
if($Abstract_Jou11=~m{(BIOLIN|BOTLIN|ZOOLIN)}gsi)
{
	...
	while($Abs_Tmp11=~m{<(\d+)><HeadB>(.*?)<\/HeadB>}gsi)
	{
		my $Paragraph_No=$1;
...
if($Abstract_Jou1=~m{(PPAREP|EARLYJ|INTIMM|CWWRIT|ANFRON)}gsi)
{
	...
	while($Abs_Tmp1=~m{<(\d+)><(FMAbstract)}gsi)
	{
		my $Paragraph_No=$1;
...
if($Abstract_Jou2=~m{(AESAME|AMTEST|AOBPLA|ASJOFJ|DSCILJ|ENVENT|GERONA|GERONB|GERONT|GERONI|ISDIVE|JEENTO|JIPMAN|JISESA|JMENTO|MSPECI|TRANAS|JANSCI|INSILC|AJCPAT|ASJOUR|JBREIM|PUBOPQ|ENDREV)}gsi)
{
	...
	if($Abs_Tmp2!~m{<(\d+)><(FMAbstractHead)}gsi)
	{
		my $Paragraph_No=$1;
...
while($Extract_Tmp=~m{<(\d+)><Extract>(.*?)<\/Extract>}gsi)
{
	my $FM_Keyword=$1;
	my $Paragraph_No=$1;
...
while($Keyword_Tmp=~m{<(\d+)><Keywords>(.*?)<\/Keywords>}gsi)
{
	my $Paragraph_No=$1;
	my $Keyword=$2;
	my $Keyword1=$2;
...
while($Keyword_Tmp1=~m{<(\d+)><KeywordsTrans>(.*?)<\/KeywordsTrans>}gsi)
{
	my $Paragraph_No=$1;
	my $Keyword=$2;
	my $Keyword1=$2;
...
while($Abbre_Child_Tmp=~m{<(\d+)><Abbreviations>(.*?)<\/Abbreviations>}gsi)
{
	my $Paragraph_No=$1;
	my $Abbreviations=$2;
...
while($FM_Orcid_Tmp=~m{<(\d+)><FMOrcid>(.*?)<\/FMOrcid>}gsi)
{
	my $Paragraph_No=$1;
	my $Orcid=$2;
...
while($Abbre_Child_Tmp=~m{<(\d+)><FMNoteGuestEditor>(.*?)<\/FMNoteGuestEditor>}gsi)
{
	my $Paragraph_No=$1;
	my $Abbreviations=$2;
...
while($Table_Tmp=~m{<(\d+)><TableCaption>(.*?)<\/TableCaption>}gsi)
{
	my $Paragraph_No=$1;
	my $Table_Caption=$2;
...
{
	if($Reprint_Tmp=~m{<(\d+)><([^\>]+)\>(Reprints will not be available)}gsi)
	{
		my $Style=$2;
	...
	if($Reprint_Tmp1=~m{<(\d+)><([^\>]+)\>(This manuscript was handled by)}gsi)
	{
		my $Style=$2;
...
{
	if($Conflict_Tmp=~m{<(\d+)><FMText>Conflict}gsi)
	{
		my $Paragraph_No=$1;
		$Err="1";
...
{
	if($Version_Tmp=~m{<(\d+)><([^\>]+)\>Version_of_Record}gsi)
	{
		my $Style=$2;
...
while($Table_Footnote=~m{<(\d+)><TableNote>(.*?)<\/TableNote>}gsi)
{
	my $Paragraph_No=$1;
	my $Table_Note=$2;
...
while($Table_Footnote1=~m{<(\d+)><TableNote>(.*?)<\/TableNote>}gsi)
{
	my $Paragraph_No=$1;
	my $Table_Note=$2;
	if($Table_Note=~m{^(\s?)<sub>}gsi)
...
while($Figure_Tmp=~m{<(\d+)><FigureCaption>(.*?)<\/FigureCaption>}gsi)
{
	my $Paragraph_No=$1;
	my $Figure_Caption=$2;
...
while($Figure_Tmp=~m{<(\d+)><ModelCaption>(.*?)<\/ModelCaption>}gsi)
{
	my $Paragraph_No=$1;
	my $Figure_Caption=$2;
...
{
my $FM_Article_Title1=$Tmp;
while($FM_Article_Title1=~m{<(\d+)><(HeadA|HeadB|HeadC|HeadD|HeadE)>(.*?)<\/\2>}gsi)
{
...
if($New_Jou11=~m{^(PG9|MD|MD9|MPG|IA9|CP9|HM9|JD9|MC9|PN9|RD9|BS9|CD9|JR9|JP9)$}gsi)
{
	...
	while($Subject_Tmp4444=~m{<(\d+)><(HeadA|HeadB|HeadC|HeadD)>(.*?)<\/\2>}gs)
	{
	my $Paragraph_No=$1;
	...
	while($Subject_Tmp5555=~m{<(\d+)><(EMReferencesHead)>(.*?)<\/\2>}gs)
	{
	my $Paragraph_No=$1;
...
while($Arti_Tmp11=~m{(Zoobank|Nomenclature)}gsi)
{
	...
	if($Arti_Tmp12!~m{<(\d+)><FMVersionofRecord>}gs)
	{
...
$Para_Tmp=~s{<\/(fmsubjectCode|fmdoi|fmauSurname|fmauGivenName|fmauPrefix|fmauSuffix|fmauDegree|fmcorrSurname|fmcorrGivenName|fmcorrPrefix|fmcorrSuffix|fmcorrDegree|fmauCollab|fmaffInstitution|fmaffAddressLine|fmaffCountry|fmaffFax|fmaffPhone|fmaffEmail|fmmsHistoryreceivedDate|fmmsHistoryrevisedRequestDate|fmmsHistoryrevisedDate|fmmsHistoryacceptedDate|fmabstractsectionHeadrunIn|keywordsHead|keywords|abbreviation|abbreviationExpansion|abbreviationsHead|fmcopyrightDate|fmcopyrightHolder|headArunIn|headBrunIn|headCrunIn|headDrunIn|headErunIn|label|listheadrunIn|emappendixNumber|emauthorBioname|glossaryTerm|glossaryDefinition|refanonymous|refauCollab|refedCollab|reftransCollab|refassigneeCollab|refcompilerCollab|refguestedCollab|refinventorCollab|refauSurname|refedSurname|reftransSurname|reftransedSurname|refassigneeSurname|refcompilerSurname|refdirectorSurname|refguestedSurname|refinventorSurname|refauGivenName|refedGivenName|reftransGivenName|reftransedGivenName|refassigneeGivenName|refcompilerGivenName|refdirectorGivenName|refguestedGivenName|refinventorGivenName|refauPrefix|refedPrefix|reftransPrefix|reftransedPrefix|refassigneePrefix|refcompilerPrefix|refdirectorPrefix|refguestedPrefix|refinventorPrefix|refauSuffix|refedSuffix|reftransSuffix|reftransedSuffix|refassigneeSuffix|refcompilerSuffix|refdirectorSuffix|refguestedSuffix|refinventorSuffix|refaffiliation|refetal|reftitleArticle|reftitleChapter|reftitlePatent|reftitleDiscussion|reftitleTransArticle|reftitleTransChapter|reftitleTransPatent|reftitleTransCommunication|reftitleTransDiscussion|reftitleJournal|reftitleBook|reftitleThesis|reftitleWebsite|refcommunicationType|refdiscussionType|refidPatentNumber|reftitleTransJournal|reftitleTransBook|reftitleTransThesis|reftitleTransWebsite|refpubdateYear|refpubdateMonth|refpubdateDay|refpubdateTime|refpubdateSeason|refaccessDate|refvolumeNumber|refedition|refconferenceName|refconferenceDate|refconferencePlace|refissueNumber|refsupplement|refpublisherLocation|refpublisherName|refpageFirst|refpageLast|refpageCount|refseriesTitle|refidDOI|refidCrossref|refidPMID|refannotationinline|refnumber|refURL|refconferenceSponsor|refidGovernmentReportNumber|refidISBN|refidISSN|refissueTitle|refidStandardsNumber|boxnumberrunIn|supplementaryMaterialnumber|videonumber|equationDisplayNumber|chemicalStructureNumber|URL|pi|query|BibXref|UIXref|BibXrefonline|FnXref|FnXrefonline|TabXref|Eqnxref|FigXref|modxref|videoxref|genxref|AffXref|AffXrefonline|allcaps|allcapsbold|allcapsbolditalic|allcapsitalic|bold|bolditalic|italic|math|mathitalic|smallcaps|strikethrough|sub|subbold|subbolditalic|subitalic|sup|supbold|supbolditalic|supitalic|underline|comment|EMNotes|EMNotesText|EMHeadA|EMText|EMTextInd|FMText|FMVersionofRecord|FMNotesText|EMNotesText|fmgrantSponsor|bibxrefonline|SupplementaryMaterialXref|FMNoteGuestEditor|FMTransAbstractParaInd|FMTransAbstractParaFlushLeft|FMTransAbstractHead|FMTransAbstractSectionHeadDisplayed|FMGrapAbstractHead|FMGrapAbstractParaFlushLeft|FMGrapAbstractParaInd|FMVideoAbstractParaFlushLeft|FMVideoAbstractParaInd|FMVideoAbstractHead|KeywordsTrans|TSComment|PullQuotes|FMLayAbstractHead|FMLayAbstractParaFlushLeft|FMLayAbstractParaInd|Pests|ExtractBL1|ExtractBL2|ExtractBL3|ExtractNL1|ExtractNL2|ExtractNL3|ExtractOL1|ExtractOL2|ExtractOL3|FMSummaryHead|FMSummaryParaFlushLeft|FMSummaryParaInd|OrcXref|FMNoteReport|FMNoteReprint|FMNoteStudyGroupMembers|FootnoteExtract)\/>}{}gsi;
...
while($Nested_Para=~m{<(\d+)><(FMSectionTitle|FMSubsectionTitle|FMSectionEditor|FMSubjectCodes|FMCustomMetaLogo|FMCustomMetaPatientConsent|FMCustomMetaEthicsCommittee|FMSeriesTitle|FMSeriesText|FMArticleType|FMDOILine|FMRunningHeadRecto|FMRunningHeadVerso|FMArticleTitle|FMArticleSubtitle|FMArticleTranslatedTitle|FMArticleTranslatedSubtitle|FMRelatedArticle|FMCiteThisArticle|FMAbstractHead|FMAbstractSectionHeadDisplayed|FMTranslatedAbstract|FMNoteAuthorBio|ParaFlushLeft|ParaInd|HeadA|HeadB|HeadC|HeadD|HeadE|HeadF|HeadG|BL1|BL2|BL3|BL4|BL5|OL1|OL2|OL3|OL4|OL5|ListPara|UL1|UL2|UL3|ExtractHead|Extract|ExtractTextInd|ExtractSpaceAboveSecondExtract|ExtractList|ExtractSubList|ExtractTranslation|ExtractSource|Epigraph|EpigraphTextInd|EpigraphSource|PoetryExtract|PoetryExtractSpaceAboveStanzaBreak|PoetryExtractSource|DialogueExtract|DialogueExtractSource|Epilogue|EpilogueTextInd|EpilogueSource|Footnote|EMAcknowledgmentsHead|EMAcknowledgmentsText|EMAppendixNumber|EMAppendixTitle|EMNotesTitle|EMGlossaryTitle|EMGlossaryEntry|EMOtherSectionTitle|EMReferencesHead|RefAnnotationPara|BoxNumber|BoxTitle|BoxSubtitle|BoxParaFlushLeft|BoxParaInd|BoxHeadA|BoxHeadB|BoxHeadC|BoxExtract|BoxExtractTextInd|BoxExtractSource|BoxBL1|BoxOL1|BoxUL1|BoxBL2|BoxOL2|BoxUL2|BoxBL|BoxOL|BoxUL|BoxBL|BoxOL|BoxUL|BoxNote|BoxSource|TableNumber|TableColumnHead|TableStubEntry|TableStubSubentry|TableBody|TableNote|TableSource|TableHeadA|TableHeadB|TableHeadC|FigureNumber|FigureNote|FigureSource|EMSupplementaryMaterialTitle|SupplementaryMaterialNumber|SupplementaryMaterialCaption|SupplementaryMaterialNote|SupplementaryMaterialSource|VideoNumber|VideoNote|VideoSource|ChemicalStructureDisplay|FormalStatement|FormalStatement|ResponseArticleSeparator|SubarticleSeparator|NoteToComp|EMNotes|EMNotesText|EMHeadA|EMHeadB|EMText|EMTextInd|FMNotesText|EMNotesText|FMTransAbstractParaInd|FMTransAbstractParaFlushLeft|FMTransAbstractHead|FMTransAbstractSectionHeadDisplayed|FMVideoAbstractParaFlushLeft|FMVideoAbstractParaInd|FMVideoAbstractHead|WhiteSpaceSectionBreak|ExtractBL1|ExtractBL2|ExtractBL3|ExtractNL1|ExtractNL2|ExtractNL3|ExtractOL1|ExtractOL2|ExtractOL3|FMNoteContributions|FMNoteConflict|FMNotePreviousAddress|FMNoteDeceased|FMNoteFunding|FMNoteConferenceHistory|FMNotePresent|FMNotePresentAddressDetails|FMNoteLevel|FMCustomMetaLogo|FMNoteHandlingEditor|FMNoteDisclaimer|FMNoteAttributeHead|FMNoteContributors|FMNoteEditorBio|FMNoteURL|FMNoteArticleCitation|FMNoteClinicalTrailInfo|FMNoteEthicalApprovalStatement|FMNoteOnline|FMNoteCodeDataRequest|FMMSHistoryDetails|FMNoteConsent|FMNoteSubmitted|FMNoteMiscellaneous|FMNoteIRBInfo|FMNoteDiscussant|FMNoteMemberID|FMNoteTweet|FMNotePatentDetails|FMNoteSummary|FMNoteOrcid|FMNotePublished|FMNoteWebsite|FMNoteRoyalty|FMNoteCopyrightStatement|FMNoteProductInfo)>(.*?)<\/(FMSectionTitle|FMSubsectionTitle|FMSectionEditor|FMSubjectCodes|FMCustomMetaLogo|FMCustomMetaPatientConsent|FMCustomMetaEthicsCommittee|FMSeriesTitle|FMSeriesText|FMArticleType|FMDOILine|FMRunningHeadRecto|FMRunningHeadVerso|FMArticleTitle|FMArticleSubtitle|FMArticleTranslatedTitle|FMArticleTranslatedSubtitle|FMReviewObject|FMRelatedArticle|FMCiteThisArticle|FMAbstractHead|FMAbstractSectionHeadDisplayed|FMAbstractParaFlushLeft|FMAbstractParaInd|FMTranslatedAbstract|FMNoteAuthorBio|ParaFlushLeft|ParaInd|HeadA|HeadB|HeadC|HeadD|HeadE|HeadF|HeadG|BL1|BL2|BL3|BL4|BL5|OL1|OL2|OL3|OL4|OL5|ListPara|UL1|UL2|UL3|ExtractHead|Extract|ExtractTextInd|ExtractSpaceAboveSecondExtract|ExtractList|ExtractSubList|ExtractTranslation|ExtractSource|Epigraph|EpigraphTextInd|EpigraphSource|PoetryExtract|PoetryExtractSpaceAboveStanzaBreak|PoetryExtractSource|DialogueExtract|DialogueExtractSource|Epilogue|EpilogueTextInd|EpilogueSource|Footnote|EMAcknowledgmentsHead|EMAcknowledgmentsText|EMAppendixNumber|EMAppendixTitle|EMNotesTitle|EMGlossaryTitle|EMGlossaryEntry|EMOtherSectionTitle|EMReferencesHead|RefAnnotationPara|BoxNumber|BoxTitle|BoxSubtitle|BoxParaFlushLeft|BoxParaInd|BoxHeadA|BoxHeadB|BoxHeadC|BoxExtract|BoxExtractTextInd|BoxExtractSource|BoxBL1|BoxOL1|BoxUL1|BoxBL2|BoxOL2|BoxUL2|BoxBL|BoxOL|BoxUL|BoxBL|BoxOL|BoxUL|BoxNote|BoxSource|TableNumber|TableColumnHead|TableStubEntry|TableStubSubentry|TableBody|TableNote|TableSource|TableHeadA|TableHeadB|TableHeadC|FigureNumber|FigureNote|FigureSource|EMSupplementaryMaterialTitle|SupplementaryMaterialNumber|SupplementaryMaterialCaption|SupplementaryMaterialNote|SupplementaryMaterialSource|VideoNumber|VideoCaption|VideoNote|VideoSource|ChemicalStructureDisplay|FormalStatement|FormalStatement|ResponseArticleSeparator|SubarticleSeparator|NoteToComp|EMNotes|EMNotesText|EMHeadA|EMHeadB|EMText|EMTextInd|FMNotesText|EMNotesText|FMTransAbstractParaInd|FMTransAbstractParaFlushLeft|FMTransAbstractHead|FMTransAbstractSectionHeadDisplayed|FMVideoAbstractParaFlushLeft|FMVideoAbstractParaInd|FMVideoAbstractHead|WhiteSpaceSectionBreak|ExtractBL1|ExtractBL2|ExtractBL3|ExtractNL1|ExtractNL2|ExtractNL3|ExtractOL1|ExtractOL2|ExtractOL3|FMNoteContributions|FMNoteConflict|FMNotePreviousAddress|FMNoteDeceased|FMNoteFunding|FMNoteConferenceHistory|FMNotePresent|FMNotePresentAddressDetails|FMNoteLevel|FMCustomMetaLogo|FMNoteHandlingEditor|FMNoteDisclaimer|FMNoteAttributeHead|FMNoteContributors|FMNoteEditorBio|FMNoteURL|FMNoteArticleCitation|FMNoteClinicalTrailInfo|FMNoteEthicalApprovalStatement|FMNoteOnline|FMNoteCodeDataRequest|FMMSHistoryDetails|FMNoteConsent|FMNoteSubmitted|FMNoteMiscellaneous|FMNoteIRBInfo|FMNoteDiscussant|FMNoteMemberID|FMNoteTweet|FMNotePatentDetails|FMNoteSummary|FMNoteOrcid|FMNotePublished|FMNoteWebsite|FMNoteRoyalty|FMNoteCopyrightStatement|FMNoteProductInfo)>}gsi)
{
	my $Paragraph_No=$1;
	my $Style_Name=$2;
	my $Para_Content=$3;
	...
	$Para_Content=~s{<(italic|bold|sup|sub|smallcaps|fnxref|query|bibxref|UIXref|bibxrefonlie|fnxrefonline|figxref|modxref|videoxref|genxref|tabxref|OnlineBibXref|boxxref|tabfnxref|tabfnxrefonline|formalstatement|label|comment|url|SupplementaryMaterialXref|citationref|fmgrantSponsor|fmgrantNumber|query|eqnxref|headArunIn|headBrunIn|headCrunIn|headDrunIn|headErunIn|headFrunIn|underline|boxnumberrunIn|fmdoi|fmaffEmai|secxref|enxref|appxref|affxref|affxrefonline|pi)>}{}gsi;

	$Para_Content=~s{<\/(italic|bold|sup|sub|smallcaps|fnxref|query|bibxref|UIXref|bibxrefonlie|fnxrefonline|figxref|modxref|videoxref|tabxref|OnlineBibXref|boxxref|tabfnxref|tabfnxrefonline|formalstatement|label|comment|url|SupplementaryMaterialXref|citationref|fmgrantSponsor|fmgrantNumber|query|eqnxref|headArunIn|headBrunIn|headCrunIn|headDrunIn|headErunIn|headFrunIn|underline|boxnumberrunIn|fmdoi|fmaffEmai|secxref|enxref|appxref|affxref|affxrefonline|pi)>}{}gsi;

	$Para_Content=~s{<(italic|bold|sup|sub|smallcaps|fnxref|query|bibxref|UIXref|bibxrefonlie|fnxrefonline|figxref|modxref|videoxref|tabxref|OnlineBibXref|boxxref|formalstatement|label|comment|url|tabfnxref|tabfnxrefonline|SupplementaryMaterialXref|citationref|fmgrantSponsor|fmgrantNumber|query|eqnxref|headArunIn|headBrunIn|headCrunIn|headDrunIn|headErunIn|headFrunIn|underline|boxnumberrunIn|fmdoi|fmaffEmai|secxref|enxref|appxref|affxref|affxrefonline|pi)\/>}{}gsi;
...
while($Style_Tmp=~m{<(\d+)><(FMSectionTitle|FMSubsectionTitle|FMSectionEditor|FMSubjectCodes|FMCustomMetaLogo|FMCustomMetaPatientConsent|FMCustomMetaEthicsCommittee|FMSeriesTitle|FMSeriesText|FMArticleType|FMDOILine|FMRunningHeadRecto|FMRunningHeadVerso|FMArticleTitle|FMArticleSubtitle|FMArticleTranslatedTitle|FMArticleTranslatedSubtitle|FMReviewObject|FMRelatedArticle|FMCiteThisArticle|FMAuthors|FMAffiliations|FMAbstractHead|FMAbstractSectionHeadDisplayed|FMAbstractParaFlushLeft|FMAbstractParaInd|FMTranslatedAbstract|Keywords|Abbreviations|FMNoteAuthorBio|FMNoteCorrespondence|ParaFlushLeft|ParaInd|HeadA|HeadB|HeadC|HeadD|HeadE|HeadF|HeadG|BL1|BL2|BL3|BL4|BL5|OL1|OL2|OL3|OL4|OL5|ListPara|UL1|UL2|UL3|ExtractHead|Extract|ExtractTextInd|ExtractSpaceAboveSecondExtract|ExtractList|ExtractSubList|ExtractTranslation|ExtractSource|Epigraph|EpigraphTextInd|EpigraphSource|PoetryExtract|PoetryExtractSpaceAboveStanzaBreak|PoetryExtractSource|DialogueExtract|DialogueExtractSource|Epilogue|EpilogueTextInd|EpilogueSource|Footnote|EMAcknowledgmentsHead|EMAcknowledgmentsText|EMAppendixNumber|EMAppendixTitle|EMNotesTitle|EMGlossaryTitle|EMGlossaryEntry|EMOtherSectionTitle|EMReferencesHead|Reference|RefAnnotationPara|BoxNumber|BoxTitle|BoxSubtitle|BoxParaFlushLeft|BoxParaInd|BoxHeadA|BoxHeadB|BoxHeadC|BoxExtract|BoxExtractTextInd|BoxExtractSource|BoxBL1|BoxOL1|BoxUL1|BoxBL2|BoxOL2|BoxUL2|BoxBL|BoxOL|BoxUL|BoxBL|BoxOL|BoxUL|BoxNote|BoxSource|TableNumber|TableCaption|TableNote|TableSource|TableHeadA|TableHeadB|TableHeadC|FigureCaption|ModelCaption|ParaCreditLine|FigureNote|FigureSource|InlineGraphic|EMSupplementaryMaterialTitle|SupplementaryMaterialNumber|SupplementaryMaterialCaption|SupplementaryMaterialNote|SupplementaryMaterialSource|VideoNumber|VideoCaption|VideoNote|VideoSource|EquationDisplay|ChemicalStructureDisplay|FormalStatement|FormalStatement|ResponseArticleSeparator|SubarticleSeparator|NoteToComp|p|EMNotes|EMNotesText|EMHeadA|EMText|EMTextInd|FMNotesText|EMNotesText|FMTransAbstractParaInd|FMTransAbstractParaFlushLeft|FMTransAbstractHead|FMTransAbstractSectionHeadDisplayed|FMGrapAbstractHead|FMGrapAbstractParaFlushLeft|FMGrapAbstractParaInd|FMVideoAbstractParaFlushLeft|FMVideoAbstractParaInd|FMVideoAbstractHead|KeywordsTrans)\/>}gsi)
{
	my $Paragraph_No=$1;
	my $Style=$2;
...
while($Review_Object_Tmp=~m{<(\d+)><FMReviewObject>(.*?)<\/FMReviewObject>}gsi)
{
	my $Paragraph_No=$1;
	my $Reference=$2;
...
while($Reference_Tmp=~m{<(\d+)><Reference>(.*?)<\/Reference>}gsi)
{
	my $Paragraph_No=$1;
	my $Reference=$2;
...
#push(@Error,"\~Para No:$Paragraph_No Col No:1\'Check the unwanted symbol found before reference\†Reference\'");
	...
	$Reference1=~s{(and|pp|In|Available|accepted|eds|ed|p|submitted|unpublised|personal)}{}gsi;
...
while($FM_Authors_Tmp=~m{<FMAuthors>(.*?)<para(\w+)>(.*?)<(\d+)><FMAuthors>}gsi)
{
my $Paragraph_No=$4;
...
while($HeadA_Tmp=~m{<(\d+)><HeadA>(.*?)<\/HeadA>}gsi)
{
	my $Paragraph_No=$1;
	my $HeadB=$2;
	my $Head=$2;
...
while($HeadB_Tmp=~m{<(\d+)><HeadB>(.*?)<\/HeadB>}gsi)
{
	my $Paragraph_No=$1;
	my $HeadB=$2;
	my $Head=$2;
...
while($HeadC_Tmp=~m{<(\d+)><HeadC>(.*?)<\/HeadC>}gsi)
{
	my $Paragraph_No=$1;
	my $HeadC=$2;
	my $Head=$2;
...
push(@Error,"\~Para No:$Paragraph_No Col No:1\'Character style headCrunIn is not allowed in \†HeadC\'");
	...
	{
		$Err="1";
		#push(@Error,"\~Para No:$Paragraph_No Col No:1\'Label style is missing in \†HeadC\'");
	}
...
while($HeadD_Tmp=~m{<(\d+)><HeadD>(.*?)<\/HeadD>}gsi)
{
	my $Paragraph_No=$1;
	my $HeadD=$2;
	my $Head=$2;
...
while($HeadE_Tmp=~m{<(\d+)><HeadE>(.*?)<\/HeadE>}gsi)
{
	my $Paragraph_No=$1;
	my $HeadE=$2;
	my $Head=$2;
...
while($Head_Para_Tmp=~m{<(\d+)><(ParaInd|ParaFlushLeft)>(.*?)<\/(ParaInd|Para_FlushLeft)>}gsi)
{
	my $Paragraph_No=$1;
	my $Style=$2;
	my $Para=$3;
...
while($HeadD_Tmp=~m{<(\d+)><EquationDisplay>(.*?)<\/EquationDisplay>}gsi)
{
	my $Paragraph_No=$1;
	my $Equation=$2;
...
while($Equ_Tmp=~m{<(\d+)><EquationDisplay>(.*?)<\/EquationDisplay>}gsi)
{
	my $Paragraph_No=$1;
	my $Equation=$2;
...
while($List_Tmp=~m{<(\d+)><OL(\d+)>(.*?)<\/OL(\d+)>}gsi)
{
...
while($List_Tmp1=~m{<(\d+)><UL(\d+)>(.*?)<\/UL(\d+)>}gsi)
{
...
print "List_Start".$List_Tmp2."\n";

while($List_Tmp2=~m{<(\d+)><BoxUL>(.*?)<\/BoxUL>}gsi)
{
...
while($List_Tmp3=~m{<(\d+)><BoxOL>(.*?)<\/BoxOL>}gsi)
{
...
if($Ack_Tmp=~m{<(\d+)><EMAcknowledgmentsHead>}gsi)
{
	my $Paragraph_No=$1;
...
while($Comment_Reference_Tmp=~m{<(\d+)><([^\>]+)\>(.*?)<\/\2>}gsi)
{
	my $Paragraph_No=$1;
...
if($End_Journal=~m{(HS9)}gsi)
{
	...
	while($Correspond=~m{<(\d+)><FMNoteCorrespondence>(.*?)<\/FMNoteCorrespondence>}gsi)
	{
	...
	while($Correspond=~m{<(\d+)><Tablecaption>(.*?)<\/Tablecaption>}gsi)
	{
...
if($End_Journal2=~m{(ALN)}gsi)
{
	...
	if($Conflict_Tmp=~m{<(\d+)><([^\>]+)\>Editor\'s Perspective}gsi)
	{
		my $Style=$2;
...
while($Clinical_Tmp1=~m{<(\d+)\><FMNoteClinicalTrailInfo>(.*?)<\/FMNoteClinicalTrailInfo>}gsi)
{
...
if($Reference_Tmp=~m{<(\d+)><EMReferencesHead>}gsi)
{
	my $Paragraph_No=$1;
...
if($Reference_Tmp=~m{<(\d+)><EMReferencesHead>(.*?)<\/EMReferencesHead>}gsi)
{
	my $Paragraph_No=$1;
...
if($Reference_Tmp2=~m{<(\d+)><Reference>}gsi)
{
	my $Paragraph_No=$1;
...
while($Tab_Tmp=~m{<table frame=\"(\w+)\" orient=\"port\"\/>(.*?)<table\/>}gsi)
{
	my $Table=$2;
	my $Table1=$2;
	my $Table2=$2;
	...
	$Table=~s{<row>}{"<".$Row_Count++."><row>"}gesi;
...
push(@Error,"\~Table No:$Table_Count Row No:$Rowno Col No:1 Both TableColumn head style and table Body style is present in same row");
	...
	while($Table=~m{<(\d+)><entry (\w+)=\"([^\"]+)\"([^\>]+)\>(.*?)<\/entry>}gsi)
	{
...
$Err="1";
push(@Error,"\~Table No:$Table_Count Row No:1 Col No:1 TableColumn head style or table Body style is missing");
		}

		if($Entry1=~m{<table }si)
		{
			
$Err="1";
push(@Error,"\~Table No:$Table_Count Row No:1 Col No:1 Table found inside the table");
...
while($Tab_Tmp1=~m{<(\d+)><(TableBody|TableColumnHead)>(.*?)<\/\2>}gsi)
{
	$Err="1";
	my $Paragraph_No=$1;
...
push(@Error,"\~Para No:$Paragraph_No Col No:$cnum\'et al should be in italic format'");
	...
	while($Ref1=~m{<xref ref\-type=\"bibr\" rid=\"([^\"]+)\">}gsi)
	{
		...
		foreach my $Value(@Ref_ID)
		{
			...
			{
				if($Reference_Tmp1=~m{<(\d+)><Reference>(.*?)<\/Reference>}gsi)
				{
					my $Paragraph_No=$1;
...
while($Ref_Tmp=~m{<(\d+)><ref id=\"(\d+)\"><citation ([^\>]+)\><label>(\s?)(\d+)}gsi)
{
	my $Paragraph_No=$1;
...
my $FM_Box_Title2=$Tmp;
		...
		if($FM_Box_Title=~m{<(\d+)><Box([^\>]+)\>(TAKEAWAY)}gs)
		{
			my $Paragraph_No=$1;
			my $Style=$2;
		...
		if($FM_Box_Title1=~m{<(\d+)><Box([^\>]+)\>(Question|Findings|Meaning)(.*?)<\/Box$2>}gsi)
		{
			my $Paragraph_No=$1;
			$Err="1";
		...
		if($FM_Box_Title2=~m{<(\d+)><Box([^\>]+)\>(Takeaway)}gsi)
		{
			my $Paragraph_No=$1;
			my $Box=$2;
			if($Box!~m{Title}gsi)
...
{
	my $Abs_Tmp=$Tmp;
	while($Abs_Tmp=~m{<(\d+)><(FigureSource|TableSource)>(.*?)<\/\2>}gsi)
	{
	my $Paragraph_No=$1;
	...
	my $Abs_Tmp1=$Tmp;
	while($Abs_Tmp1=~m{<(\d+)><(FigureNote|TableNote)>(.*?)<\/\2>}gsi)
	{
	my $Paragraph_No=$1;
...
if($Abstract_Jou=~m{(AMJHSP)}gsi)
{
	...
	{
if($Authors_Tmp=~m{<(\d+)><FMAuthors>(.*?)<\/FMAuthors>}gsi)
{
	my $Paragraph_No=$1;
...
if($Abstract_Jou2=~m{(JCEMET)}gsi)
{
	...
	if($Correspondence_Tmp=~m{<(\d+)><FMNoteCorrespondence><bold>}gsi)
	{	
...
if($Abss_Jou=~m{(ENDREV)}gsi)
{
	...
	{
		if($Subject_Tmp33=~m{<(\d+)><FMGrapAbstract}gsi)
		{
...
if($Abss_Jou12=~m{(CLEANE)}gsi)
{
	...
	while($Reference_Tmp22=~m{<(\d+)><Reference>(.*?)<\/Reference>}gsi)
	{
...
if($Abss_Jou1=~m{(ENDREV|JCEMET|ENDOCR|JESOCI)}gsi)
{
	...
	if($Subject_Tmp2=~m{<(\d+)><FMAbstractHead>(.*?)<\/FMAbstractHead>}gsi)
	{
	my $Paragraph_No=$1;
	...
	while($Subject_Tmp4=~m{<(\d+)><(HeadA|HeadB|HeadC|HeadD)>(.*?)<\/\2>}gs)
	{
	my $Paragraph_No=$1;
	...
	while($Subject_Tmp3=~m{<(\d+)><EMReferencesHead>(.*?)<\/EMReferencesHead>}gsi)
	{
	my $Paragraph_No=$1;
	...
	while($Subject_Tmp414=~m{<(\d+)><FigureNote>(.*?)<\/\2>}gs)
	{
	my $Paragraph_No=$1;
...
if($Abss_Jou66=~m{(GERONT)}gsi)
{
	...
	{
		my $Subject_Tmp33=$Tmp;
		if($Subject_Tmp33=~m{<(\d+)><(HeadA|HeadB|HeadC|HeadD|HeadE|HeadF)>(Conflicts of Interest|Conflict of Interest|Competing Interest|Competing Interests|Conflict of interest statement|Conflicts of interest statement|Potential conflict of interest|Funding|Financial support)<\/\2>}gsi)
		{
...
{
	my $Abs_Tmp=$Tmp;
	while($Abs_Tmp=~m{<(\d+)><FMAbstractParaFlushLeft>(.*?)<\/FMAbstractParaFlushLeft>}gsi)
	{
	my $Paragraph_No=$1;
	...
	my $Abs_Tmp1=$Tmp;
	while($Abs_Tmp=~m{<(\d+)><([^\>]+)\>Competing interest(s?)}gsi)
	{
	my $Paragraph_No=$1;
	...
	my $Abs_Tmp2=$Tmp;
	while($Abs_Tmp2=~m{<(\d+)><([^\>]+)\>Competing interest([\.\:\;\s]+)}gsi)
	{
	my $Paragraph_No=$1;
	...
	my $Abs_Tmp3=$Tmp;
	while($Abs_Tmp3=~m{<(\d+)><([^\>]+)\>Conflict of interest(s?)([\.\:\;\s]+)}gsi)
	{
	my $Paragraph_No=$1;
...
{
	my $Abs_Tmp=$Tmp;
	while($Abs_Tmp=~m{<(\d+)><FigureCaption>(.*?)<\/FigureCaption>}gsi)
	{
	my $Paragraph_No=$1;
	...
	my $Abs_Tmp1=$Tmp;
	while($Abs_Tmp1=~m{<(\d+)><TableCaption>(.*?)<\/TableCaption>}gsi)
	{
	my $Paragraph_No=$1;
...
while($Fig_Xref_Tmp3=~m{<(\d+)><fig id=\"(\w+)\"}gsi)
{
	my $Paragraph_No=$1;
...
while($Tab_Xref_Tmp3=~m{<(\d+)><table\-wrap id=\"(\w+)\"}gsi)
{
	my $Paragraph_No=$1;
...
my $Level_Tmp=$Tmp;	
	...
	if($Level_Tmp=~m{<(\d+)><([^\>]+)\>Level of Evidence}gsi)
	{
		my $Style=$2;
...
if($Suppl_Tmp=~m{<(\d+)><SupplementaryMaterialCaption>(.*?)<\/SupplementaryMaterialCaption>}gsi)
{
	my $Paragraph_No=$1;
...
if($FM_Article_Type!~m{<(\d+)><FMArticleType>(Body Contouring|Breast Surgery|Cosmetic Medicine|Facial Surgery|Genital Aesthetic Surgery|Oculoplastic Surgery|Research|Rhinoplasty|SoMe and Behavioral Science|My Way)<\/FMArticleType>}gsi)
{
...
while($Suppl_Tmp=~m{<(\d+)><HeadB>(.*?)<\/HeadB>}gsi)
{
	my $Paragraph_No=$1;
...
$FM_Article_Type=~s{<query>(.*?)<\/query>}{}gsi;
	...
	if($FM_Article_Type!~m{<(\d+)><FMArticleType>(Cosmetic Medicine|Breast Surgery|Research|Body Contouring|Facial Surgery|Rhinoplasty|Oculoplastic Surgery|Genital Aesthetic Surgery|SoMe and Behavioral Science|Commentaries|Editorials)<\/FMArticleType>}gsi)
{
...
while($Ab_Tmp1=~m{<(\d+)><(FMAbstractHead|FMLayAbstractHead)>(.*?)<\/\2>\n<(\d+)><([^\>]+)\>(.*?)<\/\5>}gsi)
{
	my $Paragraph_No=$4;
...
{
	$Figure="<xref ref-type=\"table\" rid=\"T$Fig_Id\">$Fig_Id$Fig_Let<\/xref>";
}
return $Figure;
...
{
	$Figure="<xref ref-type=\"table\" rid=\"T$Fig_Id\">$Fig_Id<italic>$Fig_Let<\/italic><\/xref>";
}
return $Figure;
