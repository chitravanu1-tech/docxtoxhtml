...
<w:comments xmlns:wpc="http://schemas.microsoft.com/office/word/2010/wordprocessingCanvas"
   ...
   <!--<xsl:apply-templates select="//img[@data-comment|@data-commentimg][not(ancestor::*[@data-change='commentdeleted'])]"/>-->
   <xsl:apply-templates select="descendant::CommentReference[@cmtIdVal = 'true']| descendant::commentreference[@cmtIdVal = 'true']"/>
  </w:comments>
 </xsl:template>

 <xsl:template match="annotation[string-length(@title) &gt; 0]">
   ...
   <xsl:element name="w:p">
    <xsl:element name="w:r">
     <xsl:element name="w:t">
      <xsl:value-of select="substring-after(@title, ':')[1]"/>
     </xsl:element>
    </xsl:element>
   </xsl:element>
 ...
 <xsl:template match="//comment[(ancestor::*[@data-change = 'commentdeleted'])]"/>


 <!-- this template match is only for one time purpose-->
 <xsl:template match="CommentReference[@cmtIdVal = 'true']| commentreference[@cmtIdVal = 'true']| commentreference[not(@cmtIdVal)][@data-change='commentadded']">
 <xsl:variable name="cmt" select="@data-commentvalue"/>
  ...
  <xsl:element name="w:comment">
   ...
   <xsl:attribute name="w:id">
    <xsl:value-of select="substring-after($cmt,'cmt')"/>
   </xsl:attribute>
   <xsl:attribute name="w:author" select="$author"/>
   <!--<xsl:attribute name="w:date" select="@ModificationDate"/>-->
   <xsl:call-template name="infopath-rich-text"/>
...
<!--<xsl:template match="//comment[not(ancestor::*[@data-change ='commentdeleted'])]">
  <xsl:variable name="author">
   <xsl:if test="child::label">
    <xsl:value-of select="child::label"/>
   </xsl:if>
  </xsl:variable>
  <xsl:if test="ancestor::commentreference[1][(@data-commentvalue = //img/@data-commentimg[1]) or (@data-commentvalue = //img/@data-comment[1])]">
   <xsl:element name="w:comment">
    <xsl:attribute name="w:id">
     <!-\-<xsl:number level="any" count="comment[not(ancestor::*[@data-change ='commentdeleted'])]"/>-\->
     <xsl:value-of select="replace(ancestor::commentreference[1]/@data-commentvalue,'cmt','')"/>
    </xsl:attribute>
    <xsl:attribute name="w:author" select="$author"/>
    <!-\-<xsl:attribute name="w:date" select="@ModificationDate"/>-\->
    <xsl:call-template name="infopath-rich-text"/>
 ...
 <!--<xsl:template match="CommentReference|label|span[@class ='CommentReference']"/>-->

 <xsl:template match="label | span[@class = 'CommentReference'] |span[@class = 'commentreference']"/>
 ...
 <!-- <xsl:param name="pPr_Default"/>
  ...
  <xsl:choose>
   <xsl:when
    test="descendant::h1 | descendant::h2 | descendant::h3 | descendant::h4 | descendant::h5 | descendant::h6 | descendant::p | descendant::li | descendant::comment | descendant::Comment">  
 ...
 <!-- XHTML div -->
 <!-- remove comment and Comment element because from now on paragraphs will be given as paragraphs instread of comment element 18.2.2019 by hari-->
 <!--<xsl:template match="h1|h2|h3|h4|h5|h6|p|li|comment|Comment">-->
 <xsl:template match="h1 | h2 | h3 | h4 | h5 | h6 | p | li|comment[not(child::p)]|Comment[not(child::p)]">
  <xsl:param name="pPr_Default"/>
  <xsl:param name="rPr_Default"/>
 ...
 <xsl:template match="span[contains(@style, 'padding-left:')]">
  <xsl:variable name="tabSize" as="xs:string">
   <xsl:variable name="val"
    select="substring-before(substring-after(replace(lower-case(@style), ' ', ''), 'padding-left:'), 'px')"
    as="xs:string"/>
   <xsl:value-of select="string(round((number($val) div 1.33)) * 20)"/>
  </xsl:variable>
  <w:r>
   <w:tab w:val="left" w:pos="{$tabSize}"/>
  </w:r>
 ...
 <!-- XHTML hyperlink -->
 <xsl:template
  match="a[@href and not(contains(@class, 'footnotecitation')) and not(ancestor-or-self::*[(lower-case(@class) = 'endnotereference') or (lower-case(@class) = 'EndnoteNumber')])][not(@data-alias | @data-boxname | @data-boxtag | @data-id | @data-tag)]">
  <xsl:param name="pPr_Default"/>
  <xsl:param name="rPr_Default"/>
 ...
 <xsl:template name="apply-nested-character-formatting">
    ...
    <xsl:when test="ancestor-or-self::*[contains(@style, 'font-family:')]">
     ...
     <xsl:variable name="fontAttrName">
      <xsl:variable name="tempAttrName">
       ...
       <!--<xsl:message select="replace($tempAttrNameVal, 'mso-', '')"></xsl:message>-->
      </xsl:variable>
      <xsl:choose>
  ...
  <xsl:choose>
   <xsl:when test="ancestor-or-self::*[contains(@style, 'text-decoration: line-through double')][1]">
    <w:dstrike/>
   </xsl:when>
  </xsl:choose>
     ...
     <xsl:when
      ...
      <xsl:variable name="font">
       <xsl:choose>
        ...
        <xsl:otherwise>
         <xsl:if test="ancestor-or-self::*[contains(@style, 'color:#')][1]">
          <xsl:value-of
  ...
  <xsl:if
   test="ancestor-or-self::*[contains(lower-case(@style), 'background-color:') or contains(lower-case(@style), 'background:')]">
   <xsl:variable name="styleProp"
    select="replace(ancestor-or-self::*[contains(lower-case(@style), 'background-color:') or contains(lower-case(@style), 'background:')][1]/lower-case(@style), ' ', '')"/>
   <xsl:variable name="bgC">
    <xsl:choose>
     <xsl:when
      test="contains(lower-case($styleProp), 'background-color:#') or contains(lower-case($styleProp), 'background:#')">
      <xsl:choose>
     ...
     <xsl:otherwise>
      ...
      <xsl:call-template name="color">
       <xsl:with-param name="colorVal" select="$tempVal"/>
      </xsl:call-template>
     </xsl:otherwise>
    </xsl:choose>
   </xsl:variable>
   ...
   <xsl:variable name="bgC">
    <xsl:choose>
     ...
     <xsl:otherwise>
      ...
      <xsl:call-template name="color">
       <xsl:with-param name="colorVal" select="replace($tempVal, ' ', '')"/>
      </xsl:call-template>
     </xsl:otherwise>
    </xsl:choose>
   </xsl:variable>
