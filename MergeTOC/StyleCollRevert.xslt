...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
      ...
      <!--04-03-2025 commented becasue remove space inside tag example <sub> </sub> book name of SCHLAGER Book TOC Testing and also removed indent div and p tag -->
<!--      <xsl:strip-space elements="*"/>      -->
      ...
      <xsl:template match="//p | h1 | h2 | h3 | h4 | h5 | h6 | blockquote | figcaption | //div[@data-type ='textbox']//p | //span | //ol | //ul | //li">
            ...
            <xsl:variable name="Final-style">                  
                        <xsl:choose>
                              <xsl:when test="contains(@Client-Style,'$ID/[')">
                                    <xsl:value-of select="substring-before(substring-after(@Client-Style,'$ID/['),']')"/>
                              </xsl:when>
                              <xsl:when test="contains(@Client-Style,'$ID/')">
                                    <xsl:value-of select="substring-after(@Client-Style,'$ID/')"/>
                              </xsl:when>
                              <xsl:when test="contains(@Client-Style,'CharacterStyle/')">
                                    <xsl:value-of select="substring-after(@Client-Style,'CharacterStyle/')"/>
                              </xsl:when>
                              <xsl:otherwise>
                  ...
                  <xsl:apply-templates select="@*"/>
                      <xsl:if test="self::*[@class]">
                       <xsl:attribute name="class">
                           <xsl:choose>
                                 <xsl:when test="self::*[@class][not(@Client-Style)]">                                       
                                       <xsl:value-of select="$curName"/>
                                 </xsl:when>                                 
                                 <xsl:when test="self::*[@class][@Client-Style]">                                       
                                       <xsl:value-of select="$Final-style"/>
                                 </xsl:when>
      ...
      <!-- below commented for chapter contain FigureLegend and TableCaption with chapter title then can't retaine chapter title to TOC  -->
   <!--   <xsl:template match="div[child::p[@class = 'TOC_FigureLegend']]"/>
      <xsl:template match="div[child::p[@class = 'TOC_TableCaption']]"/>-->
      
      <xsl:template match="p[@class = 'TOC_FigureLegend']"/>
      <xsl:template match="p[@class = 'TOC_TableCaption']"/>
