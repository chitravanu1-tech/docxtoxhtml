...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    exclude-result-prefixes="xs html" version="2.0">
<!--26-12-2023 commented for unwanted entity comes with TOC.xhtml file    -->
    ...
    <!--    <xsl:template match="html">
        <xsl:text>&#x000A;</xsl:text>
        <xsl:text disable-output-escaping="yes">&lt;html xmlns="http://www.w3.org/1999/xhtml" xmlns:epub="http://www.idpf.org/2007/ops" lang="en" xml:lang="en"&gt;</xsl:text>
        <xsl:copy-of select="@*"/>
        <xsl:apply-templates/>
        <xsl:text>&#x000A;</xsl:text>
    ...
    <xsl:template match="div[@data-id = following::div[@data-id]/@data-id][not(descendant::p)]">
        <xsl:apply-templates/>
    </xsl:template>

    <!--<xsl:template match="div[not(@data-id)][not(child::p)]"/>-->
    
    <!--23-01-2023 commented for fm titles not comes because data-id same with chapter data-id -->    
<!--    <xsl:template match="//div[not(@data-fm)][@data-id = following::div[not(@data-fm)][@data-id]/@data-id]"/>-->

    <xsl:template match="//div[@data-id = following::div[@data-id]/@data-id and @data-fm = following::div[@data-fm]/@data-fm]"/>


    <xsl:template match="div[@data-id and @data-fm][not(descendant::p)]"/>

    <xsl:template match="br[@data-ckbr = 'true']"/>
    <!-- Added to remove empty on 25-1-2022 -->
    <xsl:template match="div[@data-id][not(descendant::p)]"/>
