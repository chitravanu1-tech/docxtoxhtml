...
<p class="FrontMatterHeading" data-name="FrontMatterHeading">Contents</p>
<xsl:apply-templates/>
<!--</xsl:element>-->
</xsl:element>
...
<xsl:apply-templates select="@* | node()"/>
   ...
   <!-- <xsl:template match="p[@class = 'PartTitle']">       
            <xsl:element name="p">             
                    <xsl:apply-templates select="@*"/>
                <xsl:value-of select="preceding-sibling::*[1][self::p[@class = 'PartNumber']]/substring-before(.,'&lt;span&gt;position-')"/>
                    <xsl:value-of select="': '"/>
                    <xsl:apply-templates select="node()"/>
            </xsl:element>
    </xsl:template>
    <xsl:template match="p[@class = 'UnitTitle']">       
        <xsl:element name="p">             
            <xsl:apply-templates select="@*"/>
            <xsl:value-of select="preceding-sibling::*[1][self::p[@class = 'UnitNumber']]/substring-before(.,'&lt;span&gt;position-')"/>
            <xsl:value-of select="': '"/>
            <xsl:apply-templates select="node()"/>
        </xsl:element>
    </xsl:template>
    <xsl:template match="p[@class = 'SectionTitle']">       
        <xsl:element name="p">             
            <xsl:apply-templates select="@*"/>
            <xsl:value-of select="preceding-sibling::*[1][self::p[@class = 'SectionNumber']]/substring-before(.,'&lt;span&gt;position-')"/>
            <xsl:value-of select="': '"/>
            <xsl:apply-templates select="node()"/>
        </xsl:element>
    </xsl:template>
    <xsl:template match="p[@class = 'ChapterTitle']">       
        <xsl:element name="p">             
            <xsl:apply-templates select="@*"/>
            <xsl:value-of select="preceding-sibling::*[1][self::p[@class = 'ChapterNumber']]/substring-before(.,'&lt;span&gt;position-')"/>
            <xsl:value-of select="': '"/>
            <xsl:apply-templates select="node()"/>
        </xsl:element>
    </xsl:template>
<xsl:template match="root">
       <xsl:copy>
           <xsl:apply-templates select="div">
               <xsl:sort select="@data-id" order="ascending" data-type="number"/>
           </xsl:apply-templates>
       </xsl:copy>
</xsl:template>-->
        
    <xsl:template match="p[lower-case(text())='contents']"/>
    <xsl:template match="p[lower-case(text())='table of contents']"/>
    ...
    <xsl:template match="span[not(@class/string-length() &gt; 0 or @style/string-length() &gt; 0 or @data-bkmark)][not(@data-tab = 'true')]"/>
