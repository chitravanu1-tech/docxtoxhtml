...
<!--    <xsl:variable name="Graphic" select="document('FinalStyleCollGroup.xml')"/>-->
    ...
    <xsl:template match="/">
        <xsl:if test="descendant::body[child::p[@class = 'TOC_FigureLegend']][string-length(.) &gt; 0]">                       
            <xsl:result-document href="{'Figures'}.xhtml">
            <xsl:element name="html">
                <xsl:element name="head">
                    <xsl:text>&#x000A;</xsl:text>
                    <xsl:element name="title">
                        <xsl:value-of select="'List Of Figures'"/>
                    </xsl:element>
                    <xsl:text>&#x000A;</xsl:text>
                    ...
                    <xsl:apply-templates select="descendant::style"/>                           
                </xsl:element>
                <xsl:element name="body">
<!--                    <xsl:element name="div">-->
                        <xsl:variable name="getFrontMatterHeadingClientStyle" select="descendant::body/child::p[@class = 'FrontMatterHeading']/@Client-Style"/>
                        <xsl:element name="p"><xsl:attribute name="class"><xsl:value-of select="$getFrontMatterHeadingClientStyle"/></xsl:attribute><xsl:attribute name="data-name"><xsl:value-of select="$getFrontMatterHeadingClientStyle"/></xsl:attribute>List of Figures</xsl:element>
<!--                        <xsl:element name="div">-->
                    <xsl:for-each select="//p[@class = 'TOC_FigureLegend']">
                   
<!--                     <xsl:element name="div">-->
                       <!--  <xsl:attribute name="data-id">
                             <xsl:value-of select="parent::div/@data-id"/>
                         </xsl:attribute>-->
                        <xsl:copy>
                          <xsl:apply-templates select="@*"/>
                            <xsl:attribute name="class">
                                <xsl:value-of select="self::p/@Client-Style"/>
                            </xsl:attribute>
                          ...
                          <xsl:if test="self::*[matches(.,'pageNo-')]">
                                ...
                                <xsl:element name="span">
                                    ...
                                    <xsl:value-of select="substring-after(.,'pageNo-')"/>
                                </xsl:element>  
                          </xsl:if>
                        </xsl:copy>
                     <!--</xsl:element>-->
    ...
    <xsl:template match="@Client-Style"/>
    <xsl:template match="div"/>
    <xsl:template match="html | meta | p">
        <xsl:text>&#x000A;</xsl:text>
        <xsl:element name="{local-name()}">
            <xsl:apply-templates select="@* | node()"/>
        </xsl:element>
    </xsl:template>
    <xsl:template match="style | head | div">
        <xsl:text>&#x000A;</xsl:text>
        <xsl:element name="{local-name()}">
            <xsl:apply-templates select="@* | node()"/>
        </xsl:element>
    ...
    <!--    11-10-2023 below span template commented for need style attribute with uppercase-->
    <!--    <xsl:template match="span[not(@class/string-length() &gt; 0)][not(@data-tab = 'true')]"/>-->
    
    <!--26-12-2023 commented for unwanted entity comes with TOC.xhtml file    -->
