...
<xsl:stylesheet
  ...
  <!-- added to retain double tab as individual on 7-2-2022 -->
  ...
  <xsl:template name="collapse-ilk">
    ...
    <xsl:for-each-group select="$among" group-adjacent="xsw:node-signature(.)">
        <!-- Added for math merge @id='mathjax' on 23-07-2025-->
      <xsl:choose>
          <xsl:when test="exists(current-group()/self::span[@data-bkmark or @data-tab='true' or @class='InlineMath' or @id='mathjax'])">
