<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="//style" xpath-default-namespace="http://www.machineedit.net/AMOD/CustomXML/MultiParaCC.htm"/>
    <xsl:template match="body" xpath-default-namespace="http://www.machineedit.net/AMOD/CustomXML/MultiParaCC.htm">
            <xsl:element name="root">
                <!--<xsl:for-each select="//span[@data-index='yes']" xpath-default-namespace="http://www.machineedit.net/AMOD/CustomXML/MultiParaCC.htm">
                    <xsl:text>&#x000A;</xsl:text>
                    <xsl:element name="chapter">
                        <xsl:text>&#x000A;</xsl:text>
                        <xsl:element name="levelCount"><xsl:value-of select="position()"/></xsl:element>
                        <xsl:text>&#x000A;</xsl:text>
                        <xsl:element name="bookmark"><xsl:value-of select="ancestor::p[1]/descendant::span[starts-with(@data-bkmark,'para')][1]/@data-bkmark"/></xsl:element>
                        <xsl:text>&#x000A;</xsl:text>
                        <xsl:element name="rdfLevel"><xsl:value-of select="position()"/></xsl:element>
                        <xsl:text>&#x000A;</xsl:text>
                        <xsl:element name="letterNode">
                            <xsl:value-of select="substring(normalize-space(substring-after(@data-instrText,'xe')),1,1)"/>
                        </xsl:element>
                        <xsl:text>&#x000A;</xsl:text>
                        <xsl:element name="IndexMarkingText">
                            <xsl:choose>
                                <xsl:when test="preceding::span[string-length(.) &gt; 0][1]">
                                    <xsl:value-of select="preceding::span[string-length(.) &gt; 0][1]/."/>
                                </xsl:when>
                                <xsl:otherwise>
                        ...
                        <xsl:text>&#x000A;</xsl:text>
                        <xsl:element name="Indextem"><xsl:value-of select="normalize-space(substring-after(@data-instrText,'xe'))"/></xsl:element>
                        <xsl:text>&#x000A;</xsl:text>
                        <xsl:element name="level"><xsl:value-of select="'main'"/></xsl:element>
                        <xsl:text>&#x000A;</xsl:text>
                        <xsl:element name="subject"><xsl:value-of select="$FileEntryId"/><xsl:value-of select="ancestor::p[1]/descendant::span[starts-with(@data-bkmark,'para')][1]/@data-bkmark"/></xsl:element>
                        <xsl:text>&#x000A;</xsl:text>
                        <xsl:element name="chapterName"><xsl:value-of select="$filename"/></xsl:element>
                        <xsl:text>&#x000A;</xsl:text>
                        <xsl:element name="title"><xsl:value-of select="''"/></xsl:element>
                        <xsl:text>&#x000A;</xsl:text>
                        <xsl:element name="uuid"><xsl:value-of select="''"/></xsl:element>
                        <xsl:text>&#x000A;</xsl:text>
                        <xsl:element name="fileEntryId"><xsl:value-of select="$FileEntryId"/></xsl:element>
                        <xsl:text>&#x000A;</xsl:text>
                        <xsl:element name="key"><xsl:value-of select="''"/></xsl:element>
                    </xsl:element>
                </xsl:for-each>-->
                <xsl:for-each select="//span[@data-index]">
                    <xsl:variable name="bkmark"><xsl:value-of select="ancestor::p[1]/descendant::span[starts-with(@data-bkmark,'para')][1]/@data-bkmark"/></xsl:variable>
                    <xsl:variable name="IndexMarkingText">
                        <xsl:choose>
                                        <xsl:when test="preceding::span[string-length(.) &gt; 0][1]">
                                            <xsl:value-of select="preceding::span[string-length(.) &gt; 0][1]/."/>
                                        </xsl:when>
                    ...
                    <xsl:variable name="num">
                        <xsl:value-of select="count(preceding::span[@data-index]) + 1"/>
                    </xsl:variable>
                    <xsl:variable name="index">
                        <xsl:value-of select="normalize-space(@data-instrText)"/>
                    </xsl:variable>
                    <xsl:variable name="aaaa">
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:choose>
                                    <xsl:when test="contains(lower-case($index),'XE')">
                    ...
                    <xsl:variable name="ccc"
                        select="replace($bbb, '(&#x005C;&#x005C;)([T][o][p][i][c][n])', '-')"/>
                    
                    <xsl:for-each select="tokenize($ccc,':')">
                        ...
                        <xsl:if test="position()=1">
                            <xsl:element name="chapter">
                                <xsl:text>&#x000A;</xsl:text>
                                <xsl:element name="levelCount"><xsl:value-of select="position()+ 1"/></xsl:element>
                                <xsl:text>&#x000A;</xsl:text>
                                <xsl:element name="bookmark"><xsl:value-of select="$bkmark"/></xsl:element>
                                <xsl:text>&#x000A;</xsl:text>
                                <xsl:element name="rdfLevel"><xsl:value-of select="position()"/></xsl:element>
                                <xsl:text>&#x000A;</xsl:text>
                                <xsl:element name="letterNode">
                                ...
                                <xsl:text>&#x000A;</xsl:text>
                                <xsl:element name="EditedText">
                                    <xsl:value-of select="$IndexMarkingText"/>                                   
                                </xsl:element>
                                <xsl:text>&#x000A;</xsl:text>
                                <xsl:element name="level"><xsl:value-of select="'Main'"/></xsl:element>
                                <xsl:text>&#x000A;</xsl:text>
                                <xsl:element name="subject"><xsl:value-of select="$FileEntryId"/><xsl:value-of select="$bkmark"/></xsl:element>
                                <xsl:text>&#x000A;</xsl:text>
                                <xsl:element name="chapterName"><xsl:value-of select="$filename"/></xsl:element>
                                <xsl:text>&#x000A;</xsl:text>
                                <xsl:element name="title"><xsl:value-of select="."/></xsl:element>
                        ...
                        <xsl:if test="position()>1">
                            <xsl:element name="chapter">
                                ...
                                <xsl:text>&#x000A;</xsl:text>
                                <xsl:element name="level"><xsl:value-of select="'Sub'"/><xsl:value-of select="position()"/></xsl:element>
                                <xsl:text>&#x000A;</xsl:text>
                                <xsl:element name="subject"><xsl:value-of select="$FileEntryId"/><xsl:value-of select="$bkmark"/></xsl:element>
                                <xsl:text>&#x000A;</xsl:text>
                                <xsl:element name="chapterName"><xsl:value-of select="$filename"/></xsl:element>
                        ...
                        <!--<xsl:element name="span">
                            <xsl:attribute name="data-instrText"><xsl:value-of select="."/></xsl:attribute>
                            <xsl:attribute name="class">
                                <xsl:choose>
                                    ...
                                    <xsl:otherwise>
                                        <xsl:value-of select="concat('Sub',$position)"/>  
                                    </xsl:otherwise>
                                </xsl:choose>
                            </xsl:attribute>
                            ...
                            <xsl:attribute name="data-id1">
                                <xsl:value-of select="$indexpos"/>
                            </xsl:attribute>
                            <xsl:attribute name="data-id">
                                <xsl:value-of select="normalize-space(upper-case(substring(.,1,5)))"/>
                                <xsl:value-of select="$num"/>
                            </xsl:attribute>
                            
                            <xsl:if test="position()=1">
                                <xsl:attribute name="data-tag">
                                    <xsl:value-of select="concat('Main',$num)"/>
                                </xsl:attribute>
                            </xsl:if>
                            <xsl:if test="position()>1">
    ...
    <!--<xsl:template match="//span[@data-index]" xpath-default-namespace="http://www.machineedit.net/AMOD/CustomXML/MultiParaCC.htm">
        <xsl:variable name="num">
            <xsl:value-of select="count(preceding::span[@data-index]) + 1"/>
        </xsl:variable>
        <xsl:variable name="index">
