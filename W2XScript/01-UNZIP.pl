...
if ($zip->read($inputFile) == AZ_OK) {
  ...
  foreach my $member (@members) {
  	  	if($member =~ m/(word\/(comments|endnotes|fontTable|footer1|footer2|footnotes|header1|header2|numbering|settings|styles|webSettings)\.xml|docProps\/app\.xml|docProps\/core\.xml)/i) {
