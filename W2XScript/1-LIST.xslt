<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
      ...
      xmlns:aid="urn:schemas-microsoft-com:office:office"
      xmlns:dt="uuid:C2F41010-65B3-11d1-A29F-00AA00C14882"
      xmlns:mf="http://example.com/mf"
      xmlns:xs="http://www.w3.org/2001/XMLSchema" 
      xmlns:st1="urn:schemas-microsoft-com:office:smarttags" version="2.0"
      xmlns:String="java:java.lang.String" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">


<!-- 27-Nov-17: To remove DIV tags for references and move its ID attr. and a-link inside LI or P tag -->
...
<!-- 20-AUG-18:  To remove DIV tags for references and move its ID attr. and a-link inside LI or P tag other than S4C styles for REFERENCE-->
    ...
    <!--21/02/2020 commented by prakash for intend issue in editor and added href and id on "anchor Tag"-->
      ...
      <!-- 02-JULY-19 -->
      <!--For Range 26-8-2019-->
      <!--Added for Normal paragraphstyle "ReferenceHeading" 13-06-2020-->
      <!--Added to remove space in data-alias 12-06-2020-->
      <!--Added for Boom [] open close 29-06-2020-->
      <!--Added for marriappan <xsl:copy-of select="ancestor::div[@id]/@id"/> for ref id missing 07-09-2020-->
    <!--Added to remove the valign super (FDA) duplicate issue on 09-06-2021-->
    <!-- Added to avaoid the space between span and a in endnote citation on 15-3-2022 -->
    <!-- Added if endnotecaption styled with endnotetext(w:endnoteref followed by w:t) on 29-03-2021 -->
    <!-- Added([not(descendant::span[@class='EndnoteReference'])]) to manage the manual and auto numbers in endnote on 06-09-2022 -->
      <xsl:output method="xml" omit-xml-declaration="no" indent="no" cdata-section-elements="alt"/>
    <xsl:variable name="StylesCollect" select="document('StylesCollection.xml')"/>
      ...
      <xsl:template match="ol">
            ...
            <xsl:variable name="pretype" select="preceding-sibling::ol[1]/@type"/>
            <xsl:variable name="nexttype" select="following-sibling::ol[1]/@type"/>
                        
            <xsl:choose>
                  <xsl:when test="preceding-sibling::*[1][self::ol] and $type = $pretype">
                        <xsl:apply-templates/>
                  </xsl:when>
                  <xsl:otherwise>
                        ...
                        <xsl:if test="@indent">
                              <xsl:text disable-output-escaping="yes"> indent="</xsl:text>
                        ...
                        <xsl:if test="@data-ilvl">
                              <xsl:text disable-output-escaping="yes"> data-ilvl="</xsl:text>
                              <xsl:value-of select="@data-ilvl"/>  
                              <xsl:text disable-output-escaping="yes">"</xsl:text>
                        </xsl:if>
                        <xsl:if test="@data-numFmt">
                              <xsl:text disable-output-escaping="yes"> data-numFmt="</xsl:text>
                              <xsl:value-of select="@data-numFmt"/>  
                              <xsl:text disable-output-escaping="yes">"</xsl:text>
                        </xsl:if>
                        <xsl:if test="@data-lvlText">
                              <xsl:text disable-output-escaping="yes"> data-lvlText="</xsl:text>
            ...
            <xsl:choose>
                  <xsl:when test="following-sibling::*[1][self::ol] and $type = $nexttype">                        
                        <!--<xsl:apply-templates/>-->
                  </xsl:when>
                  <xsl:otherwise>
      ...
      <xsl:template match="ul">
            ...
            <xsl:variable name="pretype" select="preceding-sibling::ul[1]/@style"/>
            <xsl:variable name="nexttype" select="following-sibling::ul[1]/@style"/>
            
            <xsl:choose>
                  <xsl:when test="preceding-sibling::*[1][self::ul] and $type = $pretype">
                        <xsl:apply-templates/>
                  </xsl:when>
                  <xsl:otherwise>
                        
                        <xsl:text disable-output-escaping="yes">&lt;ul class="</xsl:text>     
                        <xsl:value-of select="$className"/>                              
                        <xsl:text disable-output-escaping="yes">"</xsl:text>
            ...
            <xsl:choose>
                  <xsl:when test="following-sibling::*[1][self::ul] and $type = $nexttype">                        
                        <!--<xsl:apply-templates/>-->
                  </xsl:when>
                  <xsl:otherwise>
      ...
      <xsl:template match="one">            
            <xsl:if test="not(preceding-sibling::*[1][self::one])">
            ...
            <xsl:if test="not(following-sibling::*[1][self::one])">
      ...
      <xsl:template match="two">            
            <xsl:if test="not(preceding-sibling::*[1][self::two])">
                  <xsl:text disable-output-escaping="yes">&lt;GROUP&gt;</xsl:text>
            ...
            <xsl:if test="not(following-sibling::*[1][self::two])">
                  
                  <xsl:text disable-output-escaping="yes">&lt;/GROUP&gt;</xsl:text>
            </xsl:if>
      </xsl:template>
      <xsl:template match="three">            
            <xsl:if test="not(preceding-sibling::*[1][self::three])">
                  <xsl:text disable-output-escaping="yes">&lt;GROUP&gt;</xsl:text>
            ...
            <xsl:if test="not(following-sibling::*[1][self::three])">
                  
                  <xsl:text disable-output-escaping="yes">&lt;/GROUP&gt;</xsl:text>
            </xsl:if>
      </xsl:template>
      <xsl:template match="four">            
            <xsl:if test="not(preceding-sibling::*[1][self::four])">
                  <xsl:text disable-output-escaping="yes">&lt;GROUP&gt;</xsl:text>
            ...
            <xsl:if test="not(following-sibling::*[1][self::four])">
                  
                  <xsl:text disable-output-escaping="yes">&lt;/GROUP&gt;</xsl:text>
            </xsl:if>
      </xsl:template>
      <xsl:template match="five">            
            <xsl:if test="not(preceding-sibling::*[1][self::five])">
                  <xsl:text disable-output-escaping="yes">&lt;GROUP&gt;</xsl:text>
            ...
            <xsl:if test="not(following-sibling::*[1][self::five])">
                  
                  <xsl:text disable-output-escaping="yes">&lt;/GROUP&gt;</xsl:text>
            </xsl:if>
      </xsl:template>
      <xsl:template match="six">            
            <xsl:if test="not(preceding-sibling::*[1][self::six])">
                  <xsl:text disable-output-escaping="yes">&lt;GROUP&gt;</xsl:text>
            ...
            <xsl:if test="not(following-sibling::*[1][self::six])">
                  
                  <xsl:text disable-output-escaping="yes">&lt;/GROUP&gt;</xsl:text>
            </xsl:if>
      </xsl:template>
      <xsl:template match="seven">            
            <xsl:if test="not(preceding-sibling::*[1][self::seven])">
                  <xsl:text disable-output-escaping="yes">&lt;GROUP&gt;</xsl:text>
            ...
            <xsl:if test="not(following-sibling::*[1][self::seven])">
                  
                  <xsl:text disable-output-escaping="yes">&lt;/GROUP&gt;</xsl:text>
            </xsl:if>
      </xsl:template>
      <xsl:template match="eight">            
            <xsl:if test="not(preceding-sibling::*[1][self::eight])">
                  <xsl:text disable-output-escaping="yes">&lt;GROUP&gt;</xsl:text>
            ...
            <xsl:if test="not(following-sibling::*[1][self::eight])">
                  
                  <xsl:text disable-output-escaping="yes">&lt;/GROUP&gt;</xsl:text>
            </xsl:if>
      </xsl:template>
      <xsl:template match="nine">            
            <xsl:if test="not(preceding-sibling::*[1][self::nine])">
                  <xsl:text disable-output-escaping="yes">&lt;GROUP&gt;</xsl:text>
            ...
            <xsl:if test="not(following-sibling::*[1][self::nine])">
                  
                  <xsl:text disable-output-escaping="yes">&lt;/GROUP&gt;</xsl:text>
            </xsl:if>
      </xsl:template>
      <xsl:template match="ten">            
            <xsl:if test="not(preceding-sibling::*[1][self::ten])">
                  <xsl:text disable-output-escaping="yes">&lt;GROUP&gt;</xsl:text>
            ...
            <xsl:if test="not(following-sibling::*[1][self::ten])">
...
<!-- 27-Nov-17: To remove DIV tags for references and move its ID attr. and a-link inside LI or P tag -->
      <!--<xsl:template match="li">
            <xsl:choose>
                  <xsl:when test="contains(@class, 'Reference')">
                        <xsl:element name="li">
                              <xsl:copy-of select="@*"/>
      ...
      <xsl:template match="li">
            <xsl:choose>
                  <xsl:when test="ancestor::div[@class = 'captionreference']">                        
                        <xsl:element name="li">
                              <xsl:copy-of select="@*"/>
                              <xsl:copy-of select="ancestor::div[1]/@id"/>
                              <xsl:copy-of select="ancestor::a/@data-alias"/>
      ...
      <xsl:template match="p">
            <xsl:choose>
                  <!--Added for Normal paragraphstyle "ReferenceHeading" 13-06-2020-->
                  <!--Added for marriappan <xsl:copy-of select="ancestor::div[@id]/@id"/> for ref id missing 07-09-2020-->
                  <xsl:when test="self::p[contains(@class, 'Reference')][parent::div]">
                       <xsl:element name="p">
                              <xsl:copy-of select="@*"/>
                              <xsl:copy-of select="ancestor::div[@id]/@id"/>
                              <xsl:copy-of select="ancestor::a/@data-alias"/>
                              <xsl:copy-of select="ancestor::a/@href"/>
                              <xsl:element name="a">
                                  <!--21/02/2020 commented by prakash for intend issue in editor-->
                  ...
                  <!-- 20-AUG-18:  To remove DIV tags for references and move its ID attr. and a-link inside LI or P tag other than S4C styles for REFERENCE-->
                  <xsl:when test="ancestor-or-self::div[@class = 'captionreference']">
                        <xsl:element name="p">
                              <xsl:copy-of select="@*"/>
                              <xsl:copy-of select="ancestor::div[@id]/@id"/>
                              <xsl:copy-of select="ancestor::a/@data-alias"/>
                  ...
                  <!-- 20-AUG-18:  To remove DIV tags for references and move its ID attr. and a-link inside LI or P tag other than S4C styles for REFERENCE ends-->
                  <xsl:otherwise>
                        <xsl:copy>
                              <xsl:apply-templates select="@*|node()"/>
                        </xsl:copy>
...
<!-- 27-Nov-17: To remove DIV tags for references and move its ID attr. and a-link inside LI or P tag -->
      ...
      <!--<xsl:template match="a[@data-alias = 'Reference']|a[@data-alias = 'NL']">
            <xsl:apply-templates></xsl:apply-templates>
      </xsl:template>
      <xsl:template match="div[child::a[@data-alias = 'Reference']]|div[child::a[@data-alias = 'NL']]">
            <xsl:apply-templates></xsl:apply-templates>
      </xsl:template>-->
      <xsl:template match="a[ancestor::div[@class = 'captionreference']]">
            <xsl:apply-templates></xsl:apply-templates>
      </xsl:template>
      <xsl:template match="div[child::a[@class = 'action_link']]|div[child::a[@data-alias = 'Reference']]|div[child::a[@data-alias = 'NL']]">
            <xsl:apply-templates/>
      </xsl:template>
      
      
<!-- 07-Feb-18: The following when is only for "LUSTER" client -->
      <xsl:template match="TableCell01">
            <xsl:if test="not(preceding-sibling::*[self::TableCell01])">
                  <xsl:text disable-output-escaping="yes">&lt;tr data-newone="Yes"&gt;</xsl:text>
            </xsl:if>
            <xsl:if test="preceding-sibling::*[self::TableCell01]">
            ...
            <xsl:if test="not(following-sibling::*[self::TableCell02] or following-sibling::*[self::span])">
                  <xsl:text disable-output-escaping="yes">&lt;/tr&gt;</xsl:text>
            </xsl:if>
            <!--<xsl:if test="not(following-sibling::*[self::TableCell01])">
                  <xsl:text disable-output-escaping="yes">&lt;/trX&gt;</xsl:text>
            </xsl:if>-->
      </xsl:template>
      <xsl:template match="tbodyX">
            <xsl:element name="tbodyX">
      ...
      <xsl:template match="TableCell02">
            <xsl:element name="td">            
                  <xsl:apply-templates select="@*|node()"/>            
            </xsl:element>
            <xsl:if test="not(following-sibling::*[self::TableCell01]) or not(following-sibling::*[self::TableCell02]) and not(following-sibling::*[self::span])">
                  <xsl:text disable-output-escaping="yes">&lt;/tr&gt;</xsl:text>
            </xsl:if>
      </xsl:template>
      <xsl:template match="TableCell03|TableCell04|TableCell05|TableCell06|TableCell07|TableCell08|TableCell09|TableCell10">
            <xsl:element name="td">            
                  <xsl:apply-templates select="@*|node()"/>            
            </xsl:element>
      </xsl:template>
      ...
      <!--Added for Boom [] open close 29-06-2020-->
      <xsl:template match="span[@class='FootnoteReference'][contains(normalize-space(.),']') or contains(normalize-space(.),')') or contains(normalize-space(.),'}')][preceding-sibling::span[1][@class='EndnoteReference'][normalize-space(.)='[' or normalize-space(.)='(' or normalize-space(.)='{']]">
            <xsl:copy>
                  <xsl:apply-templates select="@*"/>
                  <xsl:if test="self::span[@class='FootnoteReference'][contains(normalize-space(.),']') or contains(normalize-space(.),')') or contains(normalize-space(.),'}')]/preceding-sibling::span[1][@class='EndnoteReference'][normalize-space(.)='[' or normalize-space(.)='(' or normalize-space(.)='{']">
                        <xsl:copy-of select="preceding-sibling::span[1][@class='EndnoteReference'][normalize-space(.)='[' or normalize-space(.)='(' or normalize-space(.)='{']"></xsl:copy-of>
                  </xsl:if>
      ...
      <xsl:template match="span[@class='EndnoteReference'][normalize-space(.)='[' or normalize-space(.)='(' or normalize-space(.)='{'][following-sibling::span[1][@class='FootnoteReference'][contains(normalize-space(.),']') or contains(normalize-space(.),')') or contains(normalize-space(.),'}')]]"/>
  <!--End-->
      
      <!-- 02-JULY-19 -->
  <xsl:template match="spanX">
        <xsl:choose>
              <xsl:when test="ancestor::p/preceding-sibling::ol[@data-numFmt='decimal']">
                    <xsl:value-of select="ancestor::p/preceding-sibling::ol[@data-numFmt='decimal'][1]/@start"></xsl:value-of>
                    <xsl:text>.</xsl:text>
                    <xsl:number count="spanX" from="ol[@data-numFmt='decimal']" level="any"></xsl:number>
              </xsl:when>
              <xsl:otherwise>
    ...
    <!--For BIB  Range 26-8-2019-->
      
      <xsl:template match="span[contains(@data-alias,'to') or contains(@data-alias,'To') or contains(@data-alias,'Through') or contains(@data-alias,'through') or contains(@data-alias,'-') or contains(@data-alias,'—') or contains(@data-alias,'–')]/child::a[contains(@data-alias,'BibRef')][1]">
      
            <xsl:copy>
                  <xsl:apply-templates select="@*|node()"/>
            </xsl:copy>
            <xsl:variable name="RVal1" select="self::a[contains(@data-alias,'BibRef')]/parent::span[contains(@data-alias,'to') or contains(@data-alias,'To') or contains(@data-alias,'Through') or contains(@data-alias,'through') or contains(@data-alias,'-') or contains(@data-alias,'—') or contains(@data-alias,'–')]/@data-alias"/>
            <!--Added to remove space in data-alias 12-06-2020-->
            <xsl:variable name="RVal" select="translate($RVal1,' ','')"/>
            <xsl:if test="self::a[contains(@data-alias,'BibRef')][parent::span[contains(@data-alias,'to') or contains(@data-alias,'To') or contains(@data-alias,'Through') or contains(@data-alias,'through') or contains(@data-alias,'-') or contains(@data-alias,'—') or contains(@data-alias,'–')]]">
                  
                   <xsl:variable name="href" select="self::a/substring-before(@href,'.xhtml')"/>
                  <xsl:analyze-string select="$RVal" regex="([\d]+)([-]?[—]?[–]?[to]?[To]?[Through]?[through]?)([\d]+)">
                        
                        
                        <!-- <xsl:analyze-string select="$RVal" regex="([\d]+)([ to ]?[ To ]?[ Through ]?[ through ]?[-]?[—]?[–]?[to]?[To]?[Through]?[through]?)([\d]+)">-->
                          <xsl:matching-substring>
                                
                              <xsl:variable name="fileRange1" select="xs:integer(regex-group(1)) +1"/>
                              
                              <xsl:variable name="fileRange2" select="xs:integer(regex-group(3)) - 1"/>
                              <xsl:for-each select="$fileRange1 to $fileRange2">
                                    <xsl:element name="a">
                                          <xsl:attribute name="href">
                                                <xsl:value-of select="concat($href,'.xhtml#Reference')"/>                  
                                                <xsl:value-of select="."/>            
                                          </xsl:attribute>
                                          
                                          <xsl:attribute name="data-tag">
                                                <xsl:value-of select="'CiteTag'"/>
                                          </xsl:attribute>
...
<xsl:output-character character=" " string="&amp;#x2029;"/>
      ...
      <xsl:template match="spanDigit">
            <xsl:element name="span">
                  <xsl:value-of select="parent::p/preceding::ol[@data-lvlText = '%1.'][1]/@start"/>
                  <!--<xsl:for-each select="parent::p/preceding::ol[@data-lvlText = '%1.'][1]">
                        <xsl:value-of select="@start"/>
                  </xsl:for-each>-->
                  <!--<xsl:value-of select="parent::p/preceding::ol[1][not(@start = '1')]/@start"/>-->
                  <xsl:text>.</xsl:text>
                  <xsl:number count="spanDigit" from="ol[@data-lvlText = '%1.']" level="any"/>
            </xsl:element>
      </xsl:template>
      <!--Delete Normal 18-7-2019-->
    <!--Added to remove the valign super (FDA) duplicate issue on 09-06-2021--> 
    <xsl:template match="span[contains(@style,'vertical-align: super;')][child::sup]/@style">
        <xsl:attribute name="style">
            <xsl:value-of select="replace(.,'vertical-align: super;','')"/>
        </xsl:attribute>
    </xsl:template>
    <xsl:template match="span[contains(@style,'vertical-align: sub;')][child::sub]/@style">
        <xsl:attribute name="style">
            <xsl:value-of select="replace(.,'vertical-align: sub;','')"/>
        </xsl:attribute>
    </xsl:template>
    <xsl:template match="figcaption[child::p]">
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:copy-of select="child::p/@class"/>
            <xsl:copy-of select="child::p/@data-name"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
    ...
    <!-- Added for endnote caption sequence on 8-3-2022 -->
    <!--<xsl:template match="a[@class='crossref']">
        <xsl:variable name="hval" select="substring-after(@href,'#')"/>
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:choose>
                <xsl:when test="preceding::a[@class='endnotecitation'][@id =$hval]">
                    <xsl:value-of select="preceding::a[@class='endnotecitation'][@id =$hval][string-length(.) &gt; 0]/."/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:apply-templates/>
                </xsl:otherwise>
    ...
    <!-- Added to avaoid the space between span and a in endnote citation on 15-3-2022 -->
    <!-- Added if endnotecaption styled with endnotetext(w:endnoteref followed by w:t) on 29-03-2021 -->
    <!-- Added to remove the endnote citation number with formating on 19-4-2022 -->
    <!-- Added([not(descendant::span[@class='EndnoteReference'])]) to manage the manual and auto numbers in endnote on 06-09-2022 -->
    <xsl:template match="span[@class='EndnoteReference'][not(descendant::span[@class='EndnoteReference'])]">
        <xsl:element name="span">
            <xsl:apply-templates select="@* | descendant::a"/>
        </xsl:element>
        <xsl:apply-templates select="node() except descendant::a"/>
    </xsl:template>
    <!-- Added to move the endnotenumber after bookmark on 20-9-2022   -->
    <xsl:template match="li[@style='mso-element:endnote'][descendant::p[1][descendant::span[@class='EndnoteReference' and @data-noendnoteref='true']]]/p/span[@class='EndnoteReference' and @data-noendnoteref='true'][following-sibling::*[1][self::span[@class='msoDel']]]"/>
    <!-- latex -->
    <xsl:variable name="eqstyle">
        <xsl:value-of select="$StylesCollect//stylesList//style[@name='DZC_EquationName']/@replacement"/>
    </xsl:variable>
    <!-- //span[contains(lower-case(@class),'equationdisplaynumber') or contains(lower-case(@class),'equationname')] commented on 05-4-2025-->
    <xsl:template match="//span[@data-name=$eqstyle]">
        <xsl:variable name="texname" select="."/>
        <xsl:variable name="textname1">
            <xsl:choose>
                <xsl:when test="contains($texname,'&lt;') or contains($texname,'&gt;')">
                    <xsl:value-of select="substring-before(substring-after($texname,'&lt;&lt;'),'&gt;&gt;')"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="$texname"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:variable name="eqName">
            <xsl:value-of select="concat($textname1,'.','tex')"/>            
        </xsl:variable>
        <xsl:variable name="bsl">
            <xsl:value-of select="concat($textname1,'.','bsl')"/>
        </xsl:variable>
        <xsl:variable name="mmlName">
            <xsl:value-of select="concat($textname1,'.','mml')"/>
        </xsl:variable>
        <xsl:variable name="mathName" select="document($eqName)//root//text()"/>
        <xsl:variable name="bslName" select="document($bsl)//root//text()"/>
        <xsl:variable name="mml" select="document($mmlName)"/>
        <xsl:choose>
            <xsl:when test="string-length(document($eqName))">
                <xsl:element name="span">
                    ...
                    <xsl:variable name="latexcode">
                        <xsl:apply-templates select="$mathName"/>
                    </xsl:variable>
                    <xsl:attribute name="latex">
                        
                        <xsl:value-of select="
                            replace(replace($latexcode,'\{\\eqbox\}', ''),'\\end\{lrbox\}', '')"/>
                    </xsl:attribute>
                    <xsl:variable name="bsl">
                        <xsl:apply-templates select="$bslName"/>
                    </xsl:variable>
                    <xsl:attribute name="depth">
                        <xsl:value-of select="substring-before(substring-after($bsl,'Depth = '),'&#xA;')"/>
                    </xsl:attribute>
                    <xsl:attribute name="height">
                        <xsl:value-of select="substring-before(substring-after($bsl,'Height = '),'&#xA;')"/>
                    </xsl:attribute>
                    <xsl:attribute name="totalheight">
                        <xsl:value-of select="substring-before(substring-after($bsl,'TotalHeight = '),'&#xA;')"/>
                    </xsl:attribute>
                    <xsl:attribute name="width">
                        <xsl:value-of select="substring-before(substring-after($bsl,'Width = '),'&#xA;')"/>
                    </xsl:attribute>
                    <xsl:attribute name="class" select="'math-content'"/>
                    <xsl:attribute name="data-filename" select="$eqName"/>
                    <xsl:apply-templates select="$mml"></xsl:apply-templates>
                </xsl:element>
            </xsl:when>
            <xsl:otherwise>
                <xsl:copy>
