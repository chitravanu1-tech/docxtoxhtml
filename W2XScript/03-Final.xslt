<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    version="2.0">
    <xsl:variable name="math" select="document('math.xml')"/>
    ...
    <!-- Added for mariappan changed bookmark to id on 13-12-2021 -->
    <!--Added for Discretionary line break multiple span removing on 8-3-2022-->
    <!-- Added website and url both  styles comes for link on 11-5-2022  -->
    <!-- footnote structure change on 9-6-2022 -->
    <!-- Added to retain the endnotes id's on 8-7-2022 -->
    <!-- Added for Editor to display the emptypara on 31-3-2023-->
    <!-- Added to split if the page br comes in same paragraph on 1-02-2-23-->
    <!-- Added to Remove the empty footnote and endnote on Azure 7535  -->
    <!-- span inside img not working changed child to descendant on 19-5-2023 -->
    <!-- Added on 13-6-2023  Azure 8963 and 8948-->
    <!-- Added [not(parent::aside[@id='endnote_' or  @data-alias='Endnotes'])] to avoid the missing Table in inside endnote on 12-12-2023 Azure 12123 -->
...
<!--Added for cic-Group 16-8-2019-->
    ...
    <xsl:template match="ul[not(parent::div)] | ol[not(parent::div)]">
        ...
        <!-- Added to avoid the text missing issue on 3-4-2026 -->
        <xsl:if test="descendant::div">
    ...
    <!-- Added [not(parent::aside[@id='endnote_' or  @data-alias='Endnotes'])] to avoid the missing Table in inside endnote on 12-12-2023 Azure 12123 -->
    <xsl:template match="ul[not(parent::aside[@id='endnote_' or  @data-alias='Endnotes'])]//div"/>
    <xsl:template match="ul[not(parent::aside[@id='endnote_' or  @data-alias='Endnotes'])]//table"/>
    <xsl:template match="div//ul[not(parent::aside[@id='endnote_' or  @data-alias='Endnotes'])]/table"/>
...
<!-- 15-June-17: added attribute "data-split" for the elements asked by Mariappan-->
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        ...
                        <xsl:element name="span">
                            <xsl:copy-of select="@*"/>
                            <xsl:attribute name="class">
                                <xsl:value-of select="'EndnoteNumber'"/>
                            </xsl:attribute>
                            <xsl:attribute name="data-name">
                                <xsl:value-of select="'EndnoteNumber'"/>
                            </xsl:attribute>
                        ...
                        <!-- Added single space for endnote number on 9-1-2024 -->
                        <!-- self::span[@class = 'EndnoteReference'][not(following::text()[1][normalize-space(.)='.'])] -->
                        <!-- self::span[@class = 'EndnoteReference'][not(ancestor::li[1]/p[matches(.,'^(\d)+\p{P}+\s+|^(\d)+\s+')])] -->
                        <xsl:if test="self::span[@class = 'EndnoteReference'][ancestor::li[1]/p[1][not(matches(.,'^(\d)+\p{P}+\s+|^(\d)+\s+'))]]">
                        <xsl:value-of select="' '"/>
                        </xsl:if>
                    </xsl:otherwise>
                </xsl:choose> 
            </xsl:when>
            <!-- added on 04-08-2021 for notes module -->
            <!-- Again commenter for Editor on 21-10-2021 -->
            <xsl:when test="ancestor::li[@style = 'mso-element:endnote']">
                <xsl:variable name="endnote-id" select="preceding-sibling::a/@style"/>
                <xsl:variable name="endnote-id2" select="substring-after($endnote-id,':')"/>
                <xsl:choose>
                    <xsl:when test="//EN">
    ...
    <xsl:template match="td">
        <xsl:element name="td">             
            ...
            <!-- Added for Ng xhtml to xml 27-5-2025 -->
            <xsl:copy-of select="@data-col"/>
            <!-- end -->
            <xsl:choose>
                <xsl:when test="@Themeformat = 'BI'">
                    <xsl:element name="b">
    ...
    <!-- 24-oct-18: Added No-B-IT property Sindhu -->
     ...
     <xsl:template match="//p">
         ...
         <!--commented on 18-8-2020 while resolve text missing issue-->
...
<xsl:output-character character="Ň" string="&amp;#x0147;"/>
<xsl:output-character character="ņ" string="&amp;#x0146;"/><xsl:output-character character="Ņ" string="&amp;#x0145;"/>
...
<xsl:output-character character=" " string="&amp;#x2029;"/>
    ...
    <!--End-->
    <!-- added to avoid the bullets in endnote list on 21-10-2021 -->
    <xsl:template match="aside[ul[li[@style='mso-element:endnote' or @style='mso-element:footnote']]]/ul">
            <xsl:copy>
            <xsl:apply-templates select="@*"/>
    ...
    <!-- added for editor on 13-12-2021 -->
    <xsl:template match="aside[not(ul[li[@style='mso-element:endnote' or @style='mso-element:footnote']])]/ul"/>
    <xsl:template match="aside[not(ul[li[@style='mso-element:endnote' or @style='mso-element:footnote']])]/@data-alias"/>
    <!-- Added for mariappan changed bookmark to id on 13-12-2021 -->
    <!--<xsl:template match="p[child::span[@data-bkmark]] | li[child::span[@data-bkmark]]">
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:attribute name="id" select="child::span[@data-bkmark][1]/@data-bkmark"/>
            <xsl:apply-templates select="node()"/>
        </xsl:copy>
    </xsl:template>-->
    ...
    <!--<xsl:template match="span[@data-bkmark]"/>-->
    <!--Added for Discretionary line break multiple span removing on 8-3-2022-->
    <xsl:template match="span[contains(@data-bkmark,'DB')]"/>
    <!--<xsl:template match="span[contains(@data-bkmark,'URL')][@data-urlstart='start']">
        <xsl:element name="span">
            <xsl:apply-templates select="@* except @data-urlstart"/>
            <xsl:apply-templates select="node()"/>
        </xsl:element>
        <!-\-<xsl:text disable-output-escaping="yes">&lt;span class="URL" data-name="URL"&gt;</xsl:text>-\->
        <xsl:text disable-output-escaping="yes">&lt;span class="</xsl:text>
        <xsl:value-of select="following::span[@class][1]/@class"/>
        <xsl:text disable-output-escaping="yes">" data-name="</xsl:text>
        <xsl:value-of select="following::span[@data-name][1]/@data-name"/>
        <xsl:text disable-output-escaping="yes">"&gt;</xsl:text>
    </xsl:template>
    <!-\- Added website and url both  styles comes for link on 11-5-2022  -\->
    <xsl:template match="span[@class][preceding-sibling::span[contains(@data-bkmark,'URL')][@data-urlstart='start'] and following-sibling::span[contains(@data-bkmark,'URL')][@data-urlstart='End']] | span[@class][parent::*/preceding-sibling::span[contains(@data-bkmark,'URL')][@data-urlstart='start'] and parent::*/following-sibling::span[contains(@data-bkmark,'URL')][@data-urlstart='End']]">
        <xsl:value-of select="."/>
    </xsl:template>
    <xsl:template match="span[contains(@data-bkmark,'URL')][@data-urlstart='End']">
        <xsl:text disable-output-escaping="yes">&lt;/span&gt;</xsl:text>
        <xsl:element name="span">
            <xsl:apply-templates select="@* except @data-urlstart"/>
            <xsl:apply-templates select="node()"/>
    ...
    <!--Added two styles applied for same url issue on on 12-5-2022-->
    <xsl:template match="span[@class and @data-urlstart='start']//child::span[@class]">
      <xsl:apply-templates/>
    </xsl:template>
    <xsl:template match="@data-urlstart"/>
    <!-- footnote structure change on 9-6-2022 -->
    <!--<xsl:template match="li[@class='footnotecaption']//p//span[@class='FootnoteReference' or @data-name='footnote reference']">
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:attribute name="class" select="'FootnoteNumber'"/>
            <xsl:attribute name="data-name" select="'FootnoteNumber'"/>
            <xsl:apply-templates select="node()"/>
        </xsl:copy>
    </xsl:template>-->
    ...
    <!-- Added to retain the endnotes id's on 8-7-2022 -->
    <xsl:template match="span[@class='EndnoteReference' or  @data-name='endnote reference' or @data-name='endnote_reference']/a[@class='endnotecitation' or @class='crossref']">
            <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:if test="@id">
            <xsl:attribute name="id">
                <xsl:value-of select="@id"/>
                <xsl:value-of select="concat('_',$FileEntryId)"/>
            </xsl:attribute>
            </xsl:if>
    ...
    <xsl:template match="aside/ul//li[@style='mso-element:endnote' or @class='endnotecaption']">
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:attribute name="data-split">
                <xsl:value-of select="generate-id()"/>
    ...
    <!-- Added for Editor to display the emptypara on 31-3-2023-->
    <!-- span inside img not working changed child to descendant on 19-5-2023 -->
    <xsl:template match="p[string-length(normalize-space(.))=0 or .='&#x9;' or .=' '][not(descendant::img)][not(descendant::Comment)]">
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
    ...
    <!-- end -->
    <!-- Added to split if the page br comes in same paragraph on 1-02-2-23-->
    <!--<xsl:template match="//br[lower-case(@data-type)='page'][following::text()//parent::p = ancestor::p[1] or following::text()//ancestor::*/parent::p = ancestor::p[1]][ancestor::p[1][string-length(.) &gt; 0]] | //br[lower-case(@data-type)='page'][following::text()][parent::p = ancestor::p[1]][not(parent::*)][ancestor::p[1][string-length(.) &gt; 0]]">
            <xsl:choose>
    ...
    <!--<xsl:template match="//br[lower-case(@data-type)='page'][following::text()//parent::p = ancestor::p[1]][ancestor::p[1][string-length(.) &gt; 0]]"/>-->
    <!-- Added to Remove the empty footnote and endnote on Azure 7535  -->
    <xsl:template match="div[@id='footnote_'][not(child::*)]"/>
    <xsl:template match="aside[@id='endnote_'][not(child::*)]"/>
    <xsl:template match="aside[@id='endnote_'][descendant::ul[not(child::*)]]"/>
    <!--<xsl:template match="//p">
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:if test="self::p[descendant::ins or descendant::span[@class='msoIns']] or self::p[descendant::del or descendant::span[@class='msoDel']] or self::p[parent::div[@data-type='textbox'] or self::p[contains(lower-case(@class),'box')][ancestor::div] or ancestor::div[@data-type='textbox']] or self::p[parent::li]">
                <xsl:attribute name="data-willclean">
                    <xsl:value-of select="'true'" />
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="self::p[parent::div[@data-alias]] or self::p[descendant::*[@data-alias]]">
                <xsl:attribute name="data-willclean">
                    <xsl:value-of select="'true'" />
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="self::p[descendant::span[@data-instrText or @data-fldChar or @data-fldData]]">
                <xsl:attribute name="data-willclean">
                    <xsl:value-of select="'true'" />
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="self::p[descendant::img][not(ancestor::figure)] or self::p[lower-case(@class)='header' or lower-case(@data-name)='header'] or self::p[lower-case(@class)='footer' or lower-case(@data-name)='footer']">
                <xsl:attribute name="data-willclean">
                    <xsl:value-of select="'true'" />
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="self::p[descendant::span[lower-case(@class)='hyperlink' or lower-case(@data-name)='hyperlink']]">
                <xsl:attribute name="data-willclean">
                    <xsl:value-of select="'true'" />
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="self::p[contains(.,'&amp;#xAD;')or contains(.,'&#xAD;')]">
                <xsl:attribute name="data-willclean">
                    <xsl:value-of select="'true'" />
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="self::p[descendant::span[contains(@style,'color:')]/@style[not(contains(.,'color: #000000'))]] or self::p[descendant::span[contains(@style,'color:')]/@style[not(contains(.,'color: #ffffff'))]] or self::p[descendant::span[contains(@style,'color:')]/@style[not(contains(.,'color:#000000'))]] or self::p[descendant::span[contains(@style,'color:')]/@style[not(contains(.,'color:#ffffff'))]]">
                <xsl:attribute name="data-willclean">
                    <xsl:value-of select="'true'" />
                </xsl:attribute>
            </xsl:if>
            ...
            <xsl:if test="self::p[descendant::br[lower-case(@data-type)='page']] or self::p[descendant::br[lower-case(@data-type)='column']] or self::p[descendant::br[not(lower-case(@data-type)='page')]]">
                <xsl:attribute name="data-willclean">
                    <xsl:value-of select="'true'" />
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="self::p[descendant::span[@class or @data-name]]">
                <xsl:attribute name="data-willclean">
                    <xsl:value-of select="'true'" />
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="self::p[descendant::span[@style][contains(@style,'color: #000000') or contains(@style,'color:#000000')]]">
                <xsl:attribute name="data-willclean">
                    <xsl:value-of select="'true'" />
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="self::p[descendant::span[@class='allcaps' or  @data-name='allcaps']]">
                <xsl:attribute name="data-willclean">
                    <xsl:value-of select="'true'" />
                </xsl:attribute>
            </xsl:if>
            <!-\- need to check the scenario -\->
            <xsl:if test="self::p[descendant::Hidden or descendant::rtl]">
                <xsl:attribute name="data-willclean">
                    <xsl:value-of select="'true'" />
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="self::p[descendant::span[lower-case(@class)='strong' or lower-case(@class)='emphasis' or  lower-case(@data-name)='emphasis'] or descendant::strong or descendant::emphasis]">
                <xsl:attribute name="data-willclean">
                    <xsl:value-of select="'true'" />
                </xsl:attribute>
            </xsl:if>
            <!-\- end -\->
            <xsl:if test="self::p[contains(.,'—')] or self::p[contains(.,'&amp;nbsp;')] or self::p[contains(.,'&amp;#160;')] or self::p[contains(.,'x2028')]">
                <xsl:attribute name="data-willclean">
                    <xsl:value-of select="'true'" />
                </xsl:attribute>
            </xsl:if>
                      
            <xsl:if test="self::p[descendant::span[@class='EndnoteReference' or @data-name='endnote reference']] or self::p[parent::li[@class='endnotecaption'] or descendant::span[@class='EndnoteNumber' or @data-name='EndnoteNumber']]">
                <xsl:attribute name="data-willclean">
                    <xsl:value-of select="'true'" />
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="self::p[descendant::span[@class='footnotecitation']] or self::p[parent::div[@class='footnotecaption'] or descendant::span[@class='MsoFootnoteReference']]">
                <xsl:attribute name="data-willclean">
                    <xsl:value-of select="'true'" />
                </xsl:attribute>
            </xsl:if>
            ...
            <xsl:if test="self::p[parent::td or ancestor::table]">
                <xsl:attribute name="data-willclean">
                    <xsl:value-of select="'true'" />
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="self::p[@class='emptypara']">
                <xsl:attribute name="data-willclean">
                    <xsl:value-of select="'true'" />
                </xsl:attribute>
            </xsl:if>
            <!-\-<xsl:if test="self::p[descendant::span[@data-bkmark]]">
                <xsl:attribute name="data-willclean">
                    <xsl:value-of select="'true'" />
                </xsl:attribute>
            </xsl:if>-\->
    ...
    <!-- commented to display the latex eqn to mathml on 12-6-2025 -->
    <!--<xsl:template match="//math">
        <xsl:variable name="id" select="self::math/@id"/>
        <xsl:copy-of select="self::math"></xsl:copy-of>
        <!-\-<span class="InlineMath">
        <xsl:copy-of select="$math//root//mml:math[@id = $id]"/>
        </span>-\->
        <span class="InlineMath">
            <xsl:copy-of select="$math//root//math[@id = $id]"/>
        </span>
    </xsl:template>-->
    <!-- comment moved inside the p, if not its create file not opend issue on 19-03-2025 -->
    <xsl:template match="p[preceding-sibling::*[1][self::span[@data-commentrange='cmtStart']]]">
   <xsl:element name="p">
       <xsl:apply-templates select="@*"/>
       <xsl:copy-of select="preceding-sibling::*[1][self::span[@data-commentrange='cmtStart']]"></xsl:copy-of>
       <xsl:apply-templates select="node()"/>
   </xsl:element>
    </xsl:template>
    <xsl:template match="span[@data-commentrange='cmtStart'][following-sibling::*[1][self::p]]"/>
    
    <!-- Added to  retain style formatting to tag on 10-7-2025-->
    <!--<xsl:template match="span[not(@class or @data-name or @data-commentrange or @data-comment or @data-commentimg or @data-commentgroup)][contains(@style,'vertical-align: super') or contains(@style,'vertical-align:super')][not(ancestor::sup)][string-length(.) &gt; 0]">
        <xsl:element name="span">
            <xsl:apply-templates select="@*"/>
            <xsl:element name="sup">
            <xsl:apply-templates select="node()"/>
    ...
    <xsl:template match="span[not(@class or @data-name or @data-commentrange or @data-comment or @data-commentimg or @data-commentgroup)][contains(@style,'vertical-align: sub') or contains(@style,'vertical-align:sub')][not(ancestor::sub)][string-length(.) &gt; 0]">
        <xsl:element name="span">
            <xsl:apply-templates select="@*"/>
    ...
    <xsl:template match="span[not(@class or @data-name or @data-commentrange or @data-comment or @data-commentimg or @data-commentgroup)][contains(@style,'text-decoration:underline') or contains(@style,'text-decoration: underline')][not(ancestor::u)][string-length(.) &gt; 0]">
        <xsl:element name="span">
            <xsl:apply-templates select="@*"/>
            <xsl:element name="u">
            <xsl:apply-templates select="node()"/>
    ...
    <xsl:template match="span[not(@class or @data-name or @data-commentrange or @data-comment or @data-commentimg or @data-commentgroup)][contains(@style,'font-weight:bold') or contains(@style,'font-weight: bold')][not(ancestor::b)][string-length(.) &gt; 0]">
        <xsl:element name="span">
            <xsl:apply-templates select="@*"/>
            <xsl:element name="b">
            <xsl:apply-templates select="node()"/>
    ...
    <xsl:template match="span[not(@class or @data-name or @data-commentrange or @data-comment or @data-commentimg or @data-commentgroup)][contains(@style,'font-style:italic') or contains(@style,'font-style: italic')][not(ancestor::i)][string-length(.) &gt; 0]">
            <xsl:element name="span">
                <xsl:apply-templates select="@*"/>
                <xsl:element name="i">
                <xsl:apply-templates select="node()"/>
    ...
    <xsl:template match="span[not(@class or @data-name or @data-commentrange or @data-comment or @data-commentimg or @data-commentgroup)][contains(@style,'font-variant: small-caps') or contains(@style,'font-variant:small-caps')][not(ancestor::small)][string-length(.) &gt; 0]">
        <xsl:element name="span">
            <xsl:apply-templates select="@*"/>
    ...
    <xsl:template match="span[not(@class or @data-name or @data-commentrange or @data-comment or @data-commentimg or @data-commentgroup)][@style][string-length(.) &gt; 0]">
        <xsl:element name="span">
            <xsl:apply-templates select="@*"/>
        <xsl:if test="self::span[not(@class or @data-name or @data-commentrange or @data-comment or @data-commentimg or @data-commentgroup)][contains(@style,'vertical-align: super') or contains(@style,'vertical-align:super')][not(ancestor::sup)][string-length(.) &gt; 0]">
        <xsl:text disable-output-escaping="yes">&lt;sup&gt;</xsl:text>
    </xsl:if>
    <xsl:if test="self::span[not(@class or @data-name or @data-commentrange or @data-comment or @data-commentimg or @data-commentgroup)][contains(@style,'vertical-align: sub') or contains(@style,'vertical-align:sub')][not(ancestor::sub)][string-length(.) &gt; 0]">
        <xsl:text disable-output-escaping="yes">&lt;sub&gt;</xsl:text>
    </xsl:if>
    <xsl:if test="self::span[not(@class or @data-name or @data-commentrange or @data-comment or @data-commentimg or @data-commentgroup)][contains(@style,'text-decoration:underline') or contains(@style,'text-decoration: underline')][not(ancestor::u)][string-length(.) &gt; 0]">
        <xsl:text disable-output-escaping="yes">&lt;u&gt;</xsl:text>
    </xsl:if>
    <xsl:if test="self::span[not(@class or @data-name or @data-commentrange or @data-comment or @data-commentimg or @data-commentgroup)][contains(@style,'font-weight:bold') or contains(@style,'font-weight: bold')][not(ancestor::b)][string-length(.) &gt; 0]">
        <xsl:text disable-output-escaping="yes">&lt;b&gt;</xsl:text>
    </xsl:if>
    <xsl:if test="self::span[not(@class or @data-name or @data-commentrange or @data-comment or @data-commentimg or @data-commentgroup)][contains(@style,'font-style:italic') or contains(@style,'font-style: italic')][not(ancestor::i)][string-length(.) &gt; 0]">
        <xsl:text disable-output-escaping="yes">&lt;i&gt;</xsl:text>
    </xsl:if>
    <xsl:if test="self::span[not(@class or @data-name or @data-commentrange or @data-comment or @data-commentimg or @data-commentgroup)][contains(@style,'font-variant: small-caps') or contains(@style,'font-variant:small-caps')][not(ancestor::small)][string-length(.) &gt; 0]">
