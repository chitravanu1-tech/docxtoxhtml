<!-- 01-SEP-18: To move Bookmarks to inside w:p -->
<!-- added to avoid the sequence on 07-10-2021 -->
<!-- Added to remove the endnoteref,which is applied for multiple para in same w:endnote Azure 6972 on 7-3-2023-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
     ...
     <xsl:template match="w:p[following-sibling::*[1][self::w:bookmarkStart]]">
           <xsl:element name="w:p">
                 <xsl:copy-of select="@*"/>
                 <xsl:apply-templates/>
                 <xsl:apply-templates select="following-sibling::*[1][self::w:bookmarkStart]" mode="moveBM"/>
           </xsl:element>
     </xsl:template>
      <xsl:template match="w:p[following-sibling::*[1][self::w:bookmarkEnd]]">
            <xsl:element name="w:p">
                  <xsl:copy-of select="@*"/>
                  <xsl:apply-templates/>
                  <xsl:apply-templates select="following-sibling::*[1][self::w:bookmarkEnd]" mode="moveBM"/>
            </xsl:element>
      </xsl:template>
    <!-- added to avoid badbreak([not(starts-with(@w:name,'BB'))]) bookmart start inside the content control on 19-4-2022  -->
    <xsl:template match="w:sdt[following-sibling::*[1][self::w:bookmarkStart][not(starts-with(@w:name,'BB'))]]">
      ...
      <xsl:template match="w:sdt[following-sibling::*[1][self::w:bookmarkEnd]]">
            <xsl:element name="w:sdt">
                  <xsl:copy-of select="@*"/>
                  <xsl:apply-templates/>
                  <xsl:apply-templates select="following-sibling::*[1][self::w:bookmarkEnd]" mode="moveBM"/>
      ...
      <xsl:template match="w:bookmarkStart | w:bookmarkEnd" mode="moveBM">
            <xsl:copy>
                  <xsl:copy-of select="@*"/>
                  <xsl:apply-templates/>
            </xsl:copy>
    ...
    <!-- added to avoid badbreak([not(starts-with(@w:name,'BB'))]) bookmart start inside the content control on 19-4-2022  -->
    <xsl:template match="w:bookmarkStart[preceding-sibling::*[1][self::w:p]] | w:bookmarkStart[not(starts-with(@w:name,'BB'))][preceding-sibling::*[1][self::w:sdt]]"/>
      <xsl:template match="w:bookmarkEnd[preceding-sibling::*[1][self::w:p]] | w:bookmarkEnd[preceding-sibling::*[1][self::w:sdt]]"/>
...
<xsl:output-character character=" " string="&amp;#x2029;"/>
  ...
  <!-- added to avoid the sequence on 07-10-2021 -->
  <xsl:template match="w:del/w:r/w:endnoteRef"/>
    <!--Added to avoid the restart issue after section break on 27-10-2021-->
    <xsl:template match="w:p[descendant::w:sectPr]">       
        <xsl:copy>
            <xsl:apply-templates select="@*|node()"/>
        </xsl:copy>
        <!--<xsl:apply-templates select="descendant::w:sectPr"/>-->
        <xsl:copy-of select="descendant::w:sectPr"/>
    </xsl:template>
    <xsl:template match="w:p/w:pPr/w:sectPr"/>
    <!-- Added to remove the endnoteref,which is applied for multiple para in same w:endnote Azure 6972 on 7-3-2023-->
    <xsl:template match="//w:endnote[count(descendant::w:p) &gt; 1]/w:p[descendant::w:endnoteRef][preceding-sibling::w:p[1][descendant::w:endnoteRef]]/descendant::w:endnoteRef"/>      
    <xsl:template match="//w:endnote[count(descendant::w:p) &gt; 1]/w:p[descendant::w:endnoteRef][preceding-sibling::w:p[1][descendant::w:endnoteRef]]/w:r[descendant::w:endnoteRef]/w:rPr/w:rStyle[@w:val='EndnoteReference']"/>
    <!-- Footnotespace concept implemented by prakash on 7-12-2023 -->
    <xsl:template match="//w:footnote/w:p/w:r[w:t[string-length(translate(.,' ','')) &gt; 0]][1]/child::w:t">      
                <xsl:element name="w:bookmarkStart">
                    <xsl:attribute name="w:name">
                        <xsl:value-of select="'Remove-preceding-tags'"/>
                    </xsl:attribute>
                    </xsl:element>
    ...
    <!-- only For General -->
    <xsl:template match="//w:endnote/w:p[1]/w:r[w:t[string-length(translate(.,' ','')) &gt; 0]][not(preceding-sibling::*/w:r/w:t[string-length(translate(.,' ','')) &gt; 0])][1]/child::w:t[1]">      
        <xsl:element name="w:bookmarkStart">
            <xsl:attribute name="w:name">
                <xsl:value-of select="'Remove-preceding-tagsendnote'"/>
            </xsl:attribute>
        </xsl:element>
        <xsl:element name="w:t">
            <!--<xsl:text>&#xa0;</xsl:text>-->
    ...
    <!-- R&l only for Monograph on 26-4-2024 -->
    <!--<xsl:template match="//w:endnote/w:p[1]/w:r[w:t[string-length(translate(.,' ','')) &gt; 0]][not(preceding-sibling::*/w:r/w:t[string-length(translate(.,' ','')) &gt; 0])][1]/child::w:t[1]">      
        <xsl:element name="w:bookmarkStart">
            <xsl:attribute name="w:name">
                <xsl:value-of select="'Remove-preceding-tagsendnote'"/>
    ...
    <xsl:template match="//m:oMath">
        <xsl:copy>
            ...
            <xsl:attribute name="id"><xsl:value-of select="count(preceding::m:oMath)+1"/></xsl:attribute>
            <xsl:apply-templates select="node()"/>
        </xsl:copy>
    </xsl:template>
</xsl:stylesheet>