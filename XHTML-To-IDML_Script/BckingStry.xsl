<!--Entity Update16.02.2017-->
<!-- Added for indesign paragraph style changed issue on 15-11-2024 -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="Bckingstry" select="document('BackingStory.xml')"/>
...
<xsl:variable name="ID" select="$Bckingstry//XmlStory/@Self"/>
...
<xsl:output-character character="ϗ" string="&amp;#x03D7;"/>	
    ...
    <!-- Added for indesign paragraph style changed issue on 15-11-2024 -->
    <xsl:template match="//Br[preceding-sibling::*[1][self::CharacterStyleRange[Br]]]"/>
    <xsl:template match="//CharacterStyleRange[Br][preceding-sibling::*[1][self::CharacterStyleRange[Br]]]"/>
    
    <!-- Added to retain the character style property for adventurekeen Azure 18998 -->
    <xsl:template match="CharacterStyleRange[@FontStyle or @StrikeThru or @Capitalization or @Underline or @Position]">
        <xsl:variable name="cf"><xsl:value-of select="name(@FontStyle)"/></xsl:variable>
        <xsl:variable name="cStrikeThru"><xsl:value-of select="name(@StrikeThru)"/></xsl:variable>
        <xsl:variable name="cCapitalization"><xsl:value-of select="name(@Capitalization)"/></xsl:variable>
        <xsl:variable name="cUnderline"><xsl:value-of select="name(@Underline)"/></xsl:variable>
        <xsl:variable name="cPosition"><xsl:value-of select="name(@Position)"/></xsl:variable>
        
        <xsl:variable name="cstyle"><xsl:value-of select="@Name"/></xsl:variable>
        <xsl:variable name="FS">
            <xsl:for-each select="$sr//CharacterStyle[@MatchStyle = $cstyle][name(@FontStyle) = $cf]">
                <xsl:if test="string-length(@FontStyle) &gt; 0">
                    <xsl:value-of select="name(@FontStyle)"/>
                </xsl:if>
            </xsl:for-each>
        </xsl:variable>
        <xsl:variable name="FStrikeThru">
            <xsl:for-each select="$sr//CharacterStyle[@MatchStyle = $cstyle][name(@StrikeThru) = $cStrikeThru]">
                <xsl:if test="string-length(@StrikeThru) &gt; 0">
                    <xsl:value-of select="name(@StrikeThru)"/>
                </xsl:if>
            </xsl:for-each>
        </xsl:variable>
        <xsl:variable name="FCapitalization">
            <xsl:for-each select="$sr//CharacterStyle[@MatchStyle = $cstyle][name(@Capitalization) = $cCapitalization]">
                <xsl:if test="string-length(@Capitalization) &gt; 0">
                    <xsl:value-of select="name(@Capitalization)"/>
                </xsl:if>
            </xsl:for-each>
        </xsl:variable>
        <xsl:variable name="FUnderline">
            <xsl:for-each select="$sr//CharacterStyle[@MatchStyle = $cstyle][name(@Underline) = $cUnderline]">
                <xsl:if test="string-length(@Underline) &gt; 0">
                    <xsl:value-of select="name(@Underline)"/>
                </xsl:if>
            </xsl:for-each>
        </xsl:variable>
        <xsl:variable name="FPosition">
            <xsl:for-each select="$sr//CharacterStyle[@MatchStyle = $cstyle][name(@Position) = $cPosition]">
                <xsl:if test="string-length(@Position) &gt; 0">
                    <xsl:value-of select="name(@Position)"/>
                </xsl:if>
            </xsl:for-each>
        </xsl:variable>
        <xsl:copy>
            <xsl:choose>
                <!--<xsl:when test="$FS='FontStyle' or $FStrikeThru='StrikeThru' or $FCapitalization='Capitalization' or $FUnderline='Underline' or $FPosition='Position'">
                    <xsl:apply-templates select="@* except (@FontStyle|@StrikeThru|@Capitalization|@Underline|@Position)"/>
                </xsl:when>-->
                <xsl:when test="$FS='FontStyle'">
                    <xsl:apply-templates select="@* except @FontStyle"/>
                </xsl:when>
                <xsl:when test="$FStrikeThru='StrikeThru'">
                    <xsl:apply-templates select="@* except @StrikeThru"/>
                </xsl:when>
                <xsl:when test="$FCapitalization='Capitalization'">
                    <xsl:apply-templates select="@* except @Capitalization"/>
                </xsl:when>
                <xsl:when test="$FUnderline='Underline'">
                    <xsl:apply-templates select="@* except @Underline"/>
                </xsl:when>
                <xsl:when test="$FPosition='Position'">
