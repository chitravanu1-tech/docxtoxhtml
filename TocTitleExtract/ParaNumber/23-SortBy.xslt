...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="*[@w:rsidR]">
        ...
        <xsl:variable name="txtFrameId">
            <xsl:choose>
                <xsl:when
                    test="not(contains(substring-after(lower-case(./@w:rsidR), '_txtid_'), '_'))">
                    <xsl:choose>
                        <xsl:when test="contains(lower-case(./@w:rsidR), '_txtid_')">
                            <xsl:value-of
                                select="substring-after(lower-case(./@w:rsidR), '_txtid_')"/>
                        </xsl:when>
                    </xsl:choose>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:choose>
                        <xsl:when test="contains(lower-case(./@w:rsidR), '_txtid_')">
                            <xsl:value-of
                                select="substring-before(substring-after(lower-case(./@w:rsidR), '_txtid_'), '_')"
                            />
                        </xsl:when>
                    </xsl:choose>
                </xsl:otherwise>
        ...
        <xsl:variable name="PageValue">
            <xsl:variable name="Page">
                <xsl:choose>
                    <xsl:when
                        test="contains(lower-case(./@w:rsidR), '_p_') and contains(lower-case(./@w:rsidR), '_x_')">
                        <xsl:value-of
                            select="substring-before(substring-after(lower-case(./@w:rsidR), '_p_'), '_x_')"
                        />
                    </xsl:when>
                    <xsl:when
                        test="contains(lower-case(./@w:rsidR), '_p_') and contains(lower-case(./@w:rsidR), '_tx_')">
                        <xsl:value-of
                            select="substring-before(substring-after(lower-case(./@w:rsidR), '_p_'), '_tx_')"
                        />
                    </xsl:when>
                    <xsl:when
                        test="contains(lower-case(./@w:rsidR), '_p_') and contains(lower-case(./@w:rsidR), '_ix_')">
                        <xsl:value-of
                            select="substring-before(substring-after(lower-case(./@w:rsidR), '_p_'), '_ix_')"
                        />
                    </xsl:when>
                </xsl:choose>
            </xsl:variable>
        ...
        <xsl:variable name="XValue">
            <xsl:choose>
                <xsl:when
                    test="contains(lower-case(./@w:rsidR), '_x_') and contains(lower-case(./@w:rsidR), '_y_')">
                    <xsl:value-of
                        select="substring-before(substring-after(lower-case(./@w:rsidR), '_x_'), '_y_')"
                    />
                </xsl:when>
                <xsl:when
                    test="contains(lower-case(./@w:rsidR), '_tx_') and contains(lower-case(./@w:rsidR), '_ty_')">
                    <xsl:value-of
                        select="substring-before(substring-after(lower-case(./@w:rsidR), '_tx_'), '_ty_')"
                    />
                </xsl:when>
                <xsl:when
                    test="contains(lower-case(./@w:rsidR), '_ix_') and contains(lower-case(./@w:rsidR), '_iy_')">
                    <xsl:value-of
                        select="substring-before(substring-after(lower-case(./@w:rsidR), '_ix_'), '_iy_')"
                    />
                </xsl:when>
            </xsl:choose>
        </xsl:variable>
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:choose>
                        <xsl:when test="contains(lower-case(./@w:rsidR), '_y_')">
                            <xsl:value-of
                                select="substring-before(substring-after(lower-case(./@w:rsidR), '_y_'), '_')"
                            />
                        </xsl:when>
                        <xsl:when test="contains(lower-case(./@w:rsidR), '_ty_')">
                            <xsl:value-of
                                select="substring-before(substring-after(lower-case(./@w:rsidR), '_ty_'), '_')"
                            />
                        </xsl:when>
                        <xsl:when test="contains(lower-case(./@w:rsidR), '_iy_')">
                            <xsl:value-of
                                select="substring-before(substring-after(lower-case(./@w:rsidR), '_iy_'), '_')"
                            />
                        </xsl:when>
                    </xsl:choose>
                </xsl:otherwise>
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:choose>
                        <xsl:when test="contains(lower-case(./@w:rsidR), '_c_')">
                            <xsl:value-of
                                select="substring-before(substring-after(lower-case(./@w:rsidR), '_c_'), '_')"
                            />
                        </xsl:when>
                    </xsl:choose>
                </xsl:otherwise>
        ...
        <xsl:variable name="storyValue">
            <xsl:choose>
                <!--                    <xsl:when test="ends-with($endCharacter,'_y_')">-->
                <xsl:when
                    test="not(contains(substring-after(lower-case(./@w:rsidR), '_story'), '_'))">
                    <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:choose>
                        <xsl:when test="contains(lower-case(./@w:rsidR), '_story')">
                            <xsl:value-of
                                select="substring-before(substring-after(lower-case(./@w:rsidR), '_story'), '_')"
                            />
                        </xsl:when>
                    </xsl:choose>
                </xsl:otherwise>
...
<xsl:template match="ParagraphStyleRange">
    <!--22-11-2023 added for include FontStyle inforamtion from linked.xml    -->
    <xsl:variable name="paraStyle">
       <!-- <xsl:choose>
            <xsl:when test="contains(@AppliedParagraphStyle, '$ID/')">
                <xsl:value-of
                    select="translate(substring-after(@AppliedParagraphStyle, '$ID/'), ' /\[\]@#&lt;&gt;%', '')"
                />
            </xsl:when>
            <xsl:when test="contains(@AppliedParagraphStyle, 'ParagraphStyle/')">
                <xsl:value-of
                    select="translate(substring-after(@AppliedParagraphStyle, 'ParagraphStyle/'), ' /\[\]@#&lt;&gt;%', '')"
                />
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of
                    select="translate(@AppliedParagraphStyle, ' /\[\]@#&lt;&gt;%', '')"
                />
            </xsl:otherwise>
        </xsl:choose>-->
        <xsl:value-of select="@Client-Style"/>
    </xsl:variable>
    <xsl:variable name="fontInfo" select="$linkedStyle/idPkg:Styles/RootParagraphStyleGroup/ParagraphStyle[@Name = $paraStyle]/@FontStyle"/>
    <xsl:variable name="position" select="$linkedStyle/idPkg:Styles/RootParagraphStyleGroup/ParagraphStyle[@Name = $paraStyle]/@Position"/>
    <xsl:variable name="capitalization" select="$linkedStyle/idPkg:Styles/RootParagraphStyleGroup/ParagraphStyle[@Name = $paraStyle]/@Capitalization"/>
    <xsl:variable name="underline" select="$linkedStyle/idPkg:Styles/RootParagraphStyleGroup/ParagraphStyle[@Name = $paraStyle]/@Underline"/>
    <xsl:choose>
        <xsl:when test="string-length($fontInfo) &gt; 0">            
            <xsl:copy>
                <xsl:apply-templates select="@*"/>
                <xsl:attribute name="FontStyle" select="$fontInfo"/>
                <xsl:apply-templates select="node()"/>
            </xsl:copy>
        </xsl:when>
        ...
        <xsl:when test="string-length($capitalization) &gt; 0">            
            <xsl:copy>
                <xsl:apply-templates select="@*"/>
                <xsl:attribute name="Capitalization" select="$capitalization"/>
                <xsl:apply-templates select="node()"/>
            </xsl:copy>
        </xsl:when>
        <xsl:when test="string-length($underline) &gt; 0">            
            <xsl:copy>
                <xsl:apply-templates select="@*"/>
    ...
    <xsl:template match="CharacterStyleRange[not(descendant::Content[string-length(translate(.,'&#x200C;','')) &gt; 0])]"/>
    <xsl:template match="XMLElement">
        <xsl:apply-templates/>
    </xsl:template>

    <xsl:template match="//Page | //Margin | //PXOrigin | //PYOrigin | //PageH | //PageW | //StoryPreference | //InCopyExportOption"/>
        ...
        <xsl:if test="$i + $stepd1e12 &lt;= $tod1e12">
            <xsl:call-template name="for-loop">
                ...
                <xsl:with-param name="j"
                    select="$j + parent::Table/Column[$i + 1]/@SingleColumnWidth"/>
