...
<!-- 09-AUG-17: Added for IDML 2 HTML -->
    <xsl:template match="HyperlinkTextDestination|XMLInstruction|w:t|XMLAttribute|w:tcPr|w:trHeight|InCopyExportOption|w:r|w:numPr|Change|w:ilvl|w:numId">
        <xsl:apply-templates/>
    </xsl:template>
         ...
         <!--<xsl:for-each select="descendant::w:p">-->
            <xsl:element name="p">
              ...
              <!--<xsl:if test="child::w:p/w:bookmarkStart">
                <xsl:variable name="bmId" select="@w:id"/>
                <xsl:text disable-output-escaping="yes">&lt;spanX data-bkmark="</xsl:text>
