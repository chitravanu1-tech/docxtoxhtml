...
<!-- 07-11-2023 below story variable commented for ignore xhtml structure for toc creation process   -->
    <!--<xsl:variable name="story" select="document('Inp.xml')"/>-->
    <xsl:variable name="story" select="document('StyleCollMap.xml')"/>
    ...
    <xsl:template match="/">
        <xsl:element name="w:body">
            <xsl:variable name="findLayerVisibilityId">
                <xsl:for-each select="descendant::TextFrame[@layerVisibility = 'false']">
                    <xsl:value-of select="@id"/>
                    <xsl:value-of select="'::'"/>
                </xsl:for-each>
            </xsl:variable>
            
            <xsl:attribute name="layerVisibilityFalseId" select="$findLayerVisibilityId"/>                
...
<!--                                     <xsl:copy-of select="document($Sto)//body/*"/>-->
    ...
    <xsl:template match="Rectangle|Polygon" name="RF">  
        <xsl:choose>
                <xsl:when test="@Image">
...
<!--                            <xsl:element name="wp:wrapTopAndBottom"/>-->
                            ...
                            <xsl:element name="wp:docPr">
                                ...
                                <xsl:if test="@ObjectStyle[lower-case(.) = 'objectstyle/map-key']">
                                    <xsl:attribute name="ObjectStyle">
                                        <xsl:value-of select="@ObjectStyle"/>
                                    </xsl:attribute>
                                </xsl:if>
