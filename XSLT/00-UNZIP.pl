...
if ($zip->read($inputFile) == AZ_OK) {
  ...
  foreach my $member (@members) {
 	  	if($member =~ m/(word\/(comments|endnotes|fontTable|footer1|footer2|footnotes|header1|header2|numbering|settings|styles|webSettings)\.xml|_rels\/document\.xml\.rels|_rels\/endnotes.xml\.rels|_rels\/footnotes.xml\.rels|customXml\/(item1|item2|item3|item4|item5|item6|item7|item8|item9|item10|item11|item12|item13|item14|item15|item16|item17|item18|item19|item20)\.xml)/i) {
