...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:mf="http://example.com/mf" exclude-result-prefixes="xs mf" 
    ...
    <xsl:variable name="imglink" select="document('SpreadImageLink.xml')"/>
   ...
   --> 
    ...
    <!-- Added [not(parent::li[@class='endnotecaption'])] for empty para retained in review xhtml on 16-3-2022 -->
    <xsl:template match="p[@data-noemptypara][string-length(.)=0][not(descendant::img)][not(descendant::Comment)]"/>
    <!-- added to highlight the tab on 03-03-2023 -->
    <xsl:template match="p[@data-noemptypara][string-length(normalize-space(.))=0][.='&#x9;' or .=' '][not(descendant::img)][not(descendant::Comment)]">
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:attribute name="class" select="'emptypara'"/>
            <xsl:apply-templates select="node()"/>
    ...
    <xsl:template match="p[@data-noemptypara]/@data-noemptypara"/>
    ...
    <xsl:template match="//p[img][string-length(.) = 0]">
        
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:variable name="img"><xsl:value-of select="child::img/@data-src"/></xsl:variable>
            <xsl:if test="$imglink/SOURCE/MAPS//Group[descendant::Link[tokenize(@LinkResourceURI, '/')[last()] = $img]]">
                <xsl:attribute name="data-parentStory"><xsl:value-of select="$imglink/SOURCE/MAPS//Group[descendant::Link[tokenize(@LinkResourceURI, '/')[last()] = $img]]/descendant::TextFrame/@Self"/></xsl:attribute>
                <xsl:attribute name="parentStory"><xsl:value-of select="$imglink/SOURCE/MAPS//Group[descendant::Link[tokenize(@LinkResourceURI, '/')[last()] = $img]]/descendant::TextFrame/@ParentStory"/></xsl:attribute>
            </xsl:if>
            <xsl:apply-templates select="node()"/>
        </xsl:copy>
    </xsl:template>
    <!--<xsl:template match="div[@data-type='textbox']">
        <xsl:variable name="parastyle"><xsl:value-of select="descendant::p[1]/@data-name"/></xsl:variable>
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:attribute name="data-boxstyle"><xsl:value-of select="$linkedStyle//ObjectStyle[substring-after(@AppliedParagraphStyle,'ParagraphStyle/') = $parastyle][1]/substring-after(@Self,'ObjectStyle/')"/></xsl:attribute>
            <xsl:apply-templates select="node()"/>
        </xsl:copy>
    </xsl:template>-->
    <!-- math 24-09-2025 -->
    <xsl:template match="//span[@latex]">
        <xsl:element name="span">
            <xsl:apply-templates select="@*"/>
            <xsl:apply-templates select="node()"/>
        </xsl:element>
    </xsl:template>
    <!--To remove the math img on 26-09-2025-->
    <xsl:template match="//img[@data-src = following-sibling::span[@id='mathjax']/@data-fileName]"/>
    </xsl:stylesheet>