...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:mf="http://example.com/mf" exclude-result-prefixes="xs mf" 
 ...
 <xsl:template match="@*|node()">
   ...
   <!-- <xsl:template match="SOURCE">
    ...
    <!-- contains(@data-AppliedObjectStyle,'$ID') Added for boxgroup from spred on 11-03-2025 -->
    <xsl:template match="//div[not(@class) and not(@data-type = 'textbox') and contains(@data-AppliedObjectStyle,'$ID') or contains(lower-case(@data-AppliedObjectStyle),'caption') or contains(lower-case(@data-AppliedObjectStyle),'running')]">        
        <xsl:apply-templates/>
    </xsl:template>
    <!-- added to ignore the group conten control in review xhtml on 6-10-2025 -->
    <xsl:template match="//div[not(@class) and not(@data-type = 'textbox') and not(contains(@data-AppliedObjectStyle,'$ID') or contains(lower-case(@data-AppliedObjectStyle),'caption') or contains(lower-case(@data-AppliedObjectStyle),'running'))]">        
        <xsl:apply-templates/>
    </xsl:template>
