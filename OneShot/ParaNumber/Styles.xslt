<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- As per Bhaskar advice on 17-Nov-17, i have taken "medium" fonts to un-bold -->
...
<!--                  <xsl:if test="contains(@FontStyle,'Black') or contains(@FontStyle,'Bold') or contains(@FontStyle,'Strong') or contains(@FontStyle,'black') or contains(@FontStyle,'bold') or contains(@FontStyle,'strong') or contains(@FontStyle,'Extra') or contains(@FontStyle,'extra') or contains(@FontStyle,'Heavy') or contains(@FontStyle,'heavy') ">-->
                      <!-- As per Bhaskar advice on 17-Nov-17, i have taken "medium" fonts to un-bold -->
                      <xsl:if test="contains(lower-case(@FontStyle),'medXium') or contains(lower-case(@FontStyle),'black') or contains(lower-case(@FontStyle),'bold') or contains(lower-case(@FontStyle),'strong') or contains(lower-case(@FontStyle),'demi') or contains(lower-case(@FontStyle),'extra') or contains(lower-case(@FontStyle),'heavy') ">
           ...
           <!-- <xsl:choose>
                <xsl:when test="./@Self eq $colorName">-->
                    ...
                    <xsl:variable name="B" select="number($RGB[3])"/>-->
                        <!--<xsl:variable name="R" select="round(number($RGB[1]) * 255 div 100)"/>
                        <xsl:variable name="G" select="round(number($RGB[2]) * 255 div 100)"/>
                        <xsl:variable name="B" select="round(number($RGB[3]) * 255 div 100)"/>-->
                    
                        <xsl:variable name="R">
                            <xsl:choose>
                                <xsl:when test="contains(./@ColorValue,'.')">
                                    <xsl:value-of select="round(number($RGB[1]) * 255 div 100)"/>
                                </xsl:when>
                                <xsl:otherwise>
                                    <xsl:value-of select="number($RGB[1])"/>
                                </xsl:otherwise>
                            </xsl:choose>
                        </xsl:variable>
                        <xsl:variable name="G">
                            <xsl:choose>
                                <xsl:when test="contains(./@ColorValue,'.')">
                                    <xsl:value-of select="round(number($RGB[2]) * 255 div 100)"/>
                                </xsl:when>
                                <xsl:otherwise>
                                    <xsl:value-of select="number($RGB[2])"/>
                                </xsl:otherwise>
                            </xsl:choose>
                        </xsl:variable>
                        <xsl:variable name="B">
                            <xsl:choose>
                                <xsl:when test="contains(./@ColorValue,'.')"> 
                                    <xsl:value-of select="round(number($RGB[3]) * 255 div 100)"/>
                                </xsl:when>
                                <xsl:otherwise>
                                    <xsl:value-of select="number($RGB[3])"/>
                                </xsl:otherwise>
                            </xsl:choose>
                        </xsl:variable>
