...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:mf="http://example.com/mf" exclude-result-prefixes="xs mf" 
    ...
    <xsl:template match="SOURCE">
        ...
        <xsl:template match="ParagraphStyleRange">  
            <!--<xsl:variable name="P" select="@AppliedParagraphStyle"/>-->
           ...
           <!-- <xsl:variable name="TX"> 
            ...
            <xsl:variable name="height">
                ...
                <xsl:if test="contains($PN,'_H_') and contains($PN,'_T_')">
                    <xsl:value-of select="number(substring-before(substring-after($PN, '_H_'), '_T_'))"/>
                </xsl:if>
                </xsl:variable>
                    ...
                    <xsl:element name="w:p">  
                        ...
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                ...
                                <xsl:for-each select="descendant::HyperlinkTextDestination[contains(@BookMarkDestination,'_UUID_') or contains(@BookMarkDestination,'story') or contains(@Name,'_UUID_') or contains(lower-case(@Name),'_story_')] | descendant::HyperlinkTextSource[contains(lower-case(@HyperlinkUDestination),'_story_')]">
                                    <xsl:variable name="id">
                                        <xsl:number level="any"/>
                                    </xsl:variable>
                                    <xsl:variable name="HUD">
    ...
    <xsl:template match="CharacterStyleRange">
        ...
        <xsl:variable name="Kern" select="@KerningValue"/>
                ...
                <xsl:for-each select="descendant::Note">
                    <xsl:variable name="ancestorRun" select="ancestor::CharacterStyleRange[1]"/>
                    <xsl:if test="$curRun = $ancestorRun">
...
<!--                                        new update 28.9.2016-->
                                ...
                                <xsl:attribute name="w:val">
                                    <xsl:choose>
                                        ...
                                        <xsl:otherwise>
                                            <xsl:value-of select="concat($csn,' ','')"/>
                                        </xsl:otherwise>
                                    </xsl:choose>
                                </xsl:attribute>
                            </xsl:element>
            ...
            <xsl:element name="w:tblGrid">
                <xsl:for-each select="self::Table/Column">
                    ...
                    <xsl:element name="w:gridCol">
                        <xsl:attribute name="w:w" select="round($CW * 20)"/>
                    </xsl:element>
                </xsl:for-each>
            </xsl:element>
