...
<!--            <xsl:if test="not($linked//ParagraphStyle[@Self = $ParaStyle]/@ManualNumberRestart)"></xsl:if>-->
    ...
    <xsl:template
        ...
        | HyperlinkTextDestination[contains(@BookMarkDestination,'_UUID_') or contains(@BookMarkDestination,'story') or contains(@Name,'_UUID_') or contains(lower-case(@Name),'_story_')] | Change 
        | HiddenText | DTD | Oval 
