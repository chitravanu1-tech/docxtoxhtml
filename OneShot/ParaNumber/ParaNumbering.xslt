...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- 13-MAR-19 -->
    <!-- Added to avoid the text missing by both BookMarkDestination and HyperlinkUDestination in same para on 4-1-2021 -->
    <!-- Added to retain the empty para which is from indd  on 11-1-2023 Azure 5779-->
    ...
    <xsl:template match="ParagraphStyleRange" priority="1.5">
        ...
        <xsl:copy>
            ...
            <xsl:variable name="bookmark">
                <xsl:value-of select="count(descendant::HyperlinkTextDestination[contains(@BookMarkDestination,'_UUID_') or contains(@BookMarkDestination,'story') or contains(@Name,'_UUID_') or contains(lower-case(@Name),'_story_')][ancestor::ParagraphStyleRange[1] = $curPara][@BookMarkDestination] 
            ...
            <xsl:message select="$bookmark"></xsl:message>
            <xsl:attribute name="BookmarkCount" select="$bookmark"/>
            <xsl:for-each select="descendant::HyperlinkTextDestination[contains(@BookMarkDestination,'_UUID_') or contains(@BookMarkDestination,'story') or contains(@Name,'_UUID_') or contains(lower-case(@Name),'_story_')][ancestor::ParagraphStyleRange[1] = $curPara][@BookMarkDestination] | descendant::HyperlinkTextSource[contains(lower-case(@HyperlinkUDestination),'_story_')][ancestor::ParagraphStyleRange[1] = $curPara][@HyperlinkUDestination]">
                <xsl:variable name="curBookMarkVal">
                    <xsl:if test="(self::HyperlinkTextSource[ancestor::ParagraphStyleRange[1] = $curPara]/@HyperlinkUDestination | self::HyperlinkTextDestination[contains(@BookMarkDestination,'_UUID_') or contains(@BookMarkDestination,'story') or contains(@Name,'_UUID_') or contains(lower-case(@Name),'_story_')][ancestor::ParagraphStyleRange[1] = $curPara]/@BookMarkDestination)">
                        <xsl:choose>
                            <xsl:when test="self::HyperlinkTextDestination[contains(@BookMarkDestination,'_UUID_') or contains(@BookMarkDestination,'story') or contains(@Name,'_UUID_') or contains(lower-case(@Name),'_story_')][ancestor::ParagraphStyleRange[1] = $curPara]/@BookMarkDestination">
                                <xsl:value-of select="self::HyperlinkTextDestination[contains(@BookMarkDestination,'_UUID_') or contains(@BookMarkDestination,'story') or contains(@Name,'_UUID_') or contains(lower-case(@Name),'_story_')][ancestor::ParagraphStyleRange[1] = $curPara]/@BookMarkDestination"/>
                            </xsl:when>
            ...
            <!-- Added to retain the empty para which is from indd  on 11-1-2023 Azure 5779-->
            <xsl:if test="self::ParagraphStyleRange[not(parent::*[1][self::XMLElement])][descendant::Content][string-length(normalize-space(.)) = 0][descendant::Br] or self::ParagraphStyleRange[not(parent::*[1][self::XMLElement])][string-length(normalize-space(.))=0][not(descendant::Content)][descendant::Br] or self::ParagraphStyleRange[following-sibling::*[1][self::Table]][descendant::Content[string-length(normalize-space(.)) = 0] and descendant::Properties[string-length(.) &gt; 0]][not(descendant::Br) or descendant::Br] or self::ParagraphStyleRange[following-sibling::*[1][self::Table]][not(descendant::Content) and descendant::Properties[string-length(.) &gt; 0]][descendant::Br]">
                <xsl:attribute name="noemptypara" select="'true'"/>
            </xsl:if>
            <!-- end -->
            ...
            <xsl:if test="$bookmark = 0">                
              <!--<xsl:variable name="preParaBookM" select="preceding::ParagraphStyleRange[ancestor::Story[1]/@Self = $story][descendant::HyperlinkTextDestination[ancestor::ParagraphStyleRange[1] = $curPara][contains(lower-case(@BookMarkDestination),'story')][1]][1]/descendant::HyperlinkTextDestination[ancestor::ParagraphStyleRange[1] = $curPara][contains(lower-case(@BookMarkDestination),'story')][1]/@BookMarkDestination | preceding::ParagraphStyleRange[ancestor::Story[1]/@Self = $story][descendant::HyperlinkTextSource[contains(lower-case(@HyperlinkUDestination),'story')][1]][1]/descendant::HyperlinkTextSource[contains(lower-case(@HyperlinkUDestination),'story')][1]/@HyperlinkUDestination"/>-->
                ...
                <!-- 13-MAR-19 -->
                <!-- Added for table not come inside of review xhtml([ancestor::Story[1]/@Self = $story] added in last)  on 10-2-2022 -->
                <xsl:variable name="prePara" select="
                    preceding::ParagraphStyleRange[1][ancestor::Story[1]/@Self = $story][descendant::HyperlinkTextDestination[contains(@BookMarkDestination,'_UUID_') or contains(@BookMarkDestination,'story') or contains(@Name,'_UUID_') or contains(lower-case(@Name),'_story_')][ancestor::ParagraphStyleRange[1] = $curPara][1]][1]/descendant::HyperlinkTextDestination[contains(@BookMarkDestination,'_UUID_') or contains(@BookMarkDestination,'story') or contains(@Name,'_UUID_') or contains(lower-case(@Name),'_story_')][1] 
                    ...
                    | preceding::ParagraphStyleRange[1][ancestor::Story[1]/@Self = $story][descendant::HyperlinkTextDestination[contains(@BookMarkDestination,'_UUID_') or contains(@BookMarkDestination,'story') or contains(@Name,'_UUID_') or contains(lower-case(@Name),'_story_')][1]][1]/descendant::HyperlinkTextDestination[contains(@BookMarkDestination,'_UUID_') or contains(@BookMarkDestination,'story') or contains(@Name,'_UUID_') or contains(lower-case(@Name),'_story_')][1]"/>
...
<!--                <xsl:variable name="preParaBookmarkXYColumnVal" select="$prePara[@bkmark]/@bkmark"/>-->
                
                <xsl:choose>
                    <xsl:when test="string-length($preParaBookM) > 0">
                        <xsl:variable name="wPreParaCount" select="count(preceding::ParagraphStyleRange)"/>
                        <xsl:variable name="preParaParaLid" select="$wPreParaCount - count((preceding::ParagraphStyleRange[ancestor::Story[1]/@Self = $story][descendant::HyperlinkTextDestination[contains(@BookMarkDestination,'_UUID_') or contains(@BookMarkDestination,'story') or contains(@Name,'_UUID_') or contains(lower-case(@Name),'_story_')][1]][1] | preceding::ParagraphStyleRange[ancestor::Story[1]/@Self = $story][descendant::HyperlinkTextSource[contains(lower-case(@HyperlinkUDestination),'_story_')][1]][1])/preceding::ParagraphStyleRange)"/>
                    ...
                    <xsl:otherwise>
                        ...
                        <!--<xsl:variable name="followParaBookmarkXYColumnVal" select="$followingPara[@bkmark]/@bkmark"/>-->
                        ...
                        <xsl:choose>
                            <xsl:when test="string-length($followParaBookM) > 0">
                                <xsl:variable name="curBookMsuffix">
                                    ...
                                    <xsl:variable name="preParaParaLid" select="$wPreParaCount - count((following::ParagraphStyleRange[ancestor::Story[1]/@Self = $story][descendant::HyperlinkTextDestination[contains(@BookMarkDestination,'_UUID_') or contains(@BookMarkDestination,'story') or contains(@Name,'_UUID_') or contains(lower-case(@Name),'_story_')][1]][1] | following::ParagraphStyleRange[ancestor::Story[1]/@Self = $story][descendant::HyperlinkTextSource[contains(lower-case(@HyperlinkUDestination),'_story_')][1]][1])/following::ParagraphStyleRange)"/>
