<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:mf="http://example.com/mf" exclude-result-prefixes="xs mf" 
    ...
    <!-- 08-Nov-18: New add to delete deleted text -->
  ...
  <xsl:template match="*">
   <xsl:element name="{name()}">
    ...
    <!-- CUSTOM Changes by RTP on 09-Aug-17 upto below -->
    <xsl:template match="one[@numberFormat = '' and @numberType = '' and @start = '']"/>
    <xsl:template match="td">
        <xsl:choose>
            <xsl:when test="child::*[1][self::w:vMerge]">
                <!-- To Delete this td, because previous td has "morerow" attribute -->
            </xsl:when>
            <xsl:otherwise>
                <xsl:element name="td">
                    <xsl:copy-of select="@*"/>
    ...
    <xsl:template match="span[@class = '[No character style]']">
        <xsl:apply-templates/>
    </xsl:template>
  
  
    <xsl:template match="HyperlinkTextDestination[contains(@BookMarkDestination,'_UUID_') or contains(@BookMarkDestination,'story') or contains(@Name,'_UUID_') or contains(lower-case(@Name),'_story_')]|XMLInstruction|w:t|XMLAttribute|w:tcPr|w:trHeight|InCopyExportOption|w:r|w:numPr|Change">
      <xsl:apply-templates/>
  </xsl:template>
    
    <xsl:template match="p|one|two|table">
        <xsl:text>&#x000A;</xsl:text>
    ...
    <!-- 08-Nov-18: New add to delete deleted text -->
    <xsl:template match="Change[@ChangeType='DeletedText']"/>
