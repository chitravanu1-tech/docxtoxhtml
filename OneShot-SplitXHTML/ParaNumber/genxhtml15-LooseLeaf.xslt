...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:mf="http://example.com/mf" exclude-result-prefixes="xs mf" 
  ...
  <!-- ************************ 15-Nov-17: For aspen LL, i generally commented the back highlights said by Bhaskar -->
    <!-- Global Vaiable -->
...
<xsl:template match="w:pict">
  ...
  <xsl:template match="//pQQQQ">
    <xsl:choose>
      <xsl:when test="child::w:pPr/w:ind">
        <xsl:element name="p">
          <xsl:copy-of select="@*"/>
          <xsl:attribute name="style">
            <xsl:text>mso-style-name:</xsl:text>
  ...
  <xsl:template match="//p">
    <xsl:choose>
      <xsl:when test="contains(@w:rsidR,'_unp') and not(descendant::w:t)">
      </xsl:when>
      <xsl:otherwise>
        <xsl:copy>
