...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="reviwXhtml" select="document('Review.xhtml')"/>
    ...
    <!--<xsl:template match="xhtml:*[child::*[1][self::xhtml:span[@data-bkmark]]] | xhtml:*[xhtml:*[descendant::*[self::xhtml:span[@data-bkmark]]]]">-->
    <xsl:template match="xhtml:p[descendant::*[self::xhtml:span[@data-bkmark]]] | xhtml:li[descendant::*[self::xhtml:span[@data-bkmark]]] | xhtml:figcaption[descendant::*[self::xhtml:span[@data-bkmark]]]">
        
        <xsl:variable name="selfBkMark" select="descendant::xhtml:span[matches(@data-bkmark,'para')][1]/@data-bkmark"/>        

        <xsl:copy>
            <xsl:apply-templates select="@*"/>   
            <xsl:if test="$reviwXhtml/xhtml:html/xhtml:body//descendant::xhtml:span[matches(@data-bkmark,'para')][1]/@data-bkmark/substring-after(.,'_UUID_') = //descendant::xhtml:span[matches(@data-bkmark,'para')][1]/@data-bkmark">
                <xsl:variable name="val-1">
                    <xsl:value-of select="$reviwXhtml/xhtml:html/xhtml:body//descendant::xhtml:span[matches(@data-bkmark,'para')][1]/@data-bkmark[substring-after(.,'_UUID_') = $selfBkMark]"/>
                </xsl:variable>
                <xsl:if test="string-length($val-1) &gt; 0">               
                    <xsl:attribute name="data-page">                    
                    <xsl:value-of select="substring-after(substring-before($val-1,'_txtid_'),'_Story_p_')"/>
                    </xsl:attribute>
                </xsl:if>                
                
            </xsl:if>
                <!--<xsl:for-each select="$reviwXhtml/xhtml:html/xhtml:body/xhtml:p/descendant::xhtml:span[matches(@data-bkmark,'para')][1]/@data-bkmark/substring-after(.,'_UUID_') = self::xhtml:p/descendant::xhtml:span[matches(@data-bkmark,'para')][1]/@data-bkmark"> 
                    <xsl:element name="span" xmlns="http://www.w3.org/1999/xhtml">
                            <xsl:attribute name="data-bkmark">
                                <xsl:value-of select="$reviwXhtml/xhtml:html/xhtml:body/xhtml:p/descendant::xhtml:span[matches(@data-bkmark,'para')][1]/@data-bkmark[substring-after(.,'_UUID_') = $selfBkMark]"/>
                            </xsl:attribute>
                        </xsl:element>      
                </xsl:for-each> -->           
               <xsl:apply-templates select="node()"/>            
...
<!--    <xsl:template match="xhtml:span[@data-bkmark[starts-with(.,'para')]]"/>-->

    <!--Ascii2Unicode added from WordtoXHTML process with join-elements.xslt-->
