...
<!--        <xsl:if test="following-sibling::*[1]/name() = 'Content'">
        ...
        <!--<xsl:apply-templates select="document($story)//Content"/>-->
                  <!--<xsl:apply-templates select="document($story)//Story" mode="textframe"/>-->
    ...
    <xsl:template match="XMLElement">
        <xsl:text>&#x000A;</xsl:text>
        <xsl:text disable-output-escaping="yes">&lt;!--</xsl:text>
        <xsl:element name="{substring-after(@MarkupTag,'XMLTag/')}">
            <xsl:attribute name="data-type" select="'XML'"/>
            <xsl:for-each select="descendant::XMLAttribute">
                ...
                <xsl:attribute name="{$x2}" select="$y"/>
