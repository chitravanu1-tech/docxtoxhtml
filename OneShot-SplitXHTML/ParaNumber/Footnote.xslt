...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:mf="http://example.com/mf" exclude-result-prefixes="xs mf idPkg" 
    ...
    version="2.0">
    <!--To avoid the property value as a text in Review xhtml footnote on  29-6-2023-->
    ...
    <xsl:template match="/">
        <w:footnotes xmlns:wpc="http://schemas.microsoft.com/office/word/2010/wordprocessingCanvas"
            ...
            <xsl:element name="w:footnote">
                <xsl:attribute name="w:type" select="'continuationSeparator'"/>
                <xsl:attribute name="w:id" select="'0'"/>
                <xsl:element name="w:p">
                    <xsl:element name="w:pPr">
                        <xsl:element name="w:spacing">
                            <xsl:attribute name="w:after" select="'0'"/>
                    ...
                    <xsl:element name="w:r">
                        <xsl:element name="w:continuationSeparator"/>
                    </xsl:element>
                </xsl:element>
            </xsl:element>
<!--            <xsl:apply-templates select="//STORIES/STORY"/>-->
   ...
   <!-- <xsl:template match="STORIES/STORY">
        ...
        <xsl:apply-templates select="document($M)//Footnote"/>
    </xsl:template>-->
  ...
  <xsl:template match="ParagraphStyleRange">  
          <xsl:variable name="Na1">
            ...
            <xsl:variable name="ListNRestart">
                <xsl:copy-of select="child::w:numPr"/>
            </xsl:variable>
            ...
            <xsl:element name="w:p">  
                        ...
                        <!-- only for the footnotes.xml -->
                
                <xsl:for-each select="descendant::HyperlinkTextDestination[not(contains(@BookMarkDestination,'_UU-Old-ID_'))][contains(@BookMarkDestination,'FTXID')] | descendant::HyperlinkTextSource[contains(lower-case(@HyperlinkUDestination),'FTXID')]">
                    ...
                    <xsl:variable name="HUD">
                        ...
                        <xsl:choose>
                            <xsl:when test="contains(self::HyperlinkTextSource[contains(lower-case(@HyperlinkUDestination),'_story_')]/@HyperlinkUDestination | self::HyperlinkTextDestination/@BookMarkDestination,'_TX_')">
                                <xsl:choose>
                                    <xsl:when test="self::HyperlinkTextSource[contains(lower-case(@HyperlinkUDestination),'_story_')]/@HyperlinkUDestination">
                            ...
                            <xsl:when test="not(contains(self::HyperlinkTextSource/@HyperlinkUDestination | self::HyperlinkTextDestination/@BookMarkDestination,'_TX_'))">
                                <xsl:choose>
                                    <xsl:when test="self::HyperlinkTextSource[contains(lower-case(@HyperlinkUDestination),'_story_')]/@HyperlinkUDestination">
                                        <xsl:value-of select="self::HyperlinkTextSource[contains(lower-case(@HyperlinkUDestination),'_story_')]/@HyperlinkUDestination"/>
                                    </xsl:when>
                                    <xsl:when test="self::HyperlinkTextDestination[contains(@BookMarkDestination,'_UUID_') or contains(@BookMarkDestination,'story') or contains(@Name,'_UUID_') or contains(lower-case(@Name),'_story_')]/@BookMarkDestination">
                                        <xsl:value-of select="self::HyperlinkTextDestination[contains(@BookMarkDestination,'_UUID_') or contains(@BookMarkDestination,'story') or contains(@Name,'_UUID_') or contains(lower-case(@Name),'_story_')]/@BookMarkDestination"/>
                                    </xsl:when>
                                </xsl:choose>
                            </xsl:when>
                        </xsl:choose>
                ...
                <xsl:for-each select="descendant::HyperlinkTextDestination[not(contains(@BookMarkDestination,'_UU-Old-ID_'))][contains(@BookMarkDestination,'FTXID')][1] | descendant::HyperlinkTextSource[contains(lower-case(@HyperlinkUDestination),'FTXID')][1]">
                    <xsl:variable name="id">
                        <xsl:number level="any"/>
                    </xsl:variable>
                    <xsl:variable name="HUD">
...
<!--                            <xsl:element name="wp:wrapTopAndBottom"/>-->
    ...
    <xsl:template match="Change">
        ...
        <xsl:if test="ancestor::ParagraphStyleRange">
            <xsl:if test="ancestor::CharacterStyleRange[ancestor::Footnote]">
                <xsl:text disable-output-escaping="yes">&lt;/w:r&gt;</xsl:text> 
            </xsl:if>
            <xsl:choose>
                <xsl:when test="./Content">
                    <xsl:choose>
                        <xsl:when test="not($tc = 'none')">
                            <xsl:element name="{$tc}">
    ...
    <xsl:template match="Content" mode="track">
        <xsl:variable name="ch">
            <xsl:choose>
                ...
                <xsl:when test="ancestor::Change[not(@ChangeType)] or parent::Change[not(@ChangeType)]">
                    <xsl:value-of select="'w:t'"/>
                </xsl:when>
            </xsl:choose>
        </xsl:variable>
...
<!--        <xsl:if test="contains(@FontStyle,'Black') or contains(@FontStyle,'Bold') or contains(@FontStyle,'Strong') or contains(@FontStyle,'black') or contains(@FontStyle,'bold') or contains(@FontStyle,'strong') or contains(@FontStyle,'Extra') or contains(@FontStyle,'extra') or contains(@FontStyle,'Heavy') or contains(@FontStyle,'heavy') ">                -->
                <xsl:if test="contains(lower-case(@FontStyle),'medium') or contains(lower-case(@FontStyle),'black') or contains(lower-case(@FontStyle),'bold') or contains(lower-case(@FontStyle),'strong') or contains(lower-case(@FontStyle),'demi') or contains(lower-case(@FontStyle),'extra') or contains(lower-case(@FontStyle),'heavy') ">
                  <xsl:element name="w:b"/>
              </xsl:if>
              <xsl:if test="contains(lower-case(@FontStyle),'italic') or contains(lower-case(@FontStyle),'oblique')">
                  <xsl:element name="w:i"/>
...
<xsl:output-character character="�" string="&amp;#xFFFD;"/>
    ...
    <!--To avoid the property value as a text in Review xhtml footnote on  29-6-2023-->
    <xsl:template match="Footnote[descendant::ListItem[descendant::AppliedCharacterStyle[contains(.,'CharacterStyle/EndnoteNumber')]]]//ListItem[descendant::AppliedCharacterStyle[contains(.,'CharacterStyle/EndnoteNumber')]]"/>
    <xsl:template match="Footnote[ParagraphStyleRange[descendant::AllNestedStyles]]//AllNestedStyles"/>
</xsl:stylesheet>