...
<xsl:output method="xml" omit-xml-declaration="no" indent="no" cdata-section-elements="alt"/>
      ...
      <xsl:template match="oneXXX">   
           
            <xsl:variable name="firstListValue">
                  <xsl:if test="ancestor::GROUP/one[1]/@numberFormat">
                        <xsl:value-of select="ancestor::GROUP/one[1]/@numberFormat"/>
                  </xsl:if>
            </xsl:variable>
            
            
            <xsl:variable name="numFormat" select="@numberFormat"/>            
            <xsl:variable name="numFormat2" select="preceding-sibling::*[1]/@numberFormat"/>
            <xsl:variable name="numFormat3" select="following-sibling::*[1]/@numberFormat"/>
            <xsl:variable name="parent" select="preceding-sibling::one[1]/@numberFormat"/>
            
            
            
            <!-- 20-July-17: Added to check numberType (to determine disc bullet and circ bullet -->            
            <xsl:variable name="numType" select="@numberType"/>
            <xsl:variable name="numType2" select="preceding-sibling::*[1]/@numberType"/>
            <xsl:variable name="numType3" select="following-sibling::*[1]/@numberType"/>
            <xsl:variable name="parent2" select="preceding-sibling::GROUP/one[1]/@numberType"/>
            <!--END 20-July-17: Added to check numberType (to determine disc bullet and circ bullet -->
            
            <!--<xsl:choose>
                  <xsl:when test="$numFormat != $numFormat2">
                        <xsl:if test="$numFormat != $parent">
                              <xsl:text>&#x000A;</xsl:text>
                              <xsl:text disable-output-escaping="yes">&lt;GROUP&gt;</xsl:text>
                              <xsl:text>&#x000A;</xsl:text>
                        </xsl:if>
                  </xsl:when>
                  <xsl:when test="$numType != $numType2">                        
                        <xsl:if test="$numType != $parent2">
                              <xsl:text>&#x000A;</xsl:text>
                              <xsl:text disable-output-escaping="yes">&lt;GROUP&gt;</xsl:text>
                              <xsl:text>&#x000A;</xsl:text>
                        </xsl:if>
            ...
            <xsl:if test="$numFormat != $firstListValue">                  
                  <xsl:if test="$numFormat != $numFormat2">                  
                        <xsl:if test="$numFormat != $firstListValue">
                              <xsl:text>&#x000A;</xsl:text>
                              <xsl:text disable-output-escaping="yes">&lt;GROUP&gt;</xsl:text>
                              <xsl:text>&#x000A;</xsl:text>
            ...
            <!-- 20-July-17: Added to check numberType (to determine disc bullet and circ bullet -->
            <xsl:if test="$numFormat != $firstListValue">  
            <xsl:if test="$numType != $numType2">
                  <xsl:if test="$numType != $firstListValue">
                        <xsl:text>&#x000A;</xsl:text>
                        <xsl:text disable-output-escaping="yes">&lt;GROUP&gt;</xsl:text>
                        <xsl:text>&#x000A;</xsl:text>
                  </xsl:if>
            ...
            <!-- To close the dummy GROUPs -->
            <!--<xsl:choose>
                  <xsl:when test="$numFormat != $numFormat3">
                        <xsl:if test="$numFormat != $parent">
                              <xsl:text>&#x000A;</xsl:text>
                              <xsl:text disable-output-escaping="yes">&lt;/GROUP&gt;</xsl:text>    
                        </xsl:if>   
                  </xsl:when>
                  <xsl:when test="$numType != $numType3">
                        <xsl:if test="$numType != $parent2">
                              <xsl:text>&#x000A;</xsl:text>
                              <xsl:text disable-output-escaping="yes">&lt;/GROUP&gt;</xsl:text>    
                        </xsl:if>                        
                  </xsl:when>
                  <xsl:when test="not(following-sibling::*[1][self::one])">
                        <xsl:if test="$numFormat != $parent">
                              <xsl:text>&#x000A;</xsl:text>
                              <xsl:text disable-output-escaping="yes">&lt;/GROUP&gt;</xsl:text>    
                        </xsl:if>
            ...
            <!-- To close the dummy GROUPs ENDs here-->
            
            
            <xsl:if test="not($numFormat = $firstListValue)">
                  <xsl:if test="$numFormat != $numFormat3">
                        <xsl:if test="$numFormat != $firstListValue">
            ...
            <!-- 20-July-17: Added to check numberType (to determine disc bullet and circ bullet -->
                              ...
                              <xsl:if test="not(following-sibling::*[1][self::one])">                  
                                    <xsl:if test="$numType != $firstListValue">
                                          <xsl:text>&#x000A;</xsl:text>
                                          <xsl:text disable-output-escaping="yes">&lt;/GROUPss&gt;</xsl:text>    
                                    </xsl:if>
                              </xsl:if>
                              </xsl:if>-->
            <!--END 20-July-17: Added to check numberType (to determine disc bullet and circ bullet -->
