...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="html:div/html:p[not(html:span[@class='index2'])]">
        <xsl:copy>
            <xsl:apply-templates select="@*|node()"/>
        </xsl:copy>
        <xsl:text>&#x000A;</xsl:text>
    ...
    <xsl:template match="html:div/html:p[html:span[@class='index2']]">
        <xsl:variable name="semi" select="//html:span[contains(@class,'index') or not(@class) or @class='pageno']/following-sibling::text()[1][normalize-space(.) =';' or normalize-space(.) =',']"/>
        <xsl:copy>
            <xsl:apply-templates select="@*|node() except (html:span[@class='index2'] | $semi)"/>
        </xsl:copy>
        <xsl:text>&#x000A;</xsl:text>
        <xsl:if test="descendant::html:span[@class='index2']">
            <xsl:for-each select="html:span[@class='index2']">
                <xsl:element name="p">
                    <xsl:apply-templates select="current()/@*"/>
                    <xsl:apply-templates select="html:span[@class='index2']/current()/node()"/>
                </xsl:element>
                <xsl:text>&#x000A;</xsl:text>
            </xsl:for-each>
        </xsl:if>
