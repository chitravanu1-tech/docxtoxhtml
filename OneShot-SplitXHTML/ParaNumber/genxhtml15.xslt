    ...
    <xsl:choose>
      ...
      <xsl:when test="matches($xxx, 'TableRef')">        
        <xsl:element name="a">  
          ...
          <xsl:attribute name="href">
            <xsl:choose>
                <xsl:when test="self::w:sdt[following-sibling::linkend/@val]">
                <xsl:value-of select="concat($filename,'.xhtml#')"/>
                <xsl:value-of select="self::w:sdt[following-sibling::linkend/@val]/following-sibling::linkend[@val][1]/@val"/>
              </xsl:when>
              <xsl:otherwise>
      ...
      <xsl:when test="matches(lower-case($xxx), 'boxref')">
        <xsl:element name="a">
          ...
          <xsl:variable name="idval"
            select="child::w:sdtContent/w:p/w:sdt[1][w:sdtPr/w:alias/@w:val = 'SeqNo']/w:sdtContent/w:r[1]/w:t[1]"/>
          <xsl:attribute name="href">
            <xsl:choose>
              <xsl:when test="self::w:sdt[following-sibling::linkend/@val]">
                <xsl:value-of select="concat($filename,'.xhtml#')"/>
              ...
              <xsl:otherwise>
                ...
                <xsl:value-of select="normalize-space(translate($figSeq, ' .,;:-–)(', ''))"/>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:attribute> 
          <xsl:apply-templates/>
...
<!-- 02-Sep-17, for inside paragraphs formats -->
  ...
  <xsl:template match="//p">
    <xsl:choose>
      <xsl:when test="contains(@w:rsidR,'_unp') and not(descendant::w:t) and not(descendant::w:commentReference)">
      </xsl:when>
      <xsl:otherwise>
        <xsl:copy>
