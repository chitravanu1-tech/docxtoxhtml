...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="IndexEntry_First" select="$stylecoll//stylesList//style[@name='DZP_FB_IndexEntry_First']/@replacement"/>
    ...
    <xsl:template match="//html:div/html:p[not(@class='title')][not(number(@data-first))][not(lower-case(@data-first) = preceding::html:p/lower-case(@data-first))]">
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:attribute name="data-name">
                <xsl:choose>
                    <xsl:when test="string-length($IndexEntry_First) &gt; 0">
                        <xsl:value-of select="$IndexEntry_First"/>
                    </xsl:when>
                    <xsl:otherwise>
